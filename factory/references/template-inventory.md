...
| 4 | **AP7 `--primary-2: #2c3e6b` hardcode** — `_base.html:20` literal navy hex. Grep audit 2026-04-22: `var(--primary-2)` returns **0 hits** across `templates/`, `static/`, `apps/`. | AP7 / CS-PAL-03 / D3 (modern professionalism) | Dead-code declaration — violates the 3-token convention (CS-PAL-03) even though nothing consumes it. Trivial 1-line deletion. Bundle with the AP2 hardening diff at zero marginal cost. The original "Solaria inherits navy bias on every page edge" claim is invalidated by the 0-usage enumeration. |
| 5 | **AP12 reduced-motion JS coverage** — `_base.html:299-301` covers buttons; `[data-lm]` reveal animations driven by `static/js/live-motion.js` are unverified. Grep audit 2026-04-22: 45 `[data-lm]` hooks across 5 of 6 pages (all except `contact.html`). | AP12 / CS-RESPONSIVE-07 / E2 | Solaria's coaching audience includes motion-sensitive visitors; the archetype's WCAG 2.3.3 claim must hold. Fix is JS-side in `static/js/live-motion.js` and lands once. |
