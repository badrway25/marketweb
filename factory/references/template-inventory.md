...
**Audit baseline**: 2026-04-21 · **Refined**: 2026-04-22 · **Branch**: `phase-x4a-corporate-factory-hardening-step0`
...
|---|---:|---|---|
| `_base.html` | 549 | Shared chrome · tokens · nav · footer · RTL · editor-preview guards | **REUSABLE-AFTER-HARDENING** (AP2 responsive + AP7 `--primary-2` + AP12 motion JS pending) |
...
|:-:|---|---|---|
| 1 | The whole `templates/live_templates/business/corporate-suite/` shell | **REUSABLE-AFTER-HARDENING** | Skin works; AP2 responsive breakpoints + AP7 `--primary-2` decoupling + AP12 motion JS verification still pending. New pilots inherit those defects until the hardening pass closes. |
...
Each item below is a **systemic** defect — fixing it fixes Pragma, Fiscus, and Solaria simultaneously. These are the items that the X.4a hardening pass must close. The Solaria Commit B un-pause is **explicitly conditional** on these closing.

| # | Systemic issue | Anchor | Why it must precede Solaria Commit B |
|:-:|---|---|---|
| 1 | **AP1 enforceability** — `CS-PAL-01` is enforced today only by the Builder L\* self-check + the live browser walk. A pre-commit palette validator would be the automated belt-and-braces complement. | AP1 / CS-PAL-01 / O1 / D12 | Solaria's pre-fix palette (`#F7F3EC` at `e8f38b5`) is the originating incident. Re-introducing the same defect on the EN/FR/ES/AR locales is structurally impossible only if the validator (or the Builder self-check binding + browser walk) is in place. |
| 2 | **AP2 responsive coverage** — `_base.html` + 6 page files carry **0 real `@media` blocks** today (only `contact.html` has one at 880px). `CS-RESPONSIVE-01` requires breakpoints at 1100 and 720 px. | AP2 / CS-RESPONSIVE-01..08 / O2 / O3 / D13 | Adding EN/FR/ES/AR locales without responsive coverage means each of the 4 new locales × 6 pages × 8 viewports walks will FAIL the rubric per `BRWS-VIEW-02`. The hardening fix lands once and unlocks Solaria + retroactively hardens Pragma + Fiscus. |
| 3 | **AP3 `business-corporate` Pexels retro-pack** — Pragma's 6 Unsplash URLs are grandfathered (`CS-IMG-SRC-01` legacy exception). Until the retro-pack lands, the archetype carries one tolerated non-Pexels exception. | AP3 / CS-IMG-SRC-01 / CS-BLOCK-07 / O7 / D11 | Not a hard precondition for Solaria specifically (Solaria uses `business-coaching` Pexels), but the SOP §10.3 framing — "every pilot lands on rails that already caught a Solaria-class defect" — depends on Pragma not being a perpetual exception. |
| 4 | **AP7 `--primary-2: #2c3e6b` hardcode** — `_base.html:20` literal navy hex. Affects Solaria warm-earth + Fiscus charcoal palettes. | AP7 / CS-PAL-03 / D3 (modern professionalism) | Solaria's warm-carbon identity inherits a navy bias on every page edge until this is derived or dropped. Drift is visible; the fix lands once. |
| 5 | **AP12 reduced-motion JS coverage** — `_base.html:299-301` covers buttons; `[data-lm]` reveal animations driven by `static/js/live-motion.js` are unverified. | AP12 / CS-RESPONSIVE-07 / E2 | Solaria's coaching audience includes motion-sensitive visitors; the archetype's WCAG 2.3.3 claim must hold. Fix is JS-side and lands once. |
| 6 | **D-054 10-gate triangulation drift** — Pragma triangulates against Elevate (different archetype); Fiscus triangulates against Pragma only; Solaria's docstring (on branch) triangulates against Pragma + Fiscus. | `CS-EXEC-02` / `CS-BLOCK-12` / `O12` | This is a **content-side** systemic issue, not a skin issue. New pilots (planner-driven) must triangulate against ALL three siblings; Solaria when un-paused must include Pragma + Fiscus rows; Pragma + Fiscus docstrings should be refreshed on next touch. |
| 7 | **Multi-agent pipeline first-end-to-end run** — every standard in `factory/standards/*.md` is now populated, but no template has yet been walked end-to-end through the 10-agent pipeline (`SOP §4.1`). The first run will surface coordination defects (gate signals, evidence-directory naming, scorecard binding). | SOP §4.1 / §10.2 | Solaria un-pause should not be the pipeline's first end-to-end exercise. A re-walk of Fiscus through the populated agents is the safer first run; that walk's `verdict.md` + scorecard validates the pipeline before Solaria's 4 missing locales add 4× the surface area. |
