...
## 🟢 Current State (2026-04-18 · after Session 67 A.13 Chiara (editorial-designer-grid · portfolio family) Editor + Multi-locale Enrollment merge)

Baseline `phase-integration-baseline-v15` tip is **`e55d55b`** (A.13 merge), pushed to origin. **9 archetype slugs enrolled / 10 templates editabili end-to-end**. Ninth enrollment opens the portfolio family: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + ultra-luxury-cinematic + **editorial-designer-grid** are all multi-locale enrolled. Three families editor-complete (law · medical-specialist · real-estate). Portfolio family is OPEN: Chiara enrolled, Pixel (`cinematic-photographer`) explicitly held out until A.13b.
...
- **editorial-designer-grid** (Chiara · 1 template · A.13 · first dedicated-schema slot of the portfolio family) · ~140 scalar + 1 scalar image (`studio.founder.image` nested-dict · Vertex `home.cover.image` precedent) + 11 readonly indexed lists, **1 with image col at deep path 2 levels** (`home.featured_works.items[].image` × 4 rows — third precedent of image-in-dict-row after Vertex + Villa) — **5 total editable image surfaces** (1 scalar + 4 image cells) · 112 translatable · 212 global · `.ed-*` skin AR RTL (46 `html[dir=rtl]` rules · highest count of any enrolled archetype, Session 37 D-070 perfection pass) · complex shapes outside perimeter (posts list incl. per-project detail records + `studio.founder.credentials` + `processo.capabilities_full[].scope` + `home.clients` + `lavoro.filters` + form structure `form_fields`/`form_sections`/`upload_field`)
- **Multi-locale editor on all nine** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker on translatable fields · image overrides stay global/plain-keyed even inside dict rows at deep paths
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- 227/227 server tests passing, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **9 archetype slugs enrolled · 9 multi-locale enrolled · 10 templates editable end-to-end**
- **Families CLOSED (3)**: law (Lex + Juris sequential dedicated-schema · A.10+A.11) · medical-specialist (Cardio+Derm shared-schema · A.9) · real-estate (Casa+Villa staged dedicated-schema · A.12+A.12b)
- **Portfolio family OPEN but NOT YET CLOSED** — Chiara (editorial-designer-grid) enrolled, Pixel (cinematic-photographer) explicitly held out until A.13b. Pixel-out guard active at 2 layers: registration-time (`test_a13_chiara_archetype_registered` asserts `cinematic-photographer` NOT in `_ARCHETYPE_SCHEMAS` NOR `_MULTILOCALE_ENABLED_ARCHETYPES`) + execution-time (runtime `assertNotIn` inside the lifecycle test). Both guards will be removed in A.13b together with Pixel's own registrations.
- **Detail-page registry-only policy consolidated**: Lex `notabili` · Juris `insights` · Casa `posts` (property detail) · Villa `posts` (series detail) · Chiara `posts` (project detail) — 5 uniform enforcements. Explicit guardrail tests reject per-item detail paths in each archetype. Detail-page-scoped editing would be a horizontal feature affecting all 5+ archetypes with per-item content and is NOT in A.13 perimeter.
- **Families still open for editor enrollment (6 families · 10 templates total)**: **portfolio** (Pixel · 1 template · closes family) · **restaurant-continuation** (Sapore + Brace beyond Gusto · 2 templates) · **medical-other** (Salute + Benessere + Famiglia · 3 templates) · **ecommerce** (Bottega + Luxe · 2 templates) · **agency-secondary** (Aura beyond Vertex · 1 template) · **startup-saas** (Elevate · 1 template)

No explicitly-deferred debt is pending.

### Next workstream — immediate candidates (A.13b planning session recommended next)

- [ ] **A.13b Pixel (cinematic-photographer) enrollment — close the portfolio family** — natural continuation from A.13, mirrors A.12→A.12b real-estate closure (staged dedicated-schema progression). Pixel has `series_list` + `publications` novel page kinds (2 vs Chiara's 1 `process`) but simpler shape overall — mostly list-of-tuple, 1 scalar image on `home.hero_image`, no image-in-dict-row in first wave (posts stays registry-only per consistent policy including `posts[].cover_image`). Lighter than Chiara · recipe mechanical reuse. Expected schema LOC comfortably under 500 (Pixel has fewer sections and simpler list shapes). ~3 commits.
- [ ] **A.13b alt · restaurant-continuation (Sapore + Brace beyond Gusto)** — two templates, likely distinct archetypes (`trattoria-warm` + `street-modern` per DNA registry). Step-0 audit decides topology; most probable outcome is staged dedicated-schema closing the restaurant category.
- [ ] **A.13b alt · ecommerce family (Bottega + Luxe)** — `artisan-workshop` vs `fashion-editorial`. Session 41/42 observations suggest wide divergence. Bottega has commerce-foundation overlap (Phase 3a+3b already landed `apps/commerce/` seed for both) — worth a Step-0 check on whether the editor schema needs to avoid overlap with commerce-managed content.
- [ ] **A.13b alt · medical-other family (Salute + Benessere + Famiglia)** — three single-template medical archetypes (`medical-clinic` / `wellness-serene` / `family-warm`) rolled `published_live` in Session 51 but never enrolled. **Three separate phases preferred over bundling** — the "one-phase-one-archetype" discipline is now 9 phases strong (A.6 through A.13).
- [ ] **A.13b alt · Aura (agency-digital-studio) individual enrollment** — low novelty, low risk, single template. Aura is the second agency template beyond Vertex.
- [ ] **A.13b alt · Elevate (startup-saas-landing) individual enrollment** — low novelty, single template. Closes the startup-saas category.
- [ ] **A.13b alt · Selective editor polish** — search Cmd-K locale-aware ranking · "solo translatable" sidebar filter · badge-per-group pre-sync at mount · palette page-slug-match boost · sticky-last-page on reopen. Low-risk, low-value — only worth opening if customer signal surfaces.
- [ ] **A.13b alt · Media evolution / remote storage** — S3/Cloudinary swap of `ProjectAsset.file` backend. Must keep `/media/` accept path in parallel (D-095 binding). **Do NOT open unless a prod-launch timeline demands it**. A.12b+A.13 validated that current infra scales to image-heavy + deep-path archetypes without pressure — no technical trigger now.
- [ ] **A.13b alt · Editor operator tools** — admin-facing (zero customer surface): admin project list filters, GC scheduler, asset audit UI. Better after another scale-out consolidates the editor footprint.
- [ ] **A.13b alt · Repeater-per-locale first wave** — flip one or two mutable Vertex lists from global to translatable-per-locale. Out-of-scope family per D-098 · opens localized repeater content only if customer signal emerges.
- [ ] **A.13b alt · MEMORY.md maintenance mini-phase** — auto-memory index ~27-28KB sopra warning-soglia 24.4KB dal post-A.11. Index entries can be shortened without losing content. Not a blocker; open as a separate maintenance phase when convenient, NOT bundled with an enrollment.

Recommended framing: **A.13b Pixel Step-0 planning session** — closes the portfolio family. Half-open family is a stronger ROI than opening a seventh. Decision deferred to the dedicated A.13b planning session.
...
- Any bundled multi-archetype phase (violates "one phase = one archetype" discipline · 9 phases strong A.6→A.13)
- Coverage expansion on already-enrolled archetypes — would bloat the phase without customer pull
- Detail-page editing as single-archetype enhancement — the horizontal feature must be designed across all 5+ archetypes with per-item content (Lex/Juris/Casa/Villa/Chiara) or not at all
- Mixing Pixel with another archetype in A.13b (same discipline · one archetype decision per phase)
