...
## 🟢 Current State (2026-04-19 · after Session 73 A.16 Salute (clinic · medical-other family · first template) Editor + Multi-locale Enrollment merge · OPENS MEDICAL-OTHER FAMILY)

Baseline `phase-integration-baseline-v15` tip is **`e25b622`** (A.16 merge), pushed to origin. **15 archetype slugs enrolled / 16 templates editabili end-to-end**. Fifteenth enrollment opens the medical-other family: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + ultra-luxury-cinematic + editorial-designer-grid + cinematic-photographer + trattoria-warm + street-modern + artisan-workshop + fashion-editorial + **clinic** are all multi-locale enrolled. **Six families editor-complete** (law · medical-specialist · real-estate · portfolio · restaurant-continuation · ecommerce). **Medical-other family HALF-OPEN** (Salute IN · Benessere/Famiglia pending A.16b/A.16c). Two families still fully open (agency-secondary · startup-saas) · 4 templates residue editor-only-pending totale (Benessere + Famiglia + Aura + Elevate).
...
- **clinic** (Salute · 1 template · A.16 · **opens the medical-other family** via first dedicated-schema · 3-phase staged progression with Benessere A.16b + Famiglia A.16c · **first 3-template staged progression**) · 9 sidebar groups · ~95 scalar + **1 scalar top-level image** (`studio.photo_src` · about page photo) + 2 image-in-dict-row lists (home.team_ribbon_people × 8 + medici.doctors × 6) — **15 total editable image surfaces · ALL RENDERED** (specialist-family precedent · no storage-only split) · **16 readonly indexed list entries** · **tutti parent-level · ZERO deep-path** · ~70 translatable · `.cl-*` skin AR RTL (18 rules) · **2 novel page kinds** (prevention + appointment) · **posts list empty** · **no form editing** (2 form structures + form_sections OUT) · **NOVEL 5th OUT category precedent** (raw `icon_svg` × 18 fields) · **bool flag OUT** (`is_popular` · preserves `popular_label` text editability) · **DUAL-OUT GUARD planted** for both `wellness` + `family` (removed in A.16b + A.16c respectively · first 3-template progression · sub-recipe extends to 2 removal phases) · **boundary editor-vs-clinic-admin verified runtime** (LiveTemplateView doesn't import scheduler/booking/patient namespaces)
- **Multi-locale editor on all fifteen** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker · image overrides global/plain-keyed (rendered + storage-only identical contract)
- **Rendered vs storage-only image widget distinction is PER-SKIN** (Bottega typographic 16 rendered + 6 storage-only · Luxe editorial 31 rendered + 0 storage-only · Salute clinical 15 rendered + 0 storage-only): the schema exposes image fields identically · only skin rendering differs · registry data always preserved · editor contract uniform across all · storage plain-key per-locale identical
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- **309/309 server tests passing**, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **15 archetype slugs enrolled · 15 multi-locale enrolled · 16 templates editable end-to-end**
...
- **Medical-other family HALF-OPEN** — Salute (`clinic`) enrolled, Benessere (`wellness`) + Famiglia (`family`) explicitly held out until A.16b + A.16c respectively. **DUAL-OUT GUARD active at 3 layers** (registration-time + runtime start + runtime end of Salute lifecycle test) · will be removed via `test_a16b_benessere_out_guard_was_removed_from_salute_tests` + `test_a16c_family_out_guard_was_removed_from_salute_tests` · **5th + 6th precedente** del guard removal pattern (first 3-template progression · sub-recipe extends to 2 removal phases).
- **Staged dedicated-schema progression è la topologia dominante** con **4 closed precedents** (real-estate · portfolio · restaurant-continuation · ecommerce) **+ 1 half-open** (medical-other · 3-phase variant). Shared-schema (A.9 · 1 precedente) + sequential dedicated-schema (A.10+A.11 · 1 precedente) restano single-use. **3-phase variant è prima generalizzazione del pattern staged**.
- **Render-side contract-alignment fix `f66ac24` resta cross-pattern confirmed**: 3 editor layers fully aligned su list numeric indexing · Salute non tocca questo path (zero deep-path su 16 entries).
- **Detail-page registry-only policy enforced uniformly on 6 archetypes**: count invariato · Salute + Luxe + Bottega + Sapore + Brace shippano `posts: []` strutturalmente.
- **Guard removal pattern consolidated · 4 precedents + 2 pending**: Villa-out (A.12b) · Pixel-out (A.13b) · Brace-out (A.14b) · Luxe-out (A.15b) · **Wellness-out + Family-out pending A.16b/A.16c** · sub-recipe standard generalizza da 1-removal a 2-removal phase.
- **Outside-gate fixture rotation alla `wellness`/`benessere-centro-olistico`** · prossimo candidato enrollment è A.16b Benessere (medical-other family · 3-phase progression).
- **Image surface patterns con multiple precedents**: typographic-first single-no-image (Bottega · first) · single-scalar-image (Pragma · Pixel · Luxe · **Salute**) · multiple-scalar-images (Gusto · specialist · Villa · Sapore · Brace) · nested-dict-scalar-image (Vertex · Chiara · Bottega · Luxe con 2) · image-in-dict-row (Vertex · Villa · Chiara · Sapore · Brace · Bottega · Luxe · **Salute con 2 liste**)
- **Stringent IN col-level audit** estesa a **5 archetypes**: Sapore (A.14) → Brace (A.14b) → Bottega (A.15) → Luxe (A.15b) → **Salute (A.16 · num + popular_label)** · audit-driven · fields editorial visible stay IN · technical/structural/bool stay OUT
- **NOVEL 5th OUT category precedent** (A.16): raw `icon_svg` fields col-level OUT · safety + poor UX rationale documented · joins flat-list-of-str + nested-list-of-str + form structures + structural identifiers/commerce-state-like booleans
- **Boundary editor-vs-{commerce,clinic}-admin clean · runtime-verified su 3 archetypes**: editor edits presentational demo showcase content in template_content registry · {commerce,clinic}-admin scopes (apps/commerce + future apps/clinic_admin) managed separately via dedicated dashboards · light smoke tests su Bottega + Luxe + Salute
- **Families still open for editor enrollment (3 families · 4 templates total · 1 family half-open)**: **medical-other half-open** (Benessere + Famiglia · 2 templates · A.16b + A.16c) · **agency-secondary** (Aura · 1 template) · **startup-saas** (Elevate · 1 template)

No explicitly-deferred debt is pending. **16 commit-clean phases** delivered back-to-back (A.6 → A.16). A.16 was 3 commits on branch (schema · lifecycle · merge) · zero additional infra fixes required (mechanical reuse di tutti i pattern pre-esistenti · 5th consecutive staged enrollment).

### Next workstream — immediate candidates (medical-other HALF-OPEN · Benessere chiude prossima · Famiglia chiude family)

A.16 opens the medical-other family. Half-open · 2 siblings pending. Top candidate is **A.16b Benessere** · staged middle-phase di un'estensione 3-fase (Salute opener · Benessere middle · Famiglia closer).

- [ ] **A.16b · Benessere (`wellness`) enrollment — middle phase of medical-other** — natural continuation from A.16 · single-template enrollment removes wellness-out guard (keeps family-out active). Step-0 audit needed for: `home.calendar`/`prenota.calendar` `has_slots`/`soldout` bool flags (commerce-state-like · same OUT decision as Luxe `available` + Salute `is_popular`) · 1 novel page kind `gallery` (`ambienti.rooms[].image` × 8 · 5th image-in-dict-row precedent) · `contatti.form_fields.interest_options` nested str-list OUT · 19 image surfaces (3 scalar + 16 image-in-dict-row across 3 lists). Expected schema LOC ~550-650 (moderate-high · richer than Salute due to more lists + calendar shape).
- [ ] **A.16b alt · Famiglia (`family`) enrollment first** — alternative ordering · Famiglia has deep-path tuple-in-dict-list (`crescita.topics[].items` · Sapore-precedent) requiring 4 STRUCTURED_FIELD_SHAPES entries one per topic · novel `faq` page kind · NO form_fields (form as flat scalars) · simpler form OUT policy. If Step-0 audit identifies preference for tackling deep-path complexity later, could be deferred to A.16c.
- [ ] **A.16b alt · Aura (`agency-digital-studio`) individual enrollment** — low novelty, low risk, single template. Closes agency-secondary category. Pulls focus away from medical-other half-open · only justifiable if customer signal surfaces.
- [ ] **A.16b alt · Elevate (`startup-saas-landing`) individual enrollment** — low novelty, single template. Closes startup-saas. Same trade-off as Aura.
- [ ] **A.16b alt · MEMORY.md maintenance mini-phase** — auto-memory index ~33-34KB sopra warning-soglia 24.4KB (grows ~1KB per enrollment phase). Index entries can be shortened without losing content. **Separate housekeeping task, NOT bundled with an enrollment**. Low-value-low-risk.
- [ ] **A.16b alt · Selective editor polish** — defer unless customer signal surfaces.
- [ ] **A.16b alt · Media evolution / remote storage (S3/Cloudinary)** — defer. Current infra scales through 15 archetypes with 31 image surfaces + deep-path · no technical trigger.
- [ ] **A.16b alt · Editor operator tools** — defer.
- [ ] **A.16b alt · Repeater-per-locale first wave** — defer. Out-of-scope family per D-098.

Recommended framing: **A.16b Benessere Step-0 planning session** — closes the wellness-out guard half of the dual-out planted in A.16 (family-out stays active for A.16c) · half-open family is stronger ROI than opening a new front. Calendar `has_slots`/`soldout` bool flags need explicit OUT decision (Luxe `available` precedent + Salute `is_popular` precedent already established).

**What NOT to open immediately (red-lamps)**:
- **Do NOT bundle Benessere + Famiglia into one A.16b** · discipline violation (one phase = one archetype decision · 16 phases strong A.6→A.16)
- **Do NOT attempt shared-schema between Salute + Benessere + Famiglia** · Step 0 audit already confirmed shared-schema impossible (~0% content-tree overlap · 3 distinct skin folders · 3 distinct DNA archetypes)
- **Do NOT skip Benessere to enroll Famiglia first** unless Step-0 audit identifies clear infra leverage (Famiglia deep-path is the only structural reason · normally we close opener-adjacent siblings first)
- **Do NOT re-open Salute coverage expansion** · A.16 is closed · any additional Salute coverage would be scope-creep
- **Do NOT open clinic-admin editing on Salute/Benessere/Famiglia** · boundary editor-vs-clinic-admin fermo · scheduler/booking/patient namespaces stay out of scope
...
- **Do NOT add bool field type for `has_slots`/`soldout`/`is_popular`** · horizontal feature · all 3 stay OUT col-level (Luxe + Salute precedents)
...
- Any bundled multi-archetype phase (16 phases A.6→A.16 have all respected discipline · preserve)
