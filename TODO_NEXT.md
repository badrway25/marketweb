...
## 🟢 Current State (2026-04-19 · after Session 72 A.15b Luxe (fashion-editorial · ecommerce family · second template) Editor + Multi-locale Enrollment merge · CLOSES ECOMMERCE FAMILY)

Baseline `phase-integration-baseline-v15` tip is **`bd9ffcb`** (A.15b merge), pushed to origin. **14 archetype slugs enrolled / 15 templates editabili end-to-end**. Fourteenth enrollment closes the ecommerce family: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + ultra-luxury-cinematic + editorial-designer-grid + cinematic-photographer + trattoria-warm + street-modern + artisan-workshop + **fashion-editorial** are all multi-locale enrolled. **Six families editor-complete** (law · medical-specialist · real-estate · portfolio · restaurant-continuation · ecommerce). Three families still fully open (medical-other · agency-secondary · startup-saas) · 5 templates residue editor-only-pending totale (3 medical-other + 1 Aura + 1 Elevate).
...
- **Multi-locale editor on all fourteen** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker · image overrides global/plain-keyed (rendered + storage-only identical contract)
- **Rendered vs storage-only image widget distinction is PER-SKIN** (Bottega typographic 16 rendered + 6 storage-only · Luxe editorial 31 rendered + 0 storage-only): the schema exposes image fields identically · only skin rendering differs · registry data always preserved · editor contract uniform across both · storage plain-key per-locale identical
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- **295/295 server tests passing**, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **14 archetype slugs enrolled · 14 multi-locale enrolled · 15 templates editable end-to-end**
...
- **Staged dedicated-schema progression è la topologia dominante** con **4 closed precedents** (real-estate · portfolio · restaurant-continuation · ecommerce). Shared-schema (A.9 · 1 precedente) + sequential dedicated-schema (A.10+A.11 · 1 precedente) restano single-use.
- **Render-side contract-alignment fix `f66ac24` resta cross-pattern confirmed**: 3 editor layers fully aligned su list numeric indexing · entrambi tuple-in-dict-list e dict-in-dict-list parent. Luxe non tocca questo path (zero deep-path su 17 entries).
- **Detail-page registry-only policy enforced uniformly on 6 archetypes**: count invariato · Luxe + Bottega + Sapore + Brace shippano `posts: []` strutturalmente.
- **Guard removal pattern consolidated · 4 precedents**: Villa-out (A.12b) · Pixel-out (A.13b) · Brace-out (A.14b) · **Luxe-out (A.15b · `test_a15b_luxe_out_guard_was_removed_from_bottega_tests`)** · sub-recipe standard per ogni staged closure phase.
- **Outside-gate fixture rotation alla `clinic`/`salute-studio-medico`** · prossimo candidato enrollment è medical-clinic family (Salute · Benessere · Famiglia) · 3 templates editor-only-pending con 5-locale parity già shipped in Sessions 51/52.
- **Image surface patterns con multiple precedents**: typographic-first single-no-image (Bottega · first) · single-scalar-image (Pragma · Pixel · **Luxe**) · multiple-scalar-images (Gusto · specialist · Villa · Sapore · Brace) · nested-dict-scalar-image (Vertex · Chiara · Bottega · **Luxe** con 2 nested-dict scalar) · image-in-dict-row (Vertex · Villa · Chiara · Sapore · Brace · Bottega · **Luxe** con 6 liste)
- **Stringent IN col-level audit** estesa a 4 archetypes: Sapore forno.pizza_signatures.n (A.14) → Brace ordina routes n (A.14b) → Bottega shop.products n/edition/tag (A.15) → **Luxe collezione.products drop/n/tag (A.15b)** · audit-driven · fields editorial visible stay IN · id/available stay OUT
- **Boundary editor-vs-commerce-admin clean · runtime-verified su both ecommerce archetypes**: editor edits presentational demo showcase content in template_content registry · `apps/commerce/` managed separately via seller dashboard Phase 3a/3b · light smoke test su entrambi Bottega + Luxe
- **Families still open for editor enrollment (3 families · 5 templates total)**: **medical-other** (Salute + Benessere + Famiglia · 3 templates) · **agency-secondary** (Aura · 1 template) · **startup-saas** (Elevate · 1 template)

No explicitly-deferred debt is pending. **15 commit-clean phases** delivered back-to-back (A.6 → A.15b). A.15b was 3 commits on branch (schema · lifecycle · merge) · zero additional infra fixes required (mechanical reuse di tutti i pattern pre-esistenti · 4th consecutive staged closure).

### Next workstream — immediate candidates (ecommerce CLOSED · three single-template families residue)

A.15b closes the ecommerce family. No half-open families remain. Residual editor-only-pending templates are 5 across 3 families:
- **medical-other** (3 templates · `clinic` Salute + `wellness` Benessere + `family` Famiglia)
- **agency-secondary** (1 template · `agency-digital-studio` Aura)
- **startup-saas** (1 template · `startup-saas-landing` Elevate)

All 3 medical-other templates share 5-locale parity PERFECT since Sessions 51/52 · outside-gate fixture already rotated to `clinic`/`salute-studio-medico` in A.15b as next candidate signal.

- [ ] **A.16 · Salute (`clinic`) enrollment** — first of the medical-other family. Single-template enrollment. Precedent match: A.10 Lex (single-template inside a multi-template category) — Benessere + Famiglia stay OUT via Salute-out dual guard planted Step 1 · removed in A.16b/A.16c. Step-0 audit needed: Salute booking-widget shape + wellness calendar-spot + family phone-and-chat are all distinct archetypes (no shared-schema possibility across all 3 medical-other). Expected schema LOC ~450-550 (moderate · similar shape to Casa).
- [ ] **A.16 alt · Benessere (`wellness`) enrollment first** — serene sage + calendar-spot DNA. Alternative ordering if Step-0 audit identifies stronger infra leverage.
- [ ] **A.16 alt · Aura (`agency-digital-studio`) individual enrollment** — low novelty, low risk, single template. Closes agency-secondary category (Vertex already enrolled as `agency-creative-studio`). Not a family opener — Aura is the only template in agency-digital-studio.
- [ ] **A.16 alt · Elevate (`startup-saas-landing`) individual enrollment** — low novelty, single template. Closes startup-saas category.
- [ ] **A.16 alt · MEMORY.md maintenance mini-phase** — auto-memory index ~31-32KB sopra warning-soglia 24.4KB (grows ~1KB per enrollment phase). Index entries can be shortened without losing content. **Separate housekeeping task, NOT bundled with an enrollment**. Low-value-low-risk.
- [ ] **A.16 alt · Selective editor polish** — defer unless customer signal surfaces.
- [ ] **A.16 alt · Media evolution / remote storage (S3/Cloudinary)** — defer. Current infra scales through 14 archetypes with 31 image surfaces + deep-path · no technical trigger.
- [ ] **A.16 alt · Editor operator tools** — defer.
- [ ] **A.16 alt · Repeater-per-locale first wave** — defer. Out-of-scope family per D-098.

Recommended framing: **A.16 Salute Step-0 planning session** — first of the medical-other family · opens a 3-template staged progression (A.16 Salute + A.16b Benessere + A.16c Famiglia). Alternative: single-template non-family enrollment (Aura or Elevate).

**What NOT to open immediately (red-lamps)**:
- **Do NOT bundle Salute + Benessere + Famiglia into one A.16** · discipline violation (one phase = one archetype decision · 15 phases strong A.6→A.15b)
- **Do NOT attempt shared-schema for medical-other** unless Step-0 audit confirms ≥80% content-tree overlap · all 3 have divergent DNA (booking-widget vs calendar-spot vs phone-and-chat) per Sessions 51/52
- **Do NOT re-open ecommerce coverage expansion on Luxe or Bottega** · A.15b is closed · any additional coverage would be scope-creep
- **Do NOT open commerce-admin editing on Luxe/Bottega** · boundary editor-vs-commerce-admin fermo · apps/commerce resta scope seller dashboard
- **Do NOT open product-detail per-item editing assuming backend catalog** · demo content è registry-driven · real catalog ha il suo CRUD
...
- Any bundled multi-archetype phase (15 phases A.6→A.15b have all respected discipline · preserve)
