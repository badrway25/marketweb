...
## 🟢 Current State (2026-04-18 · after Session 69 A.14 Sapore (trattoria-warm · restaurant-continuation family · first template) Editor + Multi-locale Enrollment merge · OPENS RESTAURANT-CONTINUATION FAMILY)

Baseline `phase-integration-baseline-v15` tip is **`8fae2df`** (A.14 merge), pushed to origin. **11 archetype slugs enrolled / 12 templates editabili end-to-end**. Eleventh enrollment opens the restaurant-continuation family: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + ultra-luxury-cinematic + editorial-designer-grid + cinematic-photographer + **trattoria-warm** are all multi-locale enrolled. **Four families editor-complete** (law · medical-specialist · real-estate · portfolio). Five families still open for editor enrollment, one of them (restaurant-continuation) now half-open (Sapore IN · Brace pending A.14b) — 8 templates residue across the 5 open families.
...
- **trattoria-warm** (Sapore · 1 template · A.14 · first dedicated-schema slot of the restaurant-continuation family) · 8 sidebar groups · ~141 scalar + **7 scalar image fields** (home.hero_image · home.forno_image · home.tavolata_image · storia.photo_image · forno.forno_story_image · forno.dough_image · eventi.birthday_image) + 2 image-in-dict-row lists (home.family + storia.family, 3 portrait cells each) — **13 total editable image surfaces** · 20 readonly indexed lists (15 base + **5 novel `menu.sections.{i}.dishes` deep-path tuple-in-dict-list parent entries**) · 121 translatable · 296 global · `.tw-*` skin AR RTL (18 `html[dir=rtl]` rules · Session 48 D-078 Sapore rollout) · complex shapes outside perimeter (storia.story paragraphs · contatti.form_sections/form_fields/occasion_options · site.hours_footer_rows · pages) · **posts list empty in the registry** (first enrollment since A.10 without posts.* exclusion paths)
- **Multi-locale editor on all eleven** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker on translatable fields · image overrides stay global/plain-keyed including at deep paths · **deep-path menu cell overrides also stay global/plain-keyed** (verified end-to-end on Sapore menu.sections.0.dishes.0.name)
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- 252/252 server tests passing, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **11 archetype slugs enrolled · 11 multi-locale enrolled · 12 templates editable end-to-end**
- **Families CLOSED (4)**: law (Lex + Juris sequential dedicated-schema · A.10+A.11) · medical-specialist (Cardio+Derm shared-schema · A.9) · real-estate (Casa+Villa staged dedicated-schema · A.12+A.12b) · portfolio (Chiara+Pixel staged dedicated-schema · A.13+A.13b)
- **Restaurant-continuation family OPEN but NOT YET CLOSED** — Sapore (`trattoria-warm`) enrolled, Brace (`street-modern`) explicitly held out until A.14b. Brace-out guard active at 2 layers: registration-time (`test_a14_sapore_archetype_registered` asserts `street-modern` NOT in `_ARCHETYPE_SCHEMAS` NOR `_MULTILOCALE_ENABLED_ARCHETYPES`) + execution-time (runtime `assertNotIn` inside the lifecycle test). Both guards will be removed in A.14b together with Brace's own registrations, contract-tested via `test_a14b_brace_out_guard_was_removed_from_sapore_tests` mirror.
- **Deep-path tuple-in-dict-list parent is a new precedentato shape** · 3 editor layers now aligned on list numeric indexing: `services._resolve_path` (always supported) · `schema._resolve_path` (A.14 Step 1 extension) · `rendering._apply_indexed` (A.14 Step 2 contract-alignment bugfix · commit `f66ac24`). Menu rows inside perimeter is precedent for future restaurant-family enrollments where menu is the editorial heart.
- **Detail-page registry-only policy enforced uniformly on 6 archetypes**: Lex `notabili` · Juris `insights` · Casa `posts` · Villa `posts` · Chiara `posts` · Pixel `posts` + `series_detail`. Sapore ships no posts list (structural absence, not a perimeter decision), so the enforcement count stays at 6.
- **Image surface patterns with multiple precedents**: single-scalar-image (Pragma · Pixel) · multiple-scalar-images (Gusto · specialist · Villa · Sapore) · nested-dict-scalar-image (Vertex · Chiara) · image-in-dict-row (Vertex A.3a/A.4 · Villa · Chiara deep path 2 levels · Sapore home.family + storia.family). The recipe scales downward and upward.
- **Families still open for editor enrollment (5 families · 8 templates total)**: **restaurant-continuation** (Brace · 1 template · closes family in A.14b) · **medical-other** (Salute + Benessere + Famiglia · 3 templates) · **ecommerce** (Bottega + Luxe · 2 templates) · **agency-secondary** (Aura · 1 template) · **startup-saas** (Elevate · 1 template)

No explicitly-deferred debt is pending. 12 commit-clean phases delivered back-to-back (A.6 → A.14). A.14 was 4 commits total (schema · render fix · lifecycle test · merge).

### Next workstream — immediate candidates (family half-open; top candidate is family closure)

A.14 opens a family; it does not close one. Top candidate is **A.14b Brace** (close restaurant-continuation) — natural continuation of the staged dedicated-schema progression pattern now applied three times consecutively (real-estate, portfolio, restaurant-continuation).

- [ ] **A.14b Brace (street-modern) enrollment — close the restaurant-continuation family** — natural continuation from A.14, mirrors A.12→A.12b and A.13→A.13b closures. Brace has a heavier shape than Sapore (18 lists-of-dict vs 12 · 5 image-in-dict-row lists vs 2 · ~45 image surfaces vs 13 · food photos per dish in `menu.sections[].items[].image`) **but no new infrastructure needed** because A.14 Step 2 already aligned the 3 editor layers on list numeric indexing, and Chiara/Villa already precedentated dict-in-dict-list image cols. Recipe: mechanical-reuse of A.14 pattern. Expected schema LOC potentially above 600 (Brace has more lists) — non-blocking exception acceptable. ~3-4 commits (schema · lifecycle · optional render polish · merge).
- [ ] **A.14b alt · eCommerce family (Bottega + Luxe)** — `artisan-workshop` vs `fashion-editorial`. Session 41/42 observations suggest wide divergence. Bottega has commerce-foundation overlap (Phase 3a+3b already landed `apps/commerce/` seed for both) — worth a Step-0 check on whether the editor schema needs to avoid overlap with commerce-managed content (the editor should edit presentational/marketing content, not commerce-admin state).
- [ ] **A.14b alt · Medical-other family (Salute + Benessere + Famiglia)** — three single-template medical archetypes (`medical-clinic` / `wellness-serene` / `family-warm`) rolled `published_live` in Session 51 but never enrolled. **Three separate phases preferred over bundling** — the "one-phase-one-archetype" discipline is now 12 phases strong (A.6 through A.14).
- [ ] **A.14b alt · Aura (agency-digital-studio) individual enrollment** — low novelty, low risk, single template. Aura is the second agency template beyond Vertex.
- [ ] **A.14b alt · Elevate (startup-saas-landing) individual enrollment** — low novelty, single template. Closes the startup-saas category.
- [ ] **A.14b alt · MEMORY.md maintenance mini-phase** — auto-memory index ~28-29KB sopra warning-soglia 24.4KB. Index entries can be shortened without losing content. **Separate housekeeping task, NOT bundled with an enrollment**. Low-value-low-risk; queue for a "breather" session between bigger phases.
- [ ] **A.14b alt · Selective editor polish** — search Cmd-K locale-aware ranking · "solo translatable" sidebar filter · badge-per-group pre-sync at mount · palette page-slug-match boost · sticky-last-page on reopen. Low-risk, low-value — only worth opening if customer signal surfaces.
- [ ] **A.14b alt · Media evolution / remote storage** — S3/Cloudinary swap of `ProjectAsset.file` backend. Must keep `/media/` accept path in parallel (D-095 binding). **Do NOT open unless a prod-launch timeline demands it**. A.12b+A.13+A.13b+A.14 validated that current infra scales to image-heavy + deep-path + single-scalar + menu-deep-path archetypes without pressure — no technical trigger now.
- [ ] **A.14b alt · Editor operator tools** — admin-facing (zero customer surface): admin project list filters, GC scheduler, asset audit UI. Better after another scale-out consolidates the editor footprint.
- [ ] **A.14b alt · Repeater-per-locale first wave** — flip one or two mutable Vertex lists from global to translatable-per-locale. Out-of-scope family per D-098 · opens localized repeater content only if customer signal emerges.

Recommended framing: **A.14b Brace Step-0 planning session** — closes the restaurant-continuation family. Half-open family is a stronger ROI than opening a sixth. Decision deferred to the dedicated A.14b planning session.

**What NOT to open immediately (value is low right now)**:
- Remote asset storage (no technical pressure)
- Repeater-per-locale family (no customer request)
- Search Cmd-K localized ranking (polish only)
- Editor operator tools (better after scale-out consolidates footprint)
- Any bundled multi-archetype phase (violates "one phase = one archetype" discipline · 12 phases strong A.6→A.14)
- Coverage expansion on already-enrolled archetypes — would bloat the phase without customer pull
- Detail-page editing as single-archetype enhancement — the horizontal feature must be designed across all 6+ archetypes with per-item content (Lex/Juris/Casa/Villa/Chiara/Pixel) or not at all
- Mixing Brace with another archetype in A.14b (same discipline · one archetype decision per phase)
- Bundling MEMORY.md maintenance with an enrollment (it's housekeeping; stays separate)
