...
## 🟢 Current State (2026-04-19 · after Session 71 A.15 Bottega (artisan-workshop · ecommerce family · first template) Editor + Multi-locale Enrollment merge · OPENS ECOMMERCE FAMILY)

Baseline `phase-integration-baseline-v15` tip is **`fab3ebc`** (A.15 merge), pushed to origin. **13 archetype slugs enrolled / 14 templates editabili end-to-end**. Thirteenth enrollment opens the ecommerce family: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + ultra-luxury-cinematic + editorial-designer-grid + cinematic-photographer + trattoria-warm + street-modern + **artisan-workshop** are all multi-locale enrolled. **Five families editor-complete** (law · medical-specialist · real-estate · portfolio · restaurant-continuation). Ecommerce family half-open (Bottega IN · Luxe pending A.15b). Three families still fully open (medical-other · agency-secondary · startup-saas) · 6 templates residue editor-only-pending totale (1 Luxe + 3 medical-other + 1 Aura + 1 Elevate).
...
- **artisan-workshop** (Bottega · 1 template · A.15 · first dedicated-schema slot of the ecommerce family · **SHOWCASE/template-content editing NOT commerce-admin editing**) · 8 sidebar groups · ~141 scalar + **0 scalar top-level image** (typographic-first DNA per Session 42 D-073 · `.aw-*` crest-mark glyphs) + 2 nested-dict scalar images (`product.artisan_portrait` · `atelier.founder_portrait`) + 4 image-in-dict-row lists (home.latest_items × 4 · home.makers × 4 · shop.products × 9 · product.related_items × 3) — **22 total editable image surfaces** · **14 readonly indexed list entries** · **tutti parent-level · ZERO deep-path** (Bottega has no list-nested-in-list-parent · simpler than Sapore/Brace) · 125 translatable · `.aw-*` skin AR RTL (31 rules) · **3 novel page kinds** (shop · product · journal) · **posts list empty** · **no form editing** · `contatti.form_fields` OUT · **boundary editor-vs-commerce-admin verified runtime** (LiveTemplateView doesn't import apps.commerce · real catalog lives separately in `apps/commerce/` models managed via seller dashboard)
- **Multi-locale editor on all thirteen** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker · image overrides global/plain-keyed (rendered + storage-only identical contract)
- **Rendered vs storage-only image widget distinction is INTENTIONAL** (intro'd in Bottega A.15): typographic-first archetypes expose portrait fields for registry-data preservation · skin chooses whether to paint them as photos or typographic stamps · editor contract blindato per storage regardless of rendering. Browser-walk verified 22 Bottega widgets = 16 rendered + 6 storage-only · all persist plain-key.
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- 280/280 server tests passing, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **13 archetype slugs enrolled · 13 multi-locale enrolled · 14 templates editable end-to-end**
...
- **Ecommerce family OPEN but NOT YET CLOSED** — Bottega (`artisan-workshop`) enrolled, Luxe (`fashion-editorial`) explicitly held out until A.15b. Luxe-out guard active at 2 layers (registration-time + runtime) · will be removed via `test_a15b_luxe_out_guard_was_removed_from_bottega_tests` · **fourth precedente** del guard removal pattern dopo Villa/Pixel/Brace.
- **Staged dedicated-schema progression è ora la topologia dominante** con 3 closed precedents (real-estate · portfolio · restaurant-continuation) + 1 half-open (ecommerce). Shared-schema (A.9 · 1 precedente) + sequential dedicated-schema (A.10+A.11 · 1 precedente) restano single-use.
- **Render-side contract-alignment fix `f66ac24` è cross-pattern operationally confirmed** (from A.14b): 3 editor layers fully aligned su list numeric indexing · entrambi tuple-in-dict-list e dict-in-dict-list parent. Bottega non tocca questo path (no deep-path).
- **Detail-page registry-only policy enforced uniformly on 6 archetypes**: count invariato · Bottega + Sapore + Brace shippano `posts: []` strutturalmente.
- **Guard removal pattern consolidated** · 3 precedents: Villa-out (A.12b) · Pixel-out (A.13b) · Brace-out (A.14b) · Luxe-out (A.15b) come quarto pending.
- **Image surface patterns con multiple precedents**: typographic-first single-no-image (Bottega · first) · single-scalar-image (Pragma · Pixel) · multiple-scalar-images (Gusto · specialist · Villa · Sapore · Brace) · nested-dict-scalar-image (Vertex · Chiara · **Bottega**) · image-in-dict-row (Vertex · Villa · Chiara · Sapore · Brace · **Bottega**)
- **Boundary editor-vs-commerce-admin clean · runtime-verified**: editor edits presentational demo showcase content in template_content registry · `apps/commerce/` (Storefront/Product/Variant/Cart/Order/PaymentIntent) managed separately via seller dashboard Phase 3a/3b. Two surfaces ortogonali · zero cross-reference · light smoke test blindates the separation at schema-path level.
- **Families still open for editor enrollment (4 families · 6 templates total)**: **ecommerce** (Luxe · 1 template · chiude family in A.15b) · **medical-other** (Salute + Benessere + Famiglia · 3 templates) · **agency-secondary** (Aura · 1 template) · **startup-saas** (Elevate · 1 template)

No explicitly-deferred debt is pending. 14 commit-clean phases delivered back-to-back (A.6 → A.15). A.15 was 3 commits on branch (schema · lifecycle · merge) · zero additional infra fixes required (mechanical reuse di tutti i pattern pre-esistenti).

### Next workstream — immediate candidates (ecommerce family half-open · Luxe chiude)

A.15 opens the ecommerce family; it does NOT close it. Top candidate is **A.15b Luxe** · quarto staged closure consecutivo dopo real-estate · portfolio · restaurant-continuation.

- [ ] **A.15b · Luxe (`fashion-editorial`) enrollment — close the ecommerce family** — natural continuation from A.15, mirrors A.12→A.12b / A.13→A.13b / A.14→A.14b closures. Luxe has heavier shape than Bottega (12 lists-of-dict vs 10 · 6 image-in-dict-row lists vs 4 · 31 image surfaces vs 22 · lookbook adds image-heavy editorial band with 6-image grid) **but no new infrastructure needed** because A.15 Bottega fully reused existing infra (nested-dict scalar + shallow image-in-dict-row · both precedented). Recipe: mechanical-reuse of A.15 Bottega pattern (typographic-vs-editorial skin divergence non cambia boundary editor-vs-commerce-admin · entrambi showcase template-content). Expected schema LOC ~650-700 (Luxe has more lists) — non-blocking exception acceptable.
- [ ] **A.15b alt · Medical-other family (Salute + Benessere + Famiglia)** — three single-template medical archetypes (`medical-clinic` · `wellness-serene` · `family-warm`) rolled `published_live` in Session 51 but never enrolled. **Three separate phases preferred over bundling** — the "one-phase-one-archetype" discipline is now 14 phases strong (A.6 → A.15). Lower leverage than Luxe (Luxe closes a family immediately).
- [ ] **A.15b alt · Aura (agency-digital-studio) individual enrollment** — low novelty, low risk, single template. Closes agency-secondary category (Vertex already enrolled as `agency-creative-studio`).
- [ ] **A.15b alt · Elevate (startup-saas-landing) individual enrollment** — low novelty, single template. Closes the startup-saas category.
- [ ] **A.15b alt · MEMORY.md maintenance mini-phase** — auto-memory index ~31-32KB sopra warning-soglia 24.4KB (grows ~1KB per enrollment phase). Index entries can be shortened without losing content. **Separate housekeeping task, NOT bundled with an enrollment**. Low-value-low-risk.
- [ ] **A.15b alt · Selective editor polish** — defer unless customer signal surfaces.
- [ ] **A.15b alt · Media evolution / remote storage (S3/Cloudinary)** — defer. Current infra scales through 13 archetypes with 44-image-surface + deep-path · no technical trigger.
- [ ] **A.15b alt · Editor operator tools** — defer.
- [ ] **A.15b alt · Repeater-per-locale first wave** — defer. Out-of-scope family per D-098.

Recommended framing: **A.15b Luxe Step-0 planning session** — closes the ecommerce family opened in A.15. Half-open family is a stronger ROI than opening a sixth.

**What NOT to open immediately (red-lamps)**:
- **Do NOT bundle Luxe + medical-other / Aura / Elevate** · discipline violation (one phase = one archetype decision · 14 phases strong)
- **Do NOT re-open ecommerce coverage expansion on Bottega** · A.15 is closed · any additional Bottega coverage would be scope-creep
- **Do NOT open commerce-admin editing on Luxe** · boundary editor-vs-commerce-admin è fermo · apps/commerce resta scope seller dashboard
...
- Any bundled multi-archetype phase (14 phases A.6→A.15 have all respected discipline · preserve)
- Coverage expansion on already-enrolled archetypes · bloats phase without customer pull
- Mixing Luxe with another archetype in A.15b (same discipline)
