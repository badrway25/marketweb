...
## 🟢 Current State (2026-04-19 · after Session 75 A.16c Famiglia (family · medical-other family · third template · CLOSER PHASE) Editor + Multi-locale Enrollment merge · CLOSES MEDICAL-OTHER FAMILY)

Baseline `phase-integration-baseline-v15` tip is **`761044c`** (A.16c merge), pushed to origin. **17 archetype slugs enrolled / 18 templates editabili end-to-end**. Seventeenth enrollment CLOSES the medical-other family: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + ultra-luxury-cinematic + editorial-designer-grid + cinematic-photographer + trattoria-warm + street-modern + artisan-workshop + fashion-editorial + clinic + wellness + **family** are all multi-locale enrolled. **SEVEN families editor-complete** (law · medical-specialist · real-estate · portfolio · restaurant-continuation · ecommerce · **medical-other**). **Zero half-open families remain**. Two families still fully open (agency-secondary · startup-saas) · 2 templates residue editor-only-pending totale (Aura + Elevate).
...
- **Multi-locale editor on all seventeen** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker · image overrides global/plain-keyed (rendered + storage-only identical contract)
- **Rendered vs storage-only image widget distinction is PER-SKIN** (Bottega typographic 16 rendered + 6 storage-only · Luxe editorial 31 rendered + 0 storage-only · Salute clinical 15 rendered + 0 storage-only · Benessere olistico 19 rendered + 0 storage-only · **Famiglia pediatric 16 rendered + 0 storage-only**): the schema exposes image fields identically · only skin rendering differs · registry data always preserved · editor contract uniform across all · storage plain-key per-locale identical
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- **341/341 server tests passing**, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **17 archetype slugs enrolled · 17 multi-locale enrolled · 18 templates editable end-to-end**
- **Families CLOSED (7)**: law (Lex + Juris sequential dedicated-schema · A.10+A.11) · medical-specialist (Cardio+Derm shared-schema · A.9) · real-estate (Casa+Villa staged dedicated-schema · A.12+A.12b) · portfolio (Chiara+Pixel staged dedicated-schema · A.13+A.13b) · restaurant-continuation (Sapore+Brace staged dedicated-schema · A.14+A.14b) · ecommerce (Bottega+Luxe staged dedicated-schema · A.15+A.15b) · **medical-other (Salute+Benessere+Famiglia staged dedicated-schema 3-phase variant · A.16+A.16b+A.16c)**
- **Zero half-open families remain** · staged dedicated-schema progression 5 closed precedents (including first 3-phase variant) · shared-schema (A.9) + sequential dedicated-schema (A.10+A.11) entrambi single-use unchanged
