...
## 🟢 Current State (2026-04-19 · after Session 74 A.16b Benessere (wellness · medical-other family · second template · MIDDLE PHASE) Editor + Multi-locale Enrollment merge)

Baseline `phase-integration-baseline-v15` tip is **`e9cc419`** (A.16b merge), pushed to origin. **16 archetype slugs enrolled / 17 templates editabili end-to-end**. Sixteenth enrollment continues the medical-other family progression: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + ultra-luxury-cinematic + editorial-designer-grid + cinematic-photographer + trattoria-warm + street-modern + artisan-workshop + fashion-editorial + clinic + **wellness** are all multi-locale enrolled. **Six families editor-complete** (law · medical-specialist · real-estate · portfolio · restaurant-continuation · ecommerce). **Medical-other family STILL HALF-OPEN** (Salute + Benessere IN · Famiglia pending A.16c closer · wellness-out guard removed in A.16b · family-out guard preserved). Two families still fully open (agency-secondary · startup-saas) · 3 templates residue editor-only-pending totale (Famiglia + Aura + Elevate).
...
- **wellness** (Benessere · 1 template · A.16b · **middle phase of medical-other 3-phase staged progression** · removes wellness-out guard · family-out guard PRESERVED for A.16c) · 9 sidebar groups · ~108 scalar + **3 scalar top-level images** (`home.hero_image` + `filosofia.photo_image` + `contatti.map_image`) + 3 image-in-dict-row lists (ambienti.rooms × 8 + home.therapists_trio × 3 + professionisti.people × 5) — **19 total editable image surfaces · ALL RENDERED** (editorial olistico skin · no storage-only split) · **17 readonly indexed list entries** · **tutti parent-level · ZERO deep-path** (13 dict + 4 tuple) · `.we-*` skin AR RTL (13 rules) · **1 novel page kind** (`gallery` · ambienti) + shared `appointment` kind with Salute · **posts list empty** · **no form editing** (5 form structures OUT: 3 nested-dict + 2 list-of-dict) · **4 bool flag cols OUT** (home.calendar + prenota.calendar has_slots + soldout · scheduler-state-like · Luxe/Salute precedent) · **2 nested list-of-str calendar slots OUT** (Juris precedent) · **NOVEL SHAPE DEFERRED** (`home.ambients` tuple-with-image × 4 tiles · whole list OUT first-wave via schema omission · zero precedent · future expansion candidate) · **Stringent IN col-level 6th archetype** (calendar day/num/month + journey.num + pillars.init + packages.tag) · **boundary editor-vs-clinic-admin re-verified runtime**
- **Multi-locale editor on all sixteen** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker · image overrides global/plain-keyed (rendered + storage-only identical contract)
- **Rendered vs storage-only image widget distinction is PER-SKIN** (Bottega typographic 16 rendered + 6 storage-only · Luxe editorial 31 rendered + 0 storage-only · Salute clinical 15 rendered + 0 storage-only · Benessere olistico 19 rendered + 0 storage-only): the schema exposes image fields identically · only skin rendering differs · registry data always preserved · editor contract uniform across all · storage plain-key per-locale identical
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- **325/325 server tests passing**, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **16 archetype slugs enrolled · 16 multi-locale enrolled · 17 templates editable end-to-end**
- **Families CLOSED (6)**: law (Lex + Juris sequential dedicated-schema · A.10+A.11) · medical-specialist (Cardio+Derm shared-schema · A.9) · real-estate (Casa+Villa staged dedicated-schema · A.12+A.12b) · portfolio (Chiara+Pixel staged dedicated-schema · A.13+A.13b) · restaurant-continuation (Sapore+Brace staged dedicated-schema · A.14+A.14b) · ecommerce (Bottega+Luxe staged dedicated-schema · A.15+A.15b)
- **Medical-other family STILL HALF-OPEN** — Salute + Benessere enrolled, Famiglia (`family`) explicitly held out until A.16c closer. **FAMILY-OUT GUARD active at 3 layers** (registration-time + runtime start + runtime end of Salute lifecycle test) · will be removed via `test_a16c_family_out_guard_was_removed_from_salute_tests` · **6th precedente** del guard removal pattern pending (first 3-template progression · sub-recipe 2-removal-phase variant · **wellness-out removed in A.16b via 5th precedent**).
- **Staged dedicated-schema progression è la topologia dominante** con **4 closed precedents** + 1 half-open (medical-other · 3-phase variant). Shared-schema (A.9 · 1 precedente) + sequential dedicated-schema (A.10+A.11 · 1 precedente) restano single-use. **3-phase variant sub-recipe generalizes from 'one-opener-one-closer' to 'one-opener-two-closers'**.
- **Render-side contract-alignment fix `f66ac24` resta cross-pattern confirmed**: 3 editor layers fully aligned su list numeric indexing · Salute + Benessere non toccano questo path (zero deep-path · Famiglia A.16c introdurrà deep-path tuple-in-dict-list `crescita.topics[].items`)
- **Detail-page registry-only policy enforced uniformly on 6 archetypes**: count invariato · Benessere + Salute + Luxe + Bottega + Sapore + Brace shippano `posts: []` strutturalmente
- **Guard removal pattern consolidated · 5 precedents + 1 pending**: Villa-out (A.12b) · Pixel-out (A.13b) · Brace-out (A.14b) · Luxe-out (A.15b) · **Wellness-out (A.16b · `test_a16b_benessere_out_guard_was_removed_from_salute_tests`)** · **Family-out pending A.16c** · sub-recipe 2-removal-phase variant NOW ACTIVE (first time 1 opener plants 2 guards and each closure phase removes 1)
- **Outside-gate fixture rotation alla `family`/`famiglia-pediatria`** · prossimo candidato enrollment è A.16c Famiglia (chiude medical-other 3-phase progression · handles deep-path tuple-in-dict-list `crescita.topics[].items` Sapore-precedent)
- **Image surface patterns con multiple precedents**: typographic-first single-no-image (Bottega · first) · single-scalar-image (Pragma · Pixel · Luxe · Salute) · multiple-scalar-images (Gusto · specialist · Villa · Sapore · Brace · **Benessere 3 scalar**) · nested-dict-scalar-image (Vertex · Chiara · Bottega · Luxe con 2) · image-in-dict-row (Vertex · Villa · Chiara · Sapore · Brace · Bottega · Luxe · Salute · **Benessere con 3 liste**)
- **Stringent IN col-level audit** estesa a **6 archetypes**: Sapore (A.14) → Brace (A.14b) → Bottega (A.15) → Luxe (A.15b) → Salute (A.16) → **Benessere (A.16b · calendar day/num/month + journey.num + pillars.init + packages.tag)** · audit-driven · fields editorial visible stay IN · technical/scheduler-state/bool stay OUT
- **5 OUT categories formalizzate**: flat-list-of-str (Juris) · nested-list-of-str-in-dict-rows (Juris) · form structures (Juris/Gusto) · structural identifiers + commerce-state-like booleans (Bottega/Luxe `available` · Salute `is_popular` · **Benessere `has_slots`/`soldout` 4 paths**) · raw SVG fields (Salute 5th category)
...
- **Boundary editor-vs-{commerce,clinic}-admin clean · runtime-verified su 4 archetypes**: editor edits presentational demo showcase content in template_content registry · {commerce,clinic}-admin scopes managed separately via dedicated dashboards · light smoke tests su Bottega + Luxe + Salute + **Benessere**
- **Families still open for editor enrollment (3 families · 3 templates total · 1 family half-open)**: **medical-other half-open** (Famiglia · 1 template · A.16c closer) · **agency-secondary** (Aura · 1 template) · **startup-saas** (Elevate · 1 template)

No explicitly-deferred debt is pending. **17 commit-clean phases** delivered back-to-back (A.6 → A.16b). A.16b was 3 commits on branch (schema · lifecycle · merge) · zero additional infra fixes required (mechanical reuse di tutti i pattern pre-esistenti · 6th consecutive staged enrollment).

### Next workstream — immediate candidates (medical-other STILL HALF-OPEN · Famiglia chiude family A.16c)

A.16b continues the medical-other progression as middle phase. Still half-open · 1 sibling pending. Top candidate is **A.16c Famiglia** · closer del 3-phase staged dedicated-schema progression.

- [ ] **A.16c · Famiglia (`family`) enrollment — closes the medical-other family** — natural continuation from A.16b · single-template enrollment removes family-out guard · **closes the family** and brings medical-other to fully-enrolled state. Step-0 audit needed for: **deep-path tuple-in-dict-list `crescita.topics[].items`** (Sapore-precedent shape · 4 × list-of-tuple `(question, answer)` · requires dedicated STRUCTURED_FIELD_SHAPES entries per topic · mechanical reuse of `f66ac24` contract-alignment fix · Brace precedent for dict-in-dict-list handles list parent walk) · novel `faq` page kind (plain string identifier · no dispatch) · NO form_fields list-of-dict (form exposed as flat scalars · simpler OUT policy than Benessere) · 16 image surfaces (3 scalar + 13 image-in-dict-row across 3 lists) · `home.gallery[].src` + `.cap` shape (image col `src` + scalar caption). Expected schema LOC ~600-700 (higher than Benessere due to deep-path shape entries · possible exception over 600-LOC soft guardrail acceptable · 4 crescita.topics.{i}.items entries + 10 base lists · Sapore precedent was +663 LOC for similar shape).
- [ ] **A.16c alt · Aura (`agency-digital-studio`) individual enrollment** — low novelty, low risk, single template. Closes agency-secondary category. Pulls focus away from medical-other half-open · only justifiable if customer signal surfaces.
- [ ] **A.16c alt · Elevate (`startup-saas-landing`) individual enrollment** — low novelty, single template. Closes startup-saas. Same trade-off as Aura.
- [ ] **A.16c alt · MEMORY.md maintenance mini-phase** — auto-memory index ~34-35KB sopra warning-soglia 24.4KB (grows ~1KB per enrollment phase). Index entries can be shortened without losing content. **Separate housekeeping task, NOT bundled with an enrollment**. Low-value-low-risk.
- [ ] **A.16c alt · `home.ambients` tuple-with-image widening mini-phase** — deferred novel shape from A.16b. Would require: runtime verification of tuple-cell-with-image-type handling (`services._resolve_path` + `schema._iter_indexed_groups` + `rendering._apply_indexed`) + widget-level render test. Low value (4 tiles only) · defer unless customer signal.
- [ ] **A.16c alt · Selective editor polish** — defer unless customer signal surfaces.
- [ ] **A.16c alt · Media evolution / remote storage (S3/Cloudinary)** — defer. Current infra scales through 16 archetypes with 19 image surfaces · no technical trigger.
- [ ] **A.16c alt · Editor operator tools** — defer.
- [ ] **A.16c alt · Repeater-per-locale first wave** — defer. Out-of-scope family per D-098.

Recommended framing: **A.16c Famiglia Step-0 planning session** — closes the family-out guard (6th precedent of guard removal pattern) and the entire medical-other family · brings catalog to **17 archetype slugs / 18 templates editable / 7 families closed**. Deep-path tuple-in-dict-list `crescita.topics[].items` is the main Step 0 audit focus (mechanical reuse of f66ac24 · no new infra needed). Outside-gate fixture rotation target TBD (residual candidates: `agency-digital-studio` Aura or `startup-saas-landing` Elevate).

**What NOT to open immediately (red-lamps)**:
- **Do NOT bundle Famiglia + Aura + Elevate into one A.16c** · discipline violation (one phase = one archetype decision · 17 phases strong A.6→A.16b)
- **Do NOT skip Famiglia to enroll Aura/Elevate first** · half-open family priority unchanged · medical-other should close before opening new fronts
- **Do NOT re-open Salute or Benessere coverage expansion** · both closed · any additional coverage would be scope-creep
...
- **Do NOT widen `home.ambients` in A.16c** · deferred novel shape stays deferred · widening is a separate mini-phase or indefinite (customer-signal-gated)
...
- Any bundled multi-archetype phase (17 phases A.6→A.16b have all respected discipline · preserve)
