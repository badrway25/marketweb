...
- **agency-digital-studio** (Aura · 1 template · A.17 · **SINGLE-TEMPLATE CLOSER** of agency-secondary family · first single-template dedicated-schema variant of D-098 topology distribution) · 9 sidebar groups · ~95 scalar + zero scalar top-level image + 3 image-in-dict-row lists · 12 image cells · `.au-*` skin AR RTL (25 rules) · 6 pages · posts list registry-only (7th uniform enforcement) · brief.slots OUT entire (5th form-structure precedent)
- **startup-saas-landing** (Elevate · 1 template · A.17b · **SINGLE-TEMPLATE CLOSER** of startup-saas family · **CLOSES THE EDITOR ENROLLMENT PROGRAM A.6 → A.17b** · 2nd single-template dedicated-schema precedent) · 9 sidebar groups · ~100 scalar + **1 single image surface total** (`home.product_demo_card.poster` nested-dict scalar · SIMPLEST image profile of any enrolled archetype · vs Aura 12 · Famiglia 16 · Luxe 31) · 16 readonly indexed list entries (6 tuple + 10 dict · including `contatti.office.schedule` nested under `contatti.office` parent dict) · **zero deep-path · zero image-in-dict-row · zero mutable repeater · zero image per-locale** · `.sl-*` skin AR RTL (**52 rules · HIGHEST of all 19 enrolled · most dense pricing-table/mockup-dashboard/ship-log RTL layout**) · **5 pages** (home · prodotto · prezzi · demo · contatti) · **posts list ABSENT by design** (not registry-only · truly not in content registry) · **6th form-structure OUT precedent** (`prezzi.billing_toggle_options` tuple `(id, label)` billing-state form-option-value) · **7th form-structure OUT precedent** (`demo.form_fields` + `demo.form_sections` + `demo.upload_field` lead-form scaffolding) · **editor-vs-product-admin boundary · 3rd category-class precedent** (after editor-vs-commerce-admin + editor-vs-clinic-admin · 9 namespace prefixes isolated: stripe/subscription/billing/payment/usage/telemetry/feature_flag/admin/dashboard/session) · **Stringent IN col-level 9th archetype** (banner_label + banner_text + shiplog.version + shiplog_release_value/chip + mockup.chrome_label + mockup.feature_label + modules.num + tiers.annual_period + highlight_badge + annual_prefix) · Zero raw SVG · zero bool flags IN · zero scheduler-state · zero novel shape · **Playwright 9-focus walk validated** (sidebar 26 groups = 9 schema + 16 indexed + 1 design · 1 image widget · 8/8 IN paths present · 0/18 OUT paths present · 5-locale flush clean · AR zero overflow · outside-gate UI coherent · catalog-complete 19/19)
- **Multi-locale editor on all nineteen** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker · image overrides global/plain-keyed (rendered + storage-only identical contract)
- **Rendered vs storage-only image widget distinction is PER-SKIN** (Bottega typographic 16 rendered + 6 storage-only · Luxe editorial 31 rendered + 0 storage-only · Salute clinical 15 rendered + 0 storage-only · Benessere olistico 19 rendered + 0 storage-only · Famiglia pediatric 16 rendered + 0 storage-only · Aura growth-tech 12 rendered + 0 storage-only · **Elevate product-landing 1 rendered + 0 storage-only**): the schema exposes image fields identically · only skin rendering differs · registry data always preserved · editor contract uniform across all · storage plain-key per-locale identical
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- **375/375 server tests passing**, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **19 archetype slugs enrolled · 19 multi-locale enrolled · 20 templates editable end-to-end · 20/20 catalog editable**
- **Families CLOSED (9)**: law (Lex + Juris sequential dedicated-schema · A.10+A.11) · medical-specialist (Cardio+Derm shared-schema · A.9) · real-estate (Casa+Villa staged dedicated-schema · A.12+A.12b) · portfolio (Chiara+Pixel staged dedicated-schema · A.13+A.13b) · restaurant-continuation (Sapore+Brace staged dedicated-schema · A.14+A.14b) · ecommerce (Bottega+Luxe staged dedicated-schema · A.15+A.15b) · medical-other (Salute+Benessere+Famiglia staged dedicated-schema 3-phase variant · A.16+A.16b+A.16c) · agency-secondary (Aura single-template dedicated-schema · A.17) · **startup-saas (Elevate single-template dedicated-schema · A.17b)**
- **Zero half-open families remain · zero non-enrolled real archetypes · editor enrollment program CLOSED** · D-098 topology distribution FINAL: 1 shared-schema (A.9) + 1 sequential dedicated-schema (A.10+A.11) + 5 staged dedicated-schema (including first 3-phase variant) + **2 single-template dedicated-schema (A.17 + A.17b)** · Total 9 closed families
- **Outside-gate retirement (D-099)**: old pattern ("fixture = real non-enrolled archetype") RETIRED · replaced by synthetic sentinel `__unsupported_archetype_sentinel__` · used ONLY for helper-level negative assertions · never exposed as UI template slug · `test_customize_start_unsupported_archetype_redirects_to_detail` deleted as post-20/20 redundant · `test_customize_start_unknown_template_redirects_to_catalog` still covers unknown-slug path
...
- **Detail-page registry-only policy enforced uniformly on 7 archetypes**: Famiglia + Benessere + Salute + Luxe + Bottega + Sapore + Brace + Aura shippano `posts: []` strutturalmente · **Elevate has posts ABSENT by design** (8th absence profile · not registry-only · truly not in registry)
- **Guard removal pattern consolidated · 7 precedents**: Villa-out (A.12b) · Pixel-out (A.13b) · Brace-out (A.14b) · Luxe-out (A.15b) · Wellness-out (A.16b) · Family-out (A.16c) · Aura-out (A.17) · **A.17b replaces the pattern altogether with sentinel retirement (D-099)**
- **Image surface patterns con multiple precedents**: typographic-first single-no-image (Bottega) · single-scalar-image (Pragma · Pixel · Luxe · Salute) · multiple-scalar-images (Gusto · specialist · Villa · Sapore · Brace · Benessere · Famiglia) · nested-dict-scalar-image (Vertex · Chiara · Bottega · Luxe con 2 · **Elevate con 1 · SIMPLEST profile of all 19 enrolled**) · image-in-dict-row (Vertex · Villa · Chiara · Sapore · Brace · Bottega · Luxe · Salute · Benessere · Famiglia · Aura)
- **Stringent IN col-level audit** estesa a **9 archetypes**: Sapore (A.14) → Brace (A.14b) → Bottega (A.15) → Luxe (A.15b) → Salute (A.16) → Benessere (A.16b) → Famiglia (A.16c) → Aura (A.17) → **Elevate (A.17b · banner_label/text + shiplog.version + shiplog_release_value/chip + mockup.chrome_label/feature_label + modules.num + tiers.annual_period + highlight_badge + annual_prefix)** · audit-driven · fields editorial visible stay IN · technical/billing-state/bool stay OUT
- **7 OUT categories formalizzate**: flat-list-of-str (Juris) · nested-list-of-str-in-dict-rows (Juris) · form structures (Juris/Gusto/Aura · nested-dict OR list-of-dict) · structural identifiers + commerce-state-like booleans (Bottega/Luxe `available` · Salute `is_popular` · Benessere `has_slots`/`soldout`) · raw SVG fields (Salute 5th category) · **billing-state form-option-value tuples (Elevate prezzi.billing_toggle_options 6th category)** · **lead-form scaffolding (Elevate demo.form_fields/form_sections/upload_field 7th category)**
- **4 deferral categories formalized**: mutable repeater (D-098) · image per-locale (D-098) · detail-page editing (7-archetype uniform enforcement · Elevate has posts ABSENT so not applicable) · novel-shape-deferred (A.16b `home.ambients` tuple-with-image · first-ever)
- **Boundary editor-vs-admin clean · 3 category-class precedents**: editor-vs-commerce-admin (Bottega/Luxe A.15/A.15b · 2 archetypes light-smoke-tested) · editor-vs-clinic-admin (Salute/Benessere/Famiglia A.16/A.16b/A.16c · 3 archetypes light-smoke-tested) · **editor-vs-product-admin (Elevate A.17b · 9-namespace prefix isolation test)**
- **Deep-path Sapore precedent unchanged · 3 archetypes precedent chain** (Sapore · Brace · Famiglia) · f66ac24 render-side contract-alignment fix covers tuple-in-dict-list + dict-in-dict-list parent walks uniformly

No explicitly-deferred debt is pending. **19 commit-clean phases** delivered back-to-back (A.6 → A.17b). A.17b was 2 commits on branch (schema+tests · lifecycle) · zero additional infra fixes required (mechanical reuse di tutti i pattern pre-esistenti · 9th consecutive single-archetype enrollment · 2nd single-template dedicated-schema closure · first retirement of a pattern via synthetic sentinel D-099).

### Next workstream — editor enrollment program CLOSED · only maintenance/polish candidates remain
...
- [ ] **Media evolution / remote storage (S3/Cloudinary)** — defer. Current infra scales through 19 archetypes · no technical trigger.
- [ ] **Editor operator tools** — defer.
- [ ] **Repeater-per-locale first wave** — defer. Out-of-scope family per D-098.

**Recommended framing: MEMORY.md maintenance mini-phase** as the natural post-program housekeeping beat. No enrollment · no schema changes · just trim the auto-memory index entries that accumulated across 19 phases.

**What NOT to open immediately (red-lamps)**:
- **Do NOT open any `A.18 template enrollment`** · the catalog is 20/20 editable · 19/19 archetypes enrolled · zero non-enrolled real archetypes · the enrollment program is CLOSED
- **Do NOT add a fake WebTemplate just to re-create the outside-gate pattern** · violates D-099 binding · sentinel replaces it
- **Do NOT re-open closed families for coverage expansion** · any additional coverage would be scope-creep · the per-family recipe is settled
- **Do NOT bundle any future single-archetype edit with a "refactor while we're here"** · discipline from 19 clean phases stays
- **Do NOT open product-admin / billing / subscription / Stripe customer-editing surfaces** · boundary editor-vs-product-admin locked (D-099/Elevate · 9 namespace prefixes isolated)
- **Do NOT widen `home.ambients`** without explicit customer signal · deferred from A.16b
- **Do NOT open detail-page editing as side quest** · horizontal feature affecting 7 archetypes (+ Elevate without posts · 8 archetypes affected)
- **Do NOT add bool / tuple-with-image field types** · horizontal features · all precedent bool OUT decisions preserved
- **Do NOT bundle MEMORY.md maintenance with an enrollment** (moot: no enrollment remains · but the guardrail stays as documentation)
- Remote asset storage (no technical pressure · current infra dimostrato scalare su 19 archetypes)
