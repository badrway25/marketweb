...
## 🟢 Current State (2026-04-18 · after Session 66 A.12b Villa (ultra-luxury-cinematic · real-estate family) Editor + Multi-locale Enrollment merge)

Baseline `phase-integration-baseline-v15` tip is **`258fa56`** (A.12b merge), pushed to origin. **8 archetype slugs enrolled / 9 templates editabili end-to-end**. Eighth enrollment closes the real-estate family: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + **ultra-luxury-cinematic** are all multi-locale enrolled. Three families editor-complete: law (A.10+A.11 sequential dedicated-schema), medical-specialist (A.9 shared-schema), real-estate (A.12+A.12b staged dedicated-schema). All three D-098 family-progression topologies now have ≥1 real precedent.
...
- **modern-transparent** (Juris · 1 template · A.11 · closes law family via second dedicated-schema) · ~180 scalar + **zero image fields** + 6 readonly indexed lists · 110 translatable · `.jr-*` skin AR RTL · complex shapes outside perimeter (dashboard_mock + trust_logos + nested bullets)
...
- **ultra-luxury-cinematic** (Villa · 1 template · A.12b · closes real-estate family via second dedicated-schema) · ~175 scalar + **4 scalar image** + 14 readonly indexed lists, **3 con image cols** (home.signature.image · territorio.territories.image · studio.advisors.portrait — **18 editable image surfaces totali**: 4 scalar + 14 image cells) · 128 translatable · 247 global · `.vp-*` skin AR RTL · second archetype to use image-in-dict-row pattern after Vertex (same widget infra, non-mutable lists · zero service-layer changes required) · complex shapes outside perimeter (nested list-of-str inside dict rows `filter_groups[].options` / `form_fields[].options` · flat list-of-str `home.territory` / `home.press_items` / `collezione.sort_options` / footer rows · `concierge.form_fields` form structure · per-post `posts` entries incl. posts.image)
- **Multi-locale editor on all eight** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker on translatable fields · **image overrides stay global/plain-keyed even inside dict rows** (verified end-to-end in Villa lifecycle test)
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- 216/216 server tests passing, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **8 archetype slugs enrolled · 8 multi-locale enrolled · 9 templates editable end-to-end**
- **Law family CLOSED** (Lex + Juris, sequential dedicated-schema · A.10+A.11) · **medical-specialist family CLOSED** (Cardio+Derm, shared-schema · A.9) · **real-estate family CLOSED** (Casa+Villa, staged dedicated-schema · A.12+A.12b)
- **Villa-out guard removed correctly** in A.12b (2 assertions dropped from A.12 Casa tests: registration-time + runtime · removal verified by `test_a12b_villa_out_guard_was_removed_from_casa_tests`)
- Families still open for editor enrollment (11 templates total): **portfolio** (Chiara + Pixel · 2 templates) · **restaurant-continuation** (Sapore + Brace beyond Gusto · 2 templates) · **medical-other** (Salute + Benessere + Famiglia · 3 templates) · **ecommerce** (Bottega + Luxe · 2 templates) · **agency-secondary** (Aura beyond Vertex · 1 template) · **startup-saas** (Elevate · 1 template)

No explicitly-deferred debt is pending.

### Next workstream — immediate candidates (A.13 planning session recommended next)

- [ ] **A.13 portfolio family (Chiara + Pixel) enrollment** — natural next closure candidate. Chiara ships `editorial-designer-grid` with novel `project_detail` / `series_detail` page kinds (Session 34 rollout) that stretch the editor beyond the home/about/services patterns every prior archetype uses; Pixel ships `cinematic-photographer` with imagery-heavy single-template shape. **Step-0 audit must decide**: (a) shared-schema vs staged dedicated-schema (likely the latter — distinct DNA), (b) whether `project_detail` / `series_detail` page-scoped fields enter the perimeter (probably out — detail editing stays registry-only consistent with all prior blog-list/post-detail policies applied to Lex notabili, Juris insights, Casa posts, Villa posts). If audit confirms staged dedicated-schema like real-estate, A.13 = Chiara first → A.13b = Pixel. ~3 commits per archetype + consolidation.
- [ ] **A.13 alt · restaurant-continuation (Sapore + Brace beyond Gusto)** — two templates, likely distinct archetypes (`trattoria-warm` + `street-modern` per DNA registry). Step-0 audit decides topology; most probable outcome is staged dedicated-schema closing the restaurant category.
- [ ] **A.13 alt · ecommerce family (Bottega + Luxe)** — `artisan-workshop` vs `fashion-editorial`. Session 41/42 observations already suggest wide divergence. Would close the ecommerce category with staged enrollments. Bottega has commerce-foundation overlap (Phase 3a+3b already landed `apps/commerce/` seed for both) — worth a Step-0 check on whether the editor schema needs to avoid overlap with commerce-managed content.
- [ ] **A.13 alt · medical-other family (Salute + Benessere + Famiglia)** — three single-template medical archetypes (`medical-clinic` / `wellness-serene` / `family-warm`) that went `published_live` in Session 51 but never entered the editor. **Three separate phases preferred over bundling** — the "one-phase-one-archetype" discipline is now 8 phases strong (A.6 through A.12b) and should not be relaxed for three simultaneous openings.
- [ ] **A.13 alt · Aura (agency-digital-studio) individual enrollment** — low novelty, low risk, single template. Aura is the second agency template (beyond Vertex) and would bring the agency category to full enrollment.
- [ ] **A.13 alt · Elevate (startup-saas-landing) individual enrollment** — low novelty, single template. Closes the startup-saas category.
- [ ] **A.13 alt · Selective editor polish** — search Cmd-K locale-aware ranking · "solo translatable" sidebar filter · badge-per-group pre-sync at mount · palette page-slug-match boost · sticky-last-page on reopen. Low-risk, low-value — only worth opening if customer signal surfaces.
- [ ] **A.13 alt · Media evolution / remote storage** — S3/Cloudinary swap of `ProjectAsset.file` backend. Must keep `/media/` accept path in parallel (D-095 binding). **Do NOT open unless a prod-launch timeline demands it**. A.12b validated that the current image infrastructure scales to image-heavy archetypes without pressure — no technical trigger for remote storage right now.
- [ ] **A.13 alt · Editor operator tools** — admin-facing (zero customer surface): admin project list filters, GC scheduler, asset audit UI. Better after another scale-out consolidates the editor footprint.
- [ ] **A.13 alt · Repeater-per-locale first wave** — flip one or two mutable Vertex lists from global to translatable-per-locale. Out-of-scope family per D-098 · opens localized repeater content only if customer signal emerges.
- [ ] **A.13 alt · MEMORY.md maintenance mini-phase** — auto-memory index is at ~27KB, above the 24.4KB warning threshold since A.11. Index entries can be shortened without losing content. Not a blocker; open as a separate maintenance phase when convenient, NOT bundled with an enrollment.

Recommended framing: **A.13 portfolio family Step-0 planning session** — closes another of the 8 MVP categories. Portfolio has the highest "closes a family" value among the candidates; if Step-0 reveals project_detail complexity is too broad, pivot to ecommerce or restaurant-continuation as fallback. Decision deferred to the dedicated A.13 planning session.

**What NOT to open immediately (value is low right now)**:
- Remote asset storage (A.12b demonstrated image-heavy enrollment works on current infra · no technical pressure)
...
- Any bundled multi-archetype phase (violates "one phase = one archetype decision" discipline · 8 phases strong A.6→A.12b)
- Coverage expansion on already-enrolled archetypes — would bloat the phase without customer pull
- Detail-page editing (project_detail / series_detail / blog_detail) — every prior family closure has kept per-item detail registry-only; open only with clear customer signal + dedicated planning
