...
## 🟢 Current State (2026-04-18 · after Session 65 A.12 Casa (mass-market · real-estate family) Editor + Multi-locale Enrollment merge)

Baseline `phase-integration-baseline-v15` tip is **`a93c50d`** (A.12 merge), pushed to origin. Seven editor-supported archetypes — Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market — are all multi-locale enrolled. Eight templates are now editable end-to-end from 7 schemas. **The law family is CLOSED COMPLETELY** (Lex + Juris via two dedicated schemas). **The real-estate family is OPEN but NOT YET CLOSED** — Casa landed in A.12, Villa (`ultra-luxury-cinematic`) stays out until the dedicated A.12b phase.
...
- **classic-gold** (Lex · 1 template · A.10 · first dedicated-schema slot of the law family) · ~102 scalar + 1 image (`notabili.lead_image`) + 6 readonly indexed lists · 79 translatable fields · `.lx-*` skin AR RTL authentic
- **modern-transparent** (Juris · 1 template · A.11 · closes law family via second dedicated-schema enrollment) · ~180 scalar + **zero image fields** + 6 readonly indexed lists (approccio.founders dict 2×3 with `credentials` list-of-str registry-only · approccio.story tuple 5×3 · approccio.manifesto tuple 4×3 · servizi.services dict 7×7 with `deliverables` list-of-str registry-only · settori.sectors dict 6×6 with `pain_points` / `signals` / `legal_ops`-bullets nested-list-of-str registry-only · settori.team dict 10×5) · 110 translatable fields · 188 global fields · `.jr-*` skin AR RTL authentic · complex shapes explicitly outside perimeter (`approccio.dashboard_mock` nested-dict + `home.trust_logos` / `insights.topics` flat lists-of-str)
- **mass-market** (Casa · 1 template · A.12 · first dedicated-schema slot of the real-estate family) · ~185 scalar + **zero image fields** (second zero-image archetype after Juris) + 15 readonly indexed lists (5 on home: featured_listings tuple 4×4 cols of 8 · neighborhoods 6×4 · stats 4×3 · agents_preview 4×4 · valuation_proof 3×2 · 1 on immobili: map_cells 5×2 · 2 on quartieri: guides tuple 8×6 of 9 · faq 4×2 · 2 on agenzia: agents dict 9×9 of 10 · facts 4×3 · 3 on valutazione: how_it_works 3×3 · proof 4×2 · faq 4×2 · 2 on contatti: channels 5×3 · offices dict 2×9 of 13) · 134 translatable fields · 313 global fields · `search_widget` exposed as 14 flat scalars in `hero_home` (with `popular_tags` flat list-of-str registry-only) · `.dm-*` skin AR RTL authentic · complex shapes outside perimeter (flat list-of-str `popular_tags` / `immobili.filters` / `immobili.sort_options` + form structure blocks + per-property `posts` entries)
- **Multi-locale editor on all seven** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker on translatable fields
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- 204/204 server tests passing, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **7/8 archetypes editor-supported · 7/8 multi-locale enrolled · 8 templates editable end-to-end**
- **Law family CLOSED** (Lex classic-gold + Juris modern-transparent, dedicated-schema) · **medical-specialist family CLOSED** (Cardio+Derm shared-schema) · **real-estate family OPEN but not yet closed** (Casa mass-market in · Villa ultra-luxury-cinematic out until A.12b)
- Villa (`ultra-luxury-cinematic`) gate state: NOT in `_ARCHETYPE_SCHEMAS`, NOT in `_MULTILOCALE_ENABLED_ARCHETYPES`. Guarded by `test_a12_casa_archetype_registered` (registration-time) + runtime `assertNotIn` inside the lifecycle test (execution-time). Both guards will be removed in A.12b together with Villa's own registrations.
- Families still open for editor enrollment: **real-estate (Villa · 1 template · closes family)** · portfolio (Chiara + Pixel · 2 templates) · restaurant-continuation (Sapore + Brace beyond Gusto · 2 templates) · medical-other archetypes (Salute + Benessere + Famiglia · 3 templates) · ecommerce (Bottega + Luxe · 2 templates) · agency-secondary (Aura beyond Vertex · 1 template) · startup-saas (Elevate · 1 template)

No explicitly-deferred debt is pending.

### Next workstream — immediate candidates (A.12b planning session recommended next)

- [ ] **A.12b Villa (ultra-luxury-cinematic) enrollment — close the real-estate family** — natural continuation from A.12, mirrors the A.10→A.11 law-family closure. Villa has distinct DNA + distinct skin folder (`.vp-*`, 34 mature `html[dir="rtl"]` rules) + divergent page slugs from Casa, so a separate schema + separate `_base.html` preview bridge + dedicated lifecycle test are required. **Critical novelty from Casa**: Villa carries **26 image fields** (1 cover + 4 signature[].image + 1 advisor_portrait + 6 territory[].image + 1 director_portrait + 4 advisor[].portrait + 8 posts[].image + 1 collezione.lead_image). The **image-inside-list-of-dict-row** pattern (signature[i].image · territories[i].image · advisors[i].portrait) is NOVEL — no currently-enrolled archetype exposes image cols inside dict rows. Gusto `produttori.items` and specialist `medici.doctors` apply col-EXCLUSION for portraits; Villa would need col-EXPOSURE for cover-in-row. Step 0 decision: expose image cols inside the dict shape (new widget pattern, may need small glue in rendering/service) vs keep them registry-only (deprives customer of core editing surface for a luxury-editorial template where the imagery IS the content). Likely ~3-4 commits (schema + bridge + lifecycle + possibly a small service-layer touch).
- [ ] **A.12b alt · Chiara (editorial-designer-grid)** — single template, novel `project_detail` / `series_detail` page kinds (Session 34 rollout) stretch the editor beyond current patterns. Good second choice if Villa's image-in-row analysis reveals higher risk than expected.
- [ ] **A.12b alt · Pixel (cinematic-photographer)** — single template, imagery-heavy but simpler shape than Villa (no list-of-dict image novelty). Straightforward A.11 recipe.
- [ ] **A.12b alt · Sapore + Brace (restaurant-family continuation beyond Gusto)** — two templates, likely distinct archetypes (`trattoria-warm` + `street-modern` per DNA registry). Would close the restaurant family if audit confirms divergence → two dedicated-schema enrollments staged like A.12 + A.12b.
- [ ] **A.12b alt · Bottega + Luxe (ecommerce family)** — similar audit gate. `artisan-workshop` vs `fashion-editorial` — Session 41/42 observations already suggest wide divergence. Would close the ecommerce family via staged enrollments.
- [ ] **A.12b alt · Salute + Benessere + Famiglia (medical other archetypes beyond specialist)** — three single-template medical archetypes (clinic / wellness-serene / family-warm) that went `published_live` in Session 51 but never entered the editor. Three separate phases (discipline kept), not a bundle.
- [ ] **A.12b alt · Aura (agency-digital-studio) + Elevate (startup-saas-landing)** — two single-template single-archetype enrollments. Low novelty, low risk.
- [ ] **A.12b alt · Selective editor polish** — search Cmd-K locale-aware ranking · "solo translatable" sidebar filter · badge-per-group pre-sync at mount · palette page-slug-match boost · sticky-last-page on reopen. Low-risk, low-value — only worth opening if customer signal surfaces.
- [ ] **A.12b alt · Media evolution / remote storage** — S3/Cloudinary swap of `ProjectAsset.file` backend. Must keep `/media/` accept path in parallel (D-095 binding). **Do NOT open unless a prod-launch timeline demands it**. Becomes relevant if A.12b Villa opens a wider image-handling surface.
- [ ] **A.12b alt · Editor operator tools** — admin-facing (zero customer surface): admin project list filters, GC scheduler, asset audit UI. Better after another scale-out consolidates the editor footprint.
- [ ] **A.12b alt · Repeater-per-locale first wave** — flip one or two mutable Vertex lists from global to translatable-per-locale. Out-of-scope family per D-098 · opens localized repeater content only if customer signal emerges.

Recommended framing: **A.12b Villa (ultra-luxury-cinematic) enrollment** — closes the real-estate family. Stronger signal than picking a new family: a half-open family is a clearer ROI than opening a seventh. The image-inside-dict-row decision is the only non-obvious design point; Step 0 planning resolves it before any code. Decision deferred to the dedicated A.12b planning session.

**What NOT to open immediately (value is low right now)**:
- Remote asset storage (wait for A.12b's image-handling outcome to decide if it pressures the ProjectAsset backend)
...
- Any bundled multi-archetype phase (violates "one phase = one archetype decision" discipline that has kept the last 7 phases clean)
- Coverage expansion on already-enrolled archetypes (Vertex / Pragma / Gusto / specialist / Lex / Juris / Casa) — would bloat the phase without customer pull
- Mixing Villa with another archetype in A.12b (image-in-row is already novel enough — pairing it with anything else risks scope creep on the novel shape decision)
