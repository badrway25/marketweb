...
## 🟢 Current State (2026-04-20 · after Session 76 A.17 Aura (agency-digital-studio · agency-secondary family · SINGLE-TEMPLATE CLOSER) Editor + Multi-locale Enrollment merge · CLOSES AGENCY-SECONDARY FAMILY)

Baseline `phase-integration-baseline-v15` tip is **`2b0d11e`** (A.17 merge), pushed to origin. **18 archetype slugs enrolled / 19 templates editabili end-to-end**. Eighteenth enrollment CLOSES the agency-secondary family: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + ultra-luxury-cinematic + editorial-designer-grid + cinematic-photographer + trattoria-warm + street-modern + artisan-workshop + fashion-editorial + clinic + wellness + family + **agency-digital-studio** are all multi-locale enrolled. **EIGHT families editor-complete** (law · medical-specialist · real-estate · portfolio · restaurant-continuation · ecommerce · medical-other · **agency-secondary** ← new · first single-template family closure with one phase). **Zero half-open families remain**. One family still fully open (startup-saas · single-template) · 1 template residue editor-only-pending (Elevate · A.17b will close the catalog to 20/20 editable).
...
- **street-modern** (Brace · 1 template · A.14b · **closes the restaurant-continuation family** via second dedicated-schema · staged progression with Sapore) · 8 sidebar groups · ~170 scalar + **3 scalar image fields** + **6 image-in-dict-row lists** including menu.sections.{0..4}.items deep-path · **44 total editable image surfaces** (3 scalar + 41 cells · 3.4× Sapore) · **30 readonly indexed list entries** · 148 translatable · `.sm-*` skin AR RTL (24 rules) · **first `order` page kind enrolled** · **no form structures** · **posts list empty**
- **artisan-workshop** (Bottega · 1 template · A.15 · first dedicated-schema slot of the ecommerce family · **SHOWCASE/template-content editing NOT commerce-admin editing**) · 8 sidebar groups · ~141 scalar + **0 scalar top-level image** (typographic-first DNA per Session 42 D-073 · `.aw-*` crest-mark glyphs) + 2 nested-dict scalar images (`product.artisan_portrait` · `atelier.founder_portrait`) + 4 image-in-dict-row lists (home.latest_items × 4 · home.makers × 4 · shop.products × 9 · product.related_items × 3) — **22 total editable image surfaces** · **14 readonly indexed list entries** · **tutti parent-level · ZERO deep-path** · 125 translatable · `.aw-*` skin AR RTL (31 rules) · **3 novel page kinds** (shop · product · journal) · **posts list empty** · **no form editing** · `contatti.form_fields` OUT · **boundary editor-vs-commerce-admin verified runtime**
- **fashion-editorial** (Luxe · 1 template · A.15b · **closes the ecommerce family** via second dedicated-schema · staged progression with Bottega) · 8 sidebar groups · ~164 scalar + **1 scalar top-level image** (`home.cover_image` · editorial cover LEFT on charcoal · photographically editorial campaign-driven DNA) + 2 nested-dict scalar images (`product.atelier_portrait` · `maison.direction_portrait`) + 6 image-in-dict-row lists (home.tiles × 4 · home.lookbook_teaser_tiles × 3 · collezione.products × 9 · product.related_items × 3 · maison.ateliers × 3 · lookbook.looks × 6) — **31 total editable image surfaces · ALL RENDERED** (no storage-only split, unlike Bottega's typographic skin) · **17 readonly indexed list entries** · **tutti parent-level · ZERO deep-path** · ~130 translatable · `.fe-*` skin AR RTL (21 rules) · **2 novel page kinds** (collezione `collection` + lookbook) · **posts list empty** · **no form editing** · `contatti.form_fields` OUT · **boundary editor-vs-commerce-admin preserved runtime**
- **clinic** (Salute · 1 template · A.16 · **opens the medical-other family** via first dedicated-schema · 3-phase staged progression with Benessere A.16b + Famiglia A.16c · **first 3-template staged progression**) · 9 sidebar groups · ~95 scalar + 1 scalar top-level image + 2 image-in-dict-row lists — **15 total editable image surfaces · ALL RENDERED** · 16 readonly indexed list entries · tutti parent-level · ZERO deep-path · `.cl-*` skin AR RTL (18 rules) · 2 novel page kinds (prevention + appointment) · NOVEL 5th OUT category precedent (raw icon_svg × 18) · bool flag `is_popular` OUT · family-out guard preserved for A.16c (wellness-out removed in A.16b)
- **wellness** (Benessere · 1 template · A.16b · middle phase of medical-other 3-phase staged progression) · 9 sidebar groups · ~108 scalar + 3 scalar top + 3 image-in-dict-row lists = 19 image surfaces all rendered · 17 indexed entries · novel `gallery` page kind · 4 bool flag cols OUT + 2 calendar slots OUT + 5 form structures OUT · home.ambients tuple-with-image DEFERRED
- **family** (Famiglia · 1 template · A.16c · **CLOSER phase of medical-other 3-phase staged progression** · **CLOSES the medical-other family**) · 8 sidebar groups · ~102 scalar + **3 scalar top-level images** (`home.hero_image` + `studio.studio_image` + `contatti.map_image`) + 3 image-in-dict-row lists (home.doctors × 4 + home.gallery × 5 + pediatre.doctors × 4) — **16 total editable image surfaces · ALL RENDERED** (pediatric skin · no storage-only split) · **20 readonly indexed list entries** (12 dict + 4 tuple + **4 DEEP-PATH `crescita.topics.{0..3}.items`** · Sapore `menu.sections.{i}.dishes` precedent mirror · f66ac24 render-side fix mechanical reuse · zero new infra) · `.fm-*` skin AR RTL (**6 rules · lowest of enrolled · sufficient for base layout**) · **1 novel page kind** (`faq` · crescita) · **NO `appointment` kind** (phone-and-WhatsApp CTA pattern) · **posts list empty** · **NO `form_fields` list-of-dict** (form as flat scalars · simpler OUT than Benessere · 2 nested-dict form structures OUT) · **Zero raw SVG** (icons are Bootstrap text refs) · **Zero bool flags** (no scheduler-state) · **Novel col name `src`** on home.gallery (mechanical reuse) · **Stringent IN col-level 7th archetype** (meta + icon text-refs + tag + exp_label/exp_value + wa_label + age) · **Deep-path edit surgical isolation verified** (target cell only · sibling `.a`/`.items.1.q`/`topics.1.items.0.q`/`topics.3.items.3.q` + parent `meta`/`title` all UNCHANGED · both draft and published states) · **boundary editor-vs-clinic-admin re-verified runtime** (3rd medical-other archetype)
- **agency-digital-studio** (Aura · 1 template · A.17 · **SINGLE-TEMPLATE CLOSER** of agency-secondary family · **CLOSES the agency-secondary family** · first single-template family closure with one phase · first single-template dedicated-schema variant added to D-098 topology distribution) · 9 sidebar groups · ~95 scalar + **zero scalar top-level image** (growth-tech dashboard-console hero is typographic · first archetype with zero-scalar + non-zero-image-cells pattern) + 3 image-in-dict-row lists (home.work_cards.cover × 3 · studio.team.portrait × 3 · lavori.projects.cover × 6) — **12 total editable image surfaces · ALL RENDERED** (growth-tech skin · no storage-only split) · **18 readonly indexed list entries** (7 tuple + 11 dict · including `home.console.kpi` nested inside `home.console` parent dict) · **zero deep-path** · `.au-*` skin AR RTL (**25 rules · most robust of enrolled**) · **6 pages** (home · studio · capabilities · lavori · sprint · brief) · **1 derived kind** (project_detail via posts[]) · **posts list registry-only** · **7th uniform enforcement of per-item content policy** (Chiara + Pixel + Sapore + Brace + Bottega + Luxe + **Aura**) · **5th form-structure OUT precedent** (brief.slots + step1/2/3 + labels + placeholders + scope_options all OUT entire · explicit schema comment + dedicated contract test `test_a17_aura_brief_slots_kept_out_explicit`) · **Stringent IN col-level 8th archetype** (site.foot_boot_line + foot_current_sprint + sprint_chip + home.sprints.output + sprint.sprints.tagline + deliverables_label + capabilities.capabilities.scope_label) · **Zero raw SVG · zero bool flags · zero scheduler-state · zero novel shape** · **outside-gate fixture rotated to startup-saas-landing/elevate-startup-landing** (7th precedent · last non-enrolled archetype · A.17b upcoming) · **Playwright 5-focus walk validated**: 12 image widgets sidebar · isolation 3-target/9-baseline · posts absent da sidebar + Cmd+K "posts"→0 risultati · Elevate `/projects/start/?template=elevate-startup-landing` redirect 302 a catalog detail (NON editor) · AR `.au-*` con `html dir=rtl lang=ar` + body/nav/hero/foot tutti direction rtl + nav/hero/foot span identico 1057px zero drift + bodyScroll zero horizontal overflow + btn arrow `←` flipped + preview-bridge presente
- **Multi-locale editor on all eighteen** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker · image overrides global/plain-keyed (rendered + storage-only identical contract)
- **Rendered vs storage-only image widget distinction is PER-SKIN** (Bottega typographic 16 rendered + 6 storage-only · Luxe editorial 31 rendered + 0 storage-only · Salute clinical 15 rendered + 0 storage-only · Benessere olistico 19 rendered + 0 storage-only · Famiglia pediatric 16 rendered + 0 storage-only · **Aura growth-tech 12 rendered + 0 storage-only**): the schema exposes image fields identically · only skin rendering differs · registry data always preserved · editor contract uniform across all · storage plain-key per-locale identical
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- **357/357 server tests passing**, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **18 archetype slugs enrolled · 18 multi-locale enrolled · 19 templates editable end-to-end**
- **Families CLOSED (8)**: law (Lex + Juris sequential dedicated-schema · A.10+A.11) · medical-specialist (Cardio+Derm shared-schema · A.9) · real-estate (Casa+Villa staged dedicated-schema · A.12+A.12b) · portfolio (Chiara+Pixel staged dedicated-schema · A.13+A.13b) · restaurant-continuation (Sapore+Brace staged dedicated-schema · A.14+A.14b) · ecommerce (Bottega+Luxe staged dedicated-schema · A.15+A.15b) · medical-other (Salute+Benessere+Famiglia staged dedicated-schema 3-phase variant · A.16+A.16b+A.16c) · **agency-secondary (Aura single-template dedicated-schema · A.17)**
- **Zero half-open families remain** · 1 single-template family open (startup-saas · Elevate only non-enrolled archetype · A.17b closer) · staged dedicated-schema progression 5 closed precedents (including first 3-phase variant) · shared-schema (A.9) + sequential dedicated-schema (A.10+A.11) entrambi single-use unchanged · **single-template dedicated-schema (A.17 agency-secondary) first precedent**
- **Render-side contract-alignment fix `f66ac24` cross-pattern confirmed su 3 archetypes**: Sapore A.14 (tuple-in-dict-list first) · Brace A.14b (dict-in-dict-list via list-index) · **Famiglia A.16c (tuple-in-dict-list mirror di Sapore)** · 3 editor layers fully aligned su list numeric indexing
- **Detail-page registry-only policy enforced uniformly on 6 archetypes**: count invariato · Famiglia + Benessere + Salute + Luxe + Bottega + Sapore + Brace shippano `posts: []` strutturalmente
- **Guard removal pattern consolidated · 6 precedents**: Villa-out (A.12b) · Pixel-out (A.13b) · Brace-out (A.14b) · Luxe-out (A.15b) · Wellness-out (A.16b) · **Family-out (A.16c · `test_a16c_family_out_guard_was_removed_from_salute_tests`)** · **2-removal-phase sub-recipe variant COMPLETED** (first 3-template staged progression closed · one-opener-two-closers topology proven · sub-recipe supports both 2-phase and 3-phase variants)
- **Outside-gate fixture rotation alla `agency-digital-studio`/`aura-digital-studio`** · prossimo candidato enrollment è A.17 Aura (chiude agency-secondary) · alternative A.17 Elevate (chiude startup-saas)
- **Image surface patterns con multiple precedents**: typographic-first single-no-image (Bottega) · single-scalar-image (Pragma · Pixel · Luxe · Salute) · multiple-scalar-images (Gusto · specialist · Villa · Sapore · Brace · Benessere · **Famiglia 3 scalar**) · nested-dict-scalar-image (Vertex · Chiara · Bottega · Luxe con 2) · image-in-dict-row (Vertex · Villa · Chiara · Sapore · Brace · Bottega · Luxe · Salute · Benessere · **Famiglia con 3 liste + novel `src` col name**)
- **Stringent IN col-level audit** estesa a **7 archetypes**: Sapore (A.14) → Brace (A.14b) → Bottega (A.15) → Luxe (A.15b) → Salute (A.16) → Benessere (A.16b) → **Famiglia (A.16c · meta + icon text-refs + tag + exp_label/exp_value + wa_label + age)** · audit-driven · fields editorial visible stay IN · technical/scheduler-state/bool stay OUT
- **5 OUT categories formalizzate**: flat-list-of-str (Juris) · nested-list-of-str-in-dict-rows (Juris) · form structures (Juris/Gusto · nested-dict OR list-of-dict) · structural identifiers + commerce-state-like booleans (Bottega/Luxe `available` · Salute `is_popular` · Benessere `has_slots`/`soldout`) · raw SVG fields (Salute 5th category)
- **4 deferral categories formalized**: mutable repeater (D-098) · image per-locale (D-098) · detail-page editing (6-archetype uniform enforcement) · novel-shape-deferred (A.16b `home.ambients` tuple-with-image · first-ever)
- **Boundary editor-vs-{commerce,clinic}-admin clean · runtime-verified su 5 archetypes**: editor edits presentational demo showcase content in template_content registry · {commerce,clinic}-admin scopes managed separately via dedicated dashboards · light smoke tests su Bottega + Luxe + Salute + Benessere + **Famiglia (3 medical-other archetypes confirm boundary uniform)**
- **Deep-path Sapore precedent re-used mechanically on 3rd archetype (A.16c)** · f66ac24 render-side contract-alignment fix covers tuple-in-dict-list + dict-in-dict-list parent walks uniformly · deep-path edit surgical isolation verified runtime (target cell only · 32-cell granularity)
- **Families still open for editor enrollment (2 families · 2 templates total · zero half-open)**: **agency-secondary** (Aura · 1 template) · **startup-saas** (Elevate · 1 template)

No explicitly-deferred debt is pending. **18 commit-clean phases** delivered back-to-back (A.6 → A.16c). A.16c was 3 commits on branch (schema · lifecycle · merge) · zero additional infra fixes required (mechanical reuse di tutti i pattern pre-esistenti · 7th consecutive staged enrollment · 5th staged dedicated-schema family closure).

### Next workstream — immediate candidates (ZERO half-open families · Aura + Elevate single-template residues)

A.16c closes the medical-other family. **All 3 staged family progressions have closed** · zero half-open families remain. Only 2 residual templates are editor-only-pending: `agency-digital-studio` (Aura · single-template in agency-secondary category · Vertex already enrolled as `agency-creative-studio`) and `startup-saas-landing` (Elevate · single-template in startup-saas category). Both are **single-template families** (no staged-progression applies) · low novelty · low complexity.

- [ ] **A.17 · Aura (`agency-digital-studio`) individual enrollment — closes agency-secondary category** — single-template family · low-novelty mechanical reuse of established recipe · outside-gate fixture already rotated to `aura-digital-studio` in A.16c as next candidate signal. Step-0 audit needed: content shape + image surfaces + form handling + AR RTL rule count. Expected schema LOC modest (under 600 soft guardrail · no deep-path expected · precedent: Pragma `corporate-suite` was +~450 LOC · Elevate `startup-saas-landing` should be similar single-template size).
- [ ] **A.17 alt · Elevate (`startup-saas-landing`) individual enrollment first** — alternative ordering · single-template · closes startup-saas category. Same structural profile as Aura (single-template · low novelty). Pick Aura first if Step-0 audit reveals shared infra leverage · Elevate first if preferred to close single-template families in the order they appeared in the catalog.
- [ ] **A.17 alt · MEMORY.md maintenance mini-phase** — auto-memory index ~35-36KB sopra warning-soglia 24.4KB (grows ~1KB per enrollment phase). Index entries can be shortened without losing content. **Separate housekeeping task, NOT bundled with an enrollment**. Low-value-low-risk. Could be queued after A.17 + A.17b close everything · clean consolidation point.
- [ ] **A.17 alt · `home.ambients` tuple-with-image widening mini-phase** — deferred novel shape from A.16b. Would require: runtime verification of tuple-cell-with-image-type handling (`services._resolve_path` + `schema._iter_indexed_groups` + `rendering._apply_indexed`) + widget-level render test. Low value (4 tiles only) · customer-signal-gated.
- [ ] **A.17 alt · Selective editor polish** — defer unless customer signal surfaces.
- [ ] **A.17 alt · Media evolution / remote storage (S3/Cloudinary)** — defer. Current infra scales through 17 archetypes · no technical trigger.
- [ ] **A.17 alt · Editor operator tools** — defer.
- [ ] **A.17 alt · Repeater-per-locale first wave** — defer. Out-of-scope family per D-098.

Recommended framing: **A.17 Aura (`agency-digital-studio`) Step-0 planning session** — closes agency-secondary · brings catalog to 18 archetype slugs / 19 templates editable / 8 families closed · leaves only Elevate for A.17b. Outside-gate fixture already rotated to `aura-digital-studio` in A.16c as next candidate signal. Low-risk mechanical reuse of established single-template recipe (precedent chain: Pragma A.6 · Lex A.10 · Juris A.11 · Casa A.12 · Villa A.12b · Chiara A.13 · Pixel A.13b · Sapore A.14 · Brace A.14b · Bottega A.15 · Luxe A.15b · Salute A.16 · Benessere A.16b · Famiglia A.16c).

**What NOT to open immediately (red-lamps)**:
- **Do NOT bundle Aura + Elevate into one A.17** · discipline violation (one phase = one archetype decision · 18 phases strong A.6→A.16c)
- **Do NOT re-open Famiglia / Salute / Benessere coverage expansion** · medical-other family CLOSED · any additional coverage would be scope-creep
- **Do NOT open clinic-admin editing on any medical-other** · boundary editor-vs-clinic-admin fermo · scheduler/booking/patient namespaces out of scope (3 archetypes light-smoke-tested)
- **Do NOT widen `home.ambients` in A.17** · deferred novel shape stays deferred · widening is a separate mini-phase or indefinite (customer-signal-gated)
- **Do NOT open detail-page editing as side quest** · horizontal feature cross-archetype (affects 6 archetypes with per-item content)
- **Do NOT add bool field type** · horizontal feature · all precedent bool OUT decisions preserved (Luxe/Salute/Benessere)
- **Do NOT add tuple-with-image widget** · horizontal feature · deferred from A.16b
- **Do NOT bundle MEMORY.md maintenance with an enrollment** · housekeeping stays separate
...
- Any bundled multi-archetype phase (18 phases A.6→A.16c have all respected discipline · preserve)
