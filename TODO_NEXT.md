...
## 🟢 Current State (2026-04-18 · after Session 68 A.13b Pixel (cinematic-photographer · portfolio family) Editor + Multi-locale Enrollment merge · CLOSES PORTFOLIO FAMILY)

Baseline `phase-integration-baseline-v15` tip is **`1c6b561`** (A.13b merge), pushed to origin. **10 archetype slugs enrolled / 11 templates editabili end-to-end**. Tenth enrollment closes the portfolio family: Vertex + Pragma + Gusto + specialist + classic-gold + modern-transparent + mass-market + ultra-luxury-cinematic + editorial-designer-grid + **cinematic-photographer** are all multi-locale enrolled. **Four families editor-complete** (law · medical-specialist · real-estate · portfolio). Five families still open for editor enrollment (9 templates residue).
...
- **Multi-locale editor on all ten** · 5 locales (it/en/fr/es/ar) with authentic RTL preview for Arabic · per-locale `@<locale>:<path>` storage · authored-only fallback · sidebar pill switcher · flush-before-switch · "per lingua" marker on translatable fields · image overrides stay global/plain-keyed including at deep paths
- Orphan asset GC available via `python manage.py gc_project_assets` (default dry-run, `--apply`, `--project`, `--grace`)
- 239/239 server tests passing, smoke_full 834/834 unchanged, catalog 20/20 `published_live` unchanged
- **10 archetype slugs enrolled · 10 multi-locale enrolled · 11 templates editable end-to-end**
...
- **Detail-page registry-only policy enforced uniformly on 6 archetypes**: Lex `notabili` · Juris `insights` · Casa `posts` · Villa `posts` · Chiara `posts` · Pixel `posts` + `series_detail`. Explicit guardrail tests reject per-item detail paths in each archetype. Detail-page-scoped editing would be a horizontal feature affecting all 6+ archetypes with per-item content — not acceptable as single-archetype special-casing.
- **Image surface patterns now have multiple precedents**: single-scalar-image (Pragma · Pixel) · multiple-scalar-images (Gusto · specialist · Villa) · nested-dict-scalar-image (Vertex · Chiara) · image-in-dict-row (Vertex A.3a/A.4 · Villa · Chiara deep path 2 levels). The recipe scales downward and upward.
- **Families still open for editor enrollment (5 families · 9 templates total)**: **restaurant-continuation** (Sapore + Brace beyond Gusto · 2 templates) · **medical-other** (Salute + Benessere + Famiglia · 3 templates) · **ecommerce** (Bottega + Luxe · 2 templates) · **agency-secondary** (Aura beyond Vertex · 1 template) · **startup-saas** (Elevate · 1 template)

No explicitly-deferred debt is pending. 11 commit-clean phases delivered back-to-back (A.6 → A.13b). Pixel-out guard and Chiara-out perimeter-guard inversion both tested contractually.

### Next workstream — immediate candidates (no family open; pick one)

A.13b closes a family; it does not prescribe the next workstream. Pick based on leverage (family-closure > single-template) and sequencing (one phase = one archetype decision):

- [ ] **A.14 Restaurant-continuation (Sapore + Brace beyond Gusto)** — closes the restaurant category (already 1/3 enrolled via Gusto). Two templates, likely distinct archetypes (`trattoria-warm` + `street-modern` per DNA registry). Step-0 audit decides topology; most probable outcome is sequential or staged dedicated-schema like real-estate / portfolio. Highest leverage candidate — restaurant category is the closest to full-editor coverage after portfolio.
- [ ] **A.14 alt · eCommerce family (Bottega + Luxe)** — `artisan-workshop` vs `fashion-editorial`. Session 41/42 observations suggest wide divergence. Bottega has commerce-foundation overlap (Phase 3a+3b already landed `apps/commerce/` seed for both) — worth a Step-0 check on whether the editor schema needs to avoid overlap with commerce-managed content (the editor should edit presentational/marketing content, not commerce-admin state).
- [ ] **A.14 alt · Medical-other family (Salute + Benessere + Famiglia)** — three single-template medical archetypes (`medical-clinic` / `wellness-serene` / `family-warm`) rolled `published_live` in Session 51 but never enrolled. **Three separate phases preferred over bundling** — the "one-phase-one-archetype" discipline is now 11 phases strong (A.6 through A.13b).
- [ ] **A.14 alt · Aura (agency-digital-studio) individual enrollment** — low novelty, low risk, single template. Aura is the second agency template beyond Vertex. Would close the agency category partially (Vertex already enrolled as `agency-creative-studio`). If Aura plus a future agency-tertiary archetype share substantial structure, shared-schema might apply; otherwise staged-with-Vertex-already-in.
- [ ] **A.14 alt · Elevate (startup-saas-landing) individual enrollment** — low novelty, single template. Closes the startup-saas category. Could be bundled considerations: Elevate is the ONLY template in its DNA family, so category closure is mechanical.
- [ ] **A.14 alt · MEMORY.md maintenance mini-phase** — auto-memory index ~27-28KB sopra warning-soglia 24.4KB dal post-A.11. Index entries can be shortened without losing content. **Separate housekeeping task, NOT bundled with an enrollment**. Low-value-low-risk; queue for a "breather" session between bigger phases.
- [ ] **A.14 alt · Selective editor polish** — search Cmd-K locale-aware ranking · "solo translatable" sidebar filter · badge-per-group pre-sync at mount · palette page-slug-match boost · sticky-last-page on reopen. Low-risk, low-value — only worth opening if customer signal surfaces.
- [ ] **A.14 alt · Media evolution / remote storage** — S3/Cloudinary swap of `ProjectAsset.file` backend. Must keep `/media/` accept path in parallel (D-095 binding). **Do NOT open unless a prod-launch timeline demands it**. A.12b+A.13+A.13b validated that current infra scales to image-heavy + deep-path + single-scalar archetypes without pressure — no technical trigger now.
- [ ] **A.14 alt · Editor operator tools** — admin-facing (zero customer surface): admin project list filters, GC scheduler, asset audit UI. Better after another scale-out consolidates the editor footprint.
- [ ] **A.14 alt · Repeater-per-locale first wave** — flip one or two mutable Vertex lists from global to translatable-per-locale. Out-of-scope family per D-098 · opens localized repeater content only if customer signal emerges.

Recommended framing: **A.14 Step-0 planning session on restaurant-continuation (Sapore + Brace)** — closing the restaurant category is the highest-leverage move after portfolio family closure. Decision deferred to the dedicated A.14 planning session.
...
- Any bundled multi-archetype phase (violates "one phase = one archetype" discipline · 11 phases strong A.6→A.13b)
