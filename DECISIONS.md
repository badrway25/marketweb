...
- **2026-04-17 · Session 61 · A.8** — Third enrollment: `fine-dining` (Gusto) with `test_a8_gusto_full_multilocale_lifecycle_end_to_end`. Combined A.6 schema register + A.7b gate flip in a single phase — no architectural change, pure recipe reuse. 8 contract tests (6 shape + Vertex/Pragma regression + 1 dedicated preview-bridge 3-point integration test). Confirms D-098 scales to an imagery-heavy non-typographic archetype with full RTL iframe behavior on the `.fd-*` skin. No new binding introduced.
- **2026-04-17 · Session 62 · A.9** — Fourth enrollment: `specialist` (Cardio + Derm) — **first multi-template archetype**. One shared schema unlocks two templates because both Cardio and Derm DNA carry `archetype: "specialist"` and share 95% of the content-tree shape (100% on 6/7 pages · 85% on home with D-064 divergent premium sections isolated and kept registry-only via a dedicated exclusion guard test). Two distinct lifecycle regression tests — `test_a9_cardio_full_multilocale_lifecycle_end_to_end` + `test_a9_derm_full_multilocale_lifecycle_end_to_end` — each with its own marker so a regression on either template surfaces with a clean name. **Operational clarification to the D-098 contract**: shared-schema enrollments are now valid, but every template carried in by the shared schema still requires its own dedicated lifecycle regression test. There is no "free ride": the schema is shared, the coverage is not. 10 contract tests + the user-imposed guardrail `test_a9_specialist_divergent_premium_sections_excluded` locks the D-064 divergent premium sections out of the whitelist via 4 independent check layers (`is_translatable` False · `validate_key_path` raises · `STRUCTURED_FIELD_SHAPES` absent · sidebar group ids free of divergent hints). No new binding introduced.
- **2026-04-17 · Session 63 · A.10** — Fifth enrollment: `classic-gold` (Lex · law family) with `test_a10_lex_full_multilocale_lifecycle_end_to_end`. **Single-template enrollment** in a category (`lawyer`) that also carries Juris (`modern-transparent`) — the two law templates are distinct archetypes with distinct skin folders and only ~25% content-tree overlap, so the A.9 shared-schema pattern does NOT apply and **shared-schema is not obligatory**. The decisive constraint from D-098 is the dedicated lifecycle regression test per enrolled template, regardless of schema topology. Juris stays explicitly out of the gate and is captured by an assertion inside `test_a10_lex_archetype_registered` (`modern-transparent` NOT in `_ARCHETYPE_SCHEMAS` NOR in `_MULTILOCALE_ENABLED_ARCHETYPES`) so a future accidental enrollment without its own dedicated phase would fail fast. 9 contract tests (shape + classifications + quadruple regression + preview-bridge 3-point integration) + the lifecycle test. Confirms D-098 scales cleanly to a fifth archetype on a `.lx-*` skin with full RTL iframe behavior. Five templates now editable end-to-end. No new binding introduced.
- **2026-04-17 · Session 64 · A.11** — Sixth enrollment: `modern-transparent` (Juris · law family · second template) with `test_a11_juris_full_multilocale_lifecycle_end_to_end`. **Second dedicated-schema enrollment inside the same category** — the law family is now closed completely but through TWO distinct archetypes (`classic-gold` + `modern-transparent`), not through a shared schema. **Operational clarification to the D-098 contract**: a family can legitimately close through multiple separate archetype enrollments, each with its own schema + skin bridge + lifecycle test. Shared-schema (A.9 recipe) is one way to close a family; stacked dedicated-schema enrollments (A.10 + A.11 recipe) are another. Neither is preferred by default — the Step-0 runtime audit decides, and D-098's invariant (dedicated lifecycle regression test per enrolled template) holds in both topologies. A.11 also introduces the first **zero-image archetype** enrollment: Juris ships no image fields anywhere in the registry, and a user-imposed guardrail test (`test_a11_juris_schema_contains_zero_image_fields`) iterates the entire schema + `STRUCTURED_FIELD_SHAPES` tree asserting zero `type: "image"` fields. This locks a DNA constraint into the test layer and stays compatible with D-098 (translatable/global classification is orthogonal to image handling). A companion guardrail test (`test_a11_juris_complex_shapes_excluded_from_perimeter`) rejects four categories of complex registry shapes (`approccio.dashboard_mock` nested-dict, flat `list-of-str` containers like `home.trust_logos` / `insights.topics`, nested `list-of-str` cells like `servizi.services[*].deliverables` / `settori.sectors[*].pain_points`-`signals`-`legal_ops`-bullets, and form-structure blocks) via `validate_key_path` so scope creep into non-scalar widgets cannot happen silently. 10 contract tests + the lifecycle test; `test_a10_lex_archetype_registered` was updated to drop the Juris-absence assertions now that Juris is enrolled (the Lex-must-stay regression guard is preserved). Six templates now editable end-to-end. No new binding introduced.
- **2026-04-18 · Session 65 · A.12** — Seventh enrollment: `mass-market` (Casa · real-estate family · first template) with `test_a12_casa_full_multilocale_lifecycle_end_to_end`. **Single-template enrollment opens a 2-template family WITHOUT closing it** — Villa (`ultra-luxury-cinematic`) stays explicitly OUT until the dedicated A.12b phase. **Operational clarification to the D-098 contract**: a family can be legitimately progressed in a staged way — one dedicated archetype enrolled now, the sibling archetype enrolled later in its own phase. This is a third valid family-progression topology alongside (i) shared-schema single-phase closure (A.9 specialist) and (ii) stacked dedicated-schema sequential closure (A.10+A.11 law). All three topologies preserve the D-098 invariant: dedicated lifecycle regression test per enrolled template. A.12 also confirms the **zero-image archetype pattern is reusable as a recipe**, not a one-off: the `test_a12_casa_schema_contains_zero_image_fields` guardrail is a mechanical mirror of A.11's equivalent (iterate schema + `STRUCTURED_FIELD_SHAPES` cols + cell_spec, assert no `type: "image"`), and it locks the DNA constraint identically. The Villa-out guard is enforced at TWO layers: registration-time inside `test_a12_casa_archetype_registered` (`ultra-luxury-cinematic NOT in _ARCHETYPE_SCHEMAS NOR in _MULTILOCALE_ENABLED_ARCHETYPES` + all 6 previous archetypes still enrolled) and execution-time inside the lifecycle test (runtime `assertNotIn` on the gate set), so a future accidental enrollment without a dedicated phase would fail fast on either layer. The complex-shape exclusion perimeter pattern (introduced in A.11) applied cleanly to Casa's distinct shape set: flat list-of-str containers (`home.search_widget.popular_tags` + `immobili.filters` + `immobili.sort_options`), form structure blocks (`valutazione` + `contatti` `form_fields`/`form_sections`), and per-property `posts` entries all rejected by `validate_key_path`. Schema LOC delta +604 (slightly over the 600-LOC soft guardrail) recorded as an explicit non-blocking exception: no new scope opened, tests green first-run, perimeter coherent, no service/rendering/views/models touched — the overshoot reflects Casa's structural richness (15 indexed lists vs Juris's 6). 10 contract tests + the lifecycle test. Seven templates now editable end-to-end; the real-estate family is open (Casa) but not yet closed (Villa pending). No new binding introduced.
- **2026-04-18 · Session 66 · A.12b** — Eighth enrollment: `ultra-luxury-cinematic` (Villa · real-estate family · second template) with `test_a12b_villa_full_multilocale_lifecycle_end_to_end`. **Closes the real-estate family** opened in A.12, completing the staged dedicated-schema progression topology. All three D-098 family-closure topologies now have ≥1 real precedent (shared-schema · sequential dedicated-schema · staged dedicated-schema). **Operational clarification added to D-098**: (i) **image-inside-list-of-dict-row is a reusable enrollment pattern, NOT a horizontal feature** — Vertex has exposed `studio.partners[].portrait` as `type: "image"` inside dict cols since A.3a/A.4 (production-hardened through ProjectAsset + `/assets/upload/`). Villa is the second archetype to use the pattern, with a *smaller* surface because Villa's lists are non-mutable (cell-level edit only, no add/remove). A.12b required zero changes to `services.py` / `rendering.py` / `views.py` / `models.py` / `live-editor.{css,js}` / `preview-bridge.js`. Pure enrollment: 3 files touched (`schema.py` + `_base.html` + `tests.py`). (ii) **Image fields stay global/plain-keyed even inside dict rows** — Villa ships 18 editable image surfaces (4 scalar + 14 cells across 3 dict lists: `home.signature[].image` × 4, `territorio.territories[].image` × 6, `studio.advisors[].portrait` × 4) and every override persists as a plain key, never as `@<locale>:path`, verified end-to-end in the lifecycle test with explicit assertNot on `@<locale>:home.cover_image` and `@<locale>:home.signature.0.image`. A per-locale image policy remains out-of-scope per D-098 — changing it would require an explicit product decision, not a planning tweak. (iii) **The Villa-out guard removal is itself a tested contract**: `test_a12b_villa_out_guard_was_removed_from_casa_tests` asserts Villa IS in both gate sets + Casa still enrolled, so the 2 Villa-absence assertions removed from the A.12 Casa tests (registration-time + runtime) are verified as correctly inverted rather than forgotten. The complex-shape exclusion perimeter pattern applied cleanly to Villa's distinct shape set: nested list-of-str inside dict rows (`collezione.filter_groups[].options` + `concierge.form_fields[].options`), flat list-of-str containers (`home.territory` + `home.press_items` + `collezione.sort_options` + 3 footer row lists), `concierge.form_fields` form structure, and all `posts[*]` entries (8 blog posts including the post image col) — 19 total paths rejected by `validate_key_path`. Schema LOC delta +587 — **under** the 600-LOC soft guardrail (vs Casa's +604 exception), margin preserved despite image col additions because Villa has 14 indexed lists vs Casa's 15 and fewer chrome/tile scalar labels. 10 contract tests + 1 lifecycle test (enriched with 1 scalar image override + 1 image-in-dict-row override + assertIn on both image URLs across all 5 public preview locales). Nine templates now editable end-to-end; real-estate family is the third to close after law (A.10+A.11) and medical-specialist (A.9). No new binding introduced.
- **2026-04-18 · Session 67 · A.13** — Ninth enrollment: `editorial-designer-grid` (Chiara · portfolio family · first template) with `test_a13_chiara_full_multilocale_lifecycle_end_to_end`. **Opens the portfolio family WITHOUT closing it** — Pixel (`cinematic-photographer`) stays OUT until A.13b, same staged dedicated-schema progression topology as real-estate (A.12+A.12b). **Operational clarification added to D-098**: (i) **Per-item detail-page editing stays OUT of enrollment perimeter as a coherent policy decision, NOT as an accidental limitation** — Chiara's `posts` list (3 project detail records with nested `sections`/`summary`/`deliverables`/`credits` bodies) is registry-only, consistent with Lex `notabili`, Juris `insights`, Casa `posts` (property detail), Villa `posts` (series detail). Five family closures + 1 open family now share this policy. Opening detail-page-scoped editing would be a horizontal feature affecting every archetype with per-item content (5+ archetypes) and needs uniform cross-archetype design — not single-archetype special-casing. `test_a13_chiara_complex_shapes_excluded_from_perimeter` formalizes this by explicitly rejecting 8 `posts.*` paths (`posts`, `posts.0.title`, `posts.0.lead`, `posts.0.sections`, `posts.0.summary`, `posts.0.deliverables`, `posts.0.credits`, `posts.2.next_label`) via `validate_key_path`, and the browser walk includes an explicit detail-page-registry-only verify step fetching `/preview/lavoro/<slug>/` for all 3 project records confirming static registry content renders with global chrome override but zero per-project editability. (ii) **Deep-path image-in-dict-row works at arbitrary path depth** — Chiara's `home.featured_works.items[].image` (path 2 levels deep via `home.featured_works` parent dict) is the third precedent of image-in-dict-row after Vertex `studio.partners[].portrait` (A.3a/A.4) and Villa's 3 A.12b image-col lists. `_resolve_path` walks any depth through dicts — verified Step-0 via direct code read. No infrastructure work required. `test_a13_chiara_image_cols_in_dict_shapes_exposed` provides **triple verification** per user request: (a) positive `get_field_spec` returning `type="image"` for deep paths `home.featured_works.items.0.image` and `home.featured_works.items.3.image`; (b) shape-level check via `get_list_shape` asserting `image` col exists in the indexed shape's cols list with `type="image"`; (c) the nested-dict scalar image `studio.founder.image` (Vertex `home.cover.image` precedent) verified separately. (iii) **The `.ed-*` CSS prefix collision with editor sidebar namespace is non-impacting** — editor sidebar (`.ed-sidebar`, `.ed-field`, `.ed-lang-pill`) lives in the editor shell DOM while the skin `.ed-nav`/`.ed-hero`/`.ed-foot`/etc. live in the preview iframe DOM. Different DOM trees, zero functional conflict. Documented in skin CSS guard block comment. 10 contract tests + 1 lifecycle test (enriched with page-specific image assertions: `home.featured_works.items.0.image` verified on home renders, `studio.founder.image` verified on studio renders, across all 5 public-preview locales). Schema LOC delta +533 — **under** the 600-LOC soft guardrail (vs Casa +604 exception, Villa +587). Pixel-out guard enforced at 2 layers (registration-time inside `test_a13_chiara_archetype_registered` + runtime inside the lifecycle test) — same pattern as A.12 Villa-out, will be removed in A.13b together with Pixel's own registrations. Ten templates now editable end-to-end; portfolio family is open (Chiara) but not yet closed (Pixel pending). No new binding introduced.
- **2026-04-18 · Session 68 · A.13b** — Tenth enrollment: `cinematic-photographer` (Pixel · portfolio family · second template) with `test_a13b_pixel_full_multilocale_lifecycle_end_to_end`. **Closes the portfolio family** opened in A.13 · same staged dedicated-schema progression topology as real-estate (A.12+A.12b). Four families editor-complete now: law · medical-specialist · real-estate · portfolio. **Operational clarification added to D-098 (no new D-number)**: (i) **Detail-page registry-only is a perimeter decision coerente, not an accidental limitation** — Pixel's `posts.*` incl. `posts[].cover_image` and `series_detail` stay registry-only, bringing the uniform enforcement count to 6 archetypes (Lex `notabili` · Juris `insights` · Casa `posts` · Villa `posts` · Chiara `posts` · Pixel `posts`+`series_detail`). Opening detail-page-scoped editing would be a horizontal feature affecting all 6+ archetypes with per-item content and must be designed cross-archetype, not Pixel-special-cased. The lifecycle test re-checks this invariant end-of-test by calling `validate_key_path` against 6 posts paths and the Step 3 browser walk includes an explicit detail-page-registry-only verify fetching `/preview/serie/<slug>/` for all 3 series records confirming zero walk-text leak. (ii) **Pixel consolidates the "single scalar image archetype enrollment" pattern** — `home.hero_image` is the only editable image surface; no image-in-dict-row, no nested-dict scalar image, no mutable repeaters. This confirms that a new archetype enrollment does NOT need image-in-dict-row or nested-dict scalar image to be complete — the recipe scales downward (simpler archetypes) as cleanly as it scales upward (Villa 18-image-surfaces, Chiara deep-path image-in-dict-row). Schema LOC delta +510 — comfortably under the 600-LOC soft guardrail (vs Chiara +533, Villa +587, Casa +604 exception). Zero service-layer / rendering / editor-widget changes required — pure enrollment on 3-file surface (`schema.py` + `_base.html` + `tests.py`). (iii) **Staged dedicated-schema closure now has 2 real precedents** (real-estate A.12+A.12b · portfolio A.13+A.13b). The three D-098 family-closure topologies each have ≥1 precedent; staged dedicated-schema has the highest precedent count. Shared-schema (A.9 specialist), sequential dedicated-schema (A.10+A.11 law), staged dedicated-schema (A.12+A.12b real-estate AND A.13+A.13b portfolio). D-098 invariant holds uniformly: dedicated lifecycle regression test per enrolled template regardless of topology. (iv) **The Chiara-out guard removal / Pixel-in inversion was done in controlled fashion and contract-tested**: `test_a13b_pixel_out_guard_was_removed_from_chiara_tests` asserts Pixel IS in both gate sets + Chiara still enrolled, mirroring the A.12b Villa-out-removal pattern. The 2 Pixel-absence assertions removed from the A.13 Chiara tests (registration-time + runtime) are verified as correctly inverted rather than forgotten. 11 contract tests + 1 lifecycle test. Pixel brings the catalog to 10 archetype slugs enrolled · 10 multi-locale enrolled · 11 templates editable end-to-end. Baseline tip `1c6b561`. No new binding introduced.
- **2026-04-18 · Session 69 · A.14** — Eleventh enrollment: `trattoria-warm` (Sapore · restaurant-continuation family · first template) with `test_a14_sapore_full_multilocale_lifecycle_end_to_end`. **Opens the restaurant-continuation family WITHOUT closing it** — Brace (`street-modern`) stays OUT until A.14b, same staged dedicated-schema progression topology already applied twice (real-estate A.12+A.12b · portfolio A.13+A.13b). Staged dedicated-schema closure now has 3 real precedents (2 closed · 1 half-open). **Operational clarification added to D-098 (no new D-number)**: (i) **Deep-path tuple-in-dict-list parent support is now fully aligned across save/schema/render layers** — Sapore's `menu.sections.{i}.dishes` is the first enrollment to expose tuple cells nested inside a dict-list parent (novel shape; Chiara's deep-path was dict-inside-dict parent). The three editor layers that walk dotted paths (`services._resolve_path` for save/diff/prefill · `schema._resolve_path` for `_iter_indexed_groups` · `rendering._apply_indexed` for render-time splicing) were historically built at different times and the render layer lagged behind the others on list numeric indexing. Step 1 extended `schema._resolve_path` with an 8-line list-index branch (mirror of services.py); Step 2 extended `rendering._apply_indexed` parent-walk identically. The render-side change is a **contract-alignment bugfix**, NOT scope-creep: without it, `menu.sections.0.dishes.0.name` overrides were silently accepted-but-never-rendered (save layer persisted them correctly, render layer dropped them). The fix is minimal, narrow, and mirrors a capability already in `services._resolve_path`. Committed as `fix: apply indexed overrides through list parents in renderer` (commit `f66ac24`), isolated from the test commit for clean history. (ii) **Menu rows stay in-perimeter on restaurant templates** — now precedentato via 5 separate `menu.sections.{0..4}.dishes` STRUCTURED_FIELD_SHAPES entries, one per section, each addressing a tuple 7/7/6/5/5 rows × 3 cols (name/desc/price). Browser walk verified the deep-path override renders universally across all 5 public-preview locales including menu page AR RTL. Leaving menu out of perimeter would have made the enrollment fake-editable (menu is the editorial heart of the template); the user-imposed "perimetro vero, non finto" guardrail in Step 0 rejected that option. (iii) **Brace-out guard dual layer + symmetric removal plan** — at registration-time (`test_a14_sapore_archetype_registered` asserts `street-modern NOT in _ARCHETYPE_SCHEMAS NOR in _MULTILOCALE_ENABLED_ARCHETYPES` + all 10 pre-A.14 archetypes still enrolled) and at runtime (lifecycle test end-of-test re-check). Both guards will be removed in A.14b together with Brace's own registrations, contract-tested via the now-standard `test_a14b_brace_out_guard_was_removed_from_sapore_tests` mirror (same pattern as A.12b Villa-out-removal and A.13b Pixel-out-removal). (iv) **First enrollment since A.10 without a posts list** — Sapore ships `posts: []` in the content registry, so the detail-page registry-only policy doesn't apply structurally (no per-item content to lock). The cross-archetype uniform enforcement count stays at 6 (Lex+Juris+Casa+Villa+Chiara+Pixel) — restaurant-continuation templates sit outside the detail-page policy because they have no per-item content to begin with. 12 contract tests + 1 lifecycle test. Schema LOC delta +663 (slightly over the 600-LOC soft guardrail) recorded as a non-blocking exception: the overshoot reflects the 5 additional menu-deep-path shape entries (one per section · each ~25-30 LOC) which are structurally necessary to keep the menu in-perimeter. Alternative would have been accepting fake-editable menu or appiattire 105 celle in pseudo-path scalars (worse). Sapore brings the catalog to 11 archetype slugs enrolled · 11 multi-locale enrolled · 12 templates editable end-to-end. Baseline tip `8fae2df`. No new binding introduced.
- **2026-04-19 · Session 70 · A.14b** — Twelfth enrollment: `street-modern` (Brace · restaurant-continuation family · second template) with `test_a14b_brace_full_multilocale_lifecycle_end_to_end`. **Closes the restaurant-continuation family** opened in A.14 with Sapore · **fifth family editor-complete** (law · medical-specialist · real-estate · portfolio · restaurant-continuation). **Operational clarification added to D-098 (no new D-number)**: (i) **Staged dedicated-schema closure topology now has 3 real closed precedents** (real-estate A.12+A.12b · portfolio A.13+A.13b · restaurant-continuation A.14+A.14b) · the three D-098 family-closure topologies all have ≥1 precedent with staged dedicated-schema now the most-applied (3 closures). Shared-schema remains single-use (A.9 specialist) and sequential dedicated-schema also single-use (A.10+A.11 law) · the Step-0 runtime audit decides per-enrollment which topology applies. (ii) **The A.14 Step 2 render-side fix (commit `f66ac24`) is cross-pattern confirmed**: the 3 editor layers (`services._resolve_path` · `schema._resolve_path` · `rendering._apply_indexed`) now speak the same language on list-index deep paths regardless of the child shape. Sapore A.14 validated the fix on tuple-in-dict-list parent (`menu.sections.{i}.dishes`); Brace A.14b validates the same fix on **dict-in-dict-list parent** (`menu.sections.{i}.items[].image` · Chiara-precedent shape) AND on tuple-in-dict-list parent (`ordina.routes.{i}.lines[].value` · Sapore-precedent shape) — both browser-walk verified end-to-end on all 5 public-preview locales. The contract-alignment bugfix thesis is operationally validated across both families of deep-path parent shapes. (iii) **Deep-path dict-in-dict-list parent precedent is expanded**: Chiara was the first (`home.featured_works.items[].image` via parent-DICT), Brace is the second (`menu.sections.{i}.items[].image` via parent-LIST — list-index in the parent chain, exercised for the first time) · the render-side fix covers both variants uniformly. (iv) **Guard removal pattern consolidated across 3 precedents**: Villa-out (A.12b · `test_a12b_villa_out_guard_was_removed_from_casa_tests`) · Pixel-out (A.13b · `test_a13b_pixel_out_guard_was_removed_from_chiara_tests`) · **Brace-out (A.14b · `test_a14b_brace_out_guard_was_removed_from_sapore_tests`)**. Each uses the same contract: assert target archetype IS in both gate sets + companion archetype still enrolled · dual-layer assertion removal (registration-time + runtime) verified correctly inverted rather than forgotten. Symmetric guard-removal is now a **stable enrollment-closure sub-recipe** for staged dedicated-schema progressions. (v) **Outside-gate fixture rotation is part of enrollment closure**: when A.14b enrolled Brace, the 6 pre-existing "unsupported archetype" test fixtures were rotated from `street-modern`/`brace-street-food-lab` to `artisan-workshop`/`bottega-shop-artigianale` (next likely enrollment candidate). This keeps the fixture set aligned with the current outside-gate reality without creating test-debt. 13 contract tests + 1 lifecycle test. Schema LOC delta +826 — recorded as non-blocking exception over 600-LOC soft guardrail: overshoot reflects 8 deep-path entries (5 `menu.sections.{0..4}.items` + 3 `ordina.routes.{0..2}.lines`) + volume 3.4× Sapore (44 image surfaces vs 13 · 30 list entries vs 20). Brace ships ZERO form structures (smaller out-policy set than Sapore) and EMPTY posts list (same as Sapore · detail-page policy stays at 6-archetype uniform enforcement). Brace brings the catalog to 12 archetype slugs enrolled · 12 multi-locale enrolled · 13 templates editable end-to-end · 5 families closed. Baseline tip `7c064f8`. No new binding introduced.
- **2026-04-19 · Session 71 · A.15** — Thirteenth enrollment: `artisan-workshop` (Bottega · ecommerce family · first template) with `test_a15_bottega_full_multilocale_lifecycle_end_to_end`. **Opens the ecommerce family WITHOUT closing it** — Luxe (`fashion-editorial`) stays OUT until A.15b · fourth consecutive staged dedicated-schema opening after real-estate (A.12) · portfolio (A.13) · restaurant-continuation (A.14). Staged dedicated-schema progression is now the dominant topology with 3 closed precedents + 1 half-open. **Operational clarification added to D-098 (no new D-number)**: (i) **Editor boundary vs `apps/commerce/` commerce-admin state is clean and runtime-verified**. Step-0 audit confirmed that `apps/catalog/views.py::LiveTemplateView` (preview render for editor iframe) does NOT import from `apps.commerce` — the two surfaces are ORTHOGONAL. The editor edits presentational demo showcase content in `template_content_bottega.py` (registry): product card listings demo · single-product demo record · hero/copy bands · journal entries · atelier about. The real catalog state (Storefront/Product/Variant/Cart/Order/PaymentIntent) lives in `apps/commerce/` models and is managed via seller dashboard (Phase 3a/3b). Zero cross-contamination by design. A light guardrail test `test_a15_bottega_commerce_admin_boundary` smoke-checks that the editor schema doesn't accidentally reference `apps.commerce` model namespaces (storefront./cart./variant./order./payment_intent./checkout./storefront_member.) — narrow sanity check, not an architectural test. This boundary is the reason A.15 is a pure 3-file enrollment surface (schema + `_base.html` + tests) with zero touches to `apps/commerce`. (ii) **Rendered vs storage-only image widget distinction is INTENTIONAL, not a bug**. Browser walk confirmed 22 total image widgets in Bottega sidebar = 16 rendered (`home.latest_items.{0..3}.image` · `shop.products.{0..8}.image` · `product.related_items.{0..2}.image`) + 6 storage-only (`product.artisan_portrait` · `atelier.founder_portrait` · `home.makers.{0..3}.portrait`). The 6 storage-only fields are exposed because the registry data is preserved (Session 42 D-073 artisan-workshop typographic-first conversion: crest-mark glyphs + monogram stamps replace portrait photos · data kept for future skin variants / customer swap · editor contract identical). Storage blindatura (plain-key per-locale) is verified on both categories; only VISIBILITY differs per skin rendering. Lifecycle test exercises 5 image overrides (3 rendered + 2 storage-only) with identical 5× `assertNotIn("@<locale>:")` contract · documented in test docstring. This is not a new pattern — it's the explicit handling of a pre-existing DNA-honest skin choice. (iii) **Stringent IN/OUT call on "technical-looking" fields** per user Step 1 guidance: values that LOOK structural but are editorial visible ('N° 042' / '3 / 8' / 'Esaurito' / '01'/'02'/'03' / 'TOP'/'NEW'/'VEG') must stay IN rather than being classified OUT by inertia. Bottega kept IN: `shop.products[].n` · `.edition` · `.tag` · `home.latest_items[].n`/`.edition`/`.tag` · `home.provenance_items[].icon` · `atelier.process_steps[].n` · `journal.entries[].n` · `product.related_items[].n`. Bottega kept OUT (truly structural): `shop.products[].id` (slug routing) · `.available` (bool flag · commerce-state-like semantic) · `home.latest_items[].id` · `product.related_items[].id`. Dedicated contract test `test_a15_bottega_visible_catalog_fields_kept_in` blindates the IN call; `test_a15_bottega_complex_shapes_excluded_from_perimeter` blindates the OUT call. Same category as Sapore forno.pizza_signatures.n and Chiara ledger_rows.num precedent decisions. (iv) **Luxe-out guard pattern ready for A.15b consolidation**: Luxe-out dual guard (registration-time in `test_a15_bottega_archetype_registered` + runtime at end-of-lifecycle-test) will be removed in A.15b via `test_a15b_luxe_out_guard_was_removed_from_bottega_tests` · fourth precedente del guard removal pattern dopo Villa/Pixel/Brace. (v) **Outside-gate fixture rotation** reapplied as standard closure sub-recipe: 6 pre-existing `unsupported archetype` fixtures rotated from `artisan-workshop`/`bottega-shop-artigianale` to `fashion-editorial`/`luxe-fashion-store` · now the standard closure workflow for staged dedicated-schema phases. 13 contract tests + 1 lifecycle test. Schema LOC delta +585 — **sotto** the 600-LOC soft guardrail (leaner than initial planning estimate of 650 · 14 entries without deep-path = simpler than Sapore/Brace with 20/30 entries respectively). Zero service-layer / rendering / editor-widget / `apps/commerce/` changes required — pure 3-file enrollment surface. Bottega brings the catalog to 13 archetype slugs enrolled · 13 multi-locale enrolled · 14 templates editable end-to-end. Baseline tip `fab3ebc`. No new binding introduced.
- **2026-04-19 · Session 72 · A.15b** — Fourteenth enrollment: `fashion-editorial` (Luxe · ecommerce family · second template) with `test_a15b_luxe_full_multilocale_lifecycle_end_to_end`. **Closes the ecommerce family** opened in A.15 with Bottega · **sixth family editor-complete** (law · medical-specialist · real-estate · portfolio · restaurant-continuation · ecommerce). Fourth staged dedicated-schema closure consecutive · staged dedicated-schema is now the dominant topology with **4 closed precedents** + 0 half-open. **Operational clarification added to D-098 (no new D-number)**: (i) **Staged dedicated-schema closure topology extends to 4 real closed precedents** (real-estate A.12+A.12b · portfolio A.13+A.13b · restaurant-continuation A.14+A.14b · ecommerce A.15+A.15b). Shared-schema remains single-use (A.9 specialist) and sequential dedicated-schema also single-use (A.10+A.11 law) · the Step-0 runtime audit decides per-enrollment which topology applies · staged dedicated-schema is the preferred topology where Step-0 audit confirms ≥2 distinct archetypes with divergent skin / content shape / DNA tone. (ii) **Boundary editor-vs-commerce-admin re-verified on second ecommerce archetype**. Luxe preserves the same architectural boundary validated Step-0 in A.15 Bottega: `apps/catalog/views.py::LiveTemplateView` does NOT import from `apps.commerce` · the editor edits presentational demo showcase content in `template_content_luxe.py` (registry: collezione product card listings demo · single-product demo record · hero/copy bands · lookbook editorial · maison about) · real catalog state stays in `apps/commerce/` managed via seller dashboard (Phase 3a/3b). Light guardrail test `test_a15b_luxe_commerce_admin_boundary` mirrors the A.15 Bottega contract — narrow sanity check that editor schema doesn't accidentally reference apps.commerce model namespaces. This boundary is the reason A.15b is a pure 3-file enrollment surface (schema + `_base.html` + tests) with zero touches to `apps/commerce`. Second ecommerce archetype confirms the boundary holds uniformly across the family, not a Bottega-special case. (iii) **Rendered vs storage-only image distinction is PER-SKIN, not per-schema**. Bottega ships 22 image widgets = 16 rendered + 6 storage-only (typographic-first DNA · Session 42 D-073 · crest-mark glyphs replace portraits). Luxe ships **31 image widgets ALL RENDERED** — photographically editorial campaign-driven DNA paints every surface. The schema exposes image fields identically in both cases (same `type: "image"` · same STRUCTURED_FIELD_SHAPES col registration · same plain-key per-locale storage contract); only skin rendering differs. This confirms that storage-only vs rendered is a **skin-level concern** (whether the template HTML paints the field), NOT a schema-level concern (whether the field is editable) · the registry data is always preserved, the editor contract is always uniform, only the skin varies. Lifecycle + browser walk verified all 31 Luxe image surfaces render across 5 locales × relevant pages. (iv) **Stringent IN col-level audit extended to drop/n/tag on 4th archetype**. Precedent chain now: Sapore forno.pizza_signatures.n (roman numeral visible counter · A.14) → Brace ordina routes n (A.14b) → Bottega shop.products n/edition/tag (A.15) → **Luxe collezione.products drop/n/tag (A.15b)**. Luxe IN: `collezione.products[].drop` ('Drop 01 · Spring 26' · editorial drop badge) · `.n` ('Look 03' · Look numbering visible) · `.tag` ('Lista d'attesa' · 'Capsula' · 'Archivio' · editorial availability badge) · `home.tiles[].tag` · `product.related_items[].n` · `lookbook.looks[].n`. Luxe OUT (truly structural): `collezione.products[].id` (slug routing) · `.available` (bool flag · commerce-state-like) · `home.tiles[].id` · `product.related_items[].id`. Dedicated contract test `test_a15b_luxe_visible_catalog_fields_kept_in` blindates the IN call; `test_a15b_luxe_complex_shapes_excluded_from_perimeter` blindates the OUT call. Audit-driven · non-inertial: fields that look technical but are customer-facing editorial content must stay IN. (v) **Guard removal sub-recipe consolidated across 4 precedents**: Villa-out (A.12b) · Pixel-out (A.13b) · Brace-out (A.14b) · **Luxe-out (A.15b via `test_a15b_luxe_out_guard_was_removed_from_bottega_tests`)**. Triple-layer assertion removal: registration-time (inside `test_a15_bottega_archetype_registered`) + runtime start + runtime end (inside the A.15 lifecycle test). All three removals verified correctly inverted rather than forgotten. **Guard removal pattern is now a stable sub-recipe standard** for every staged dedicated-schema closure phase. (vi) **Outside-gate fixture rotation to `clinic`/`salute-studio-medico`** — first rotation to a non-restaurant / non-ecommerce target since A.12+ (Pixel A.13b was ecommerce-pointing). The medical-clinic family (Salute · Benessere · Famiglia) is the next likely enrollment wave (3 templates editor-only-pending with 100% 5-locale parity already shipped in Sessions 51/52). 6 fixture location rotated as standard closure sub-recipe. 14 contract tests + 1 lifecycle test. Schema LOC delta +598 — **sotto** the 600-LOC soft guardrail (vs Bottega +585 · leaner than initial planning estimate of 680-720 · 17 entries without deep-path = simpler than Brace 30 · richer than Bottega 14). Zero service-layer / rendering / editor-widget / `apps/commerce/` changes required — pure 3-file enrollment surface confirmed as sub-recipe standard after 4 consecutive staged closures. Luxe brings the catalog to 14 archetype slugs enrolled · 14 multi-locale enrolled · 15 templates editable end-to-end · 6 families closed. Baseline tip `bd9ffcb`. No new binding introduced.

- **2026-04-19 · Session 73 · A.16** — Fifteenth enrollment: `clinic` (Salute · medical-other family · first template) with `test_a16_salute_full_multilocale_lifecycle_end_to_end`. **Opens the medical-other family WITHOUT closing it** — Benessere (`wellness`) + Famiglia (`family`) stay OUT until A.16b + A.16c · staged dedicated-schema progression EXTENDED TO **3-PHASE VARIANT** (first 3-template family to enroll). **Operational clarification added to D-098 (no new D-number)**: (i) **Staged dedicated-schema progression generalizes from 2-phase to 3-phase variant**. Previous staged closures were 2-template families (real-estate · portfolio · restaurant-continuation · ecommerce · all 4 closed). Medical-other has 3 templates with 3 distinct archetypes (clinic · wellness · family) and ~0% content-tree overlap (verified Step 0 audit) · shared-schema impossible. The 3-phase variant is mechanical generalization: A.16 Salute opens with **DUAL-OUT GUARD** (planted for both wellness + family) · A.16b Benessere removes wellness-out guard (keeps family-out active) · A.16c Famiglia removes family-out guard + closes the family. Sub-recipe pattern extends from 1-removal to 2-removal phase, preserving D-098 invariant (one lifecycle test per enrolled template) regardless of phase count. (ii) **NOVEL 5th OUT category precedent: raw `icon_svg` fields**. Salute ships 18 raw SVG XML markup fields (`home.specialties[].icon_svg` × 8 · `servizi.services[].icon_svg` × 10) used for inline icon rendering on specialty/service cards. Editing raw SVG XML as plain text is unsafe (XSS-like content via paste · Pillow.verify only covers image binaries) and customer-unfriendly UX (no SVG-editor widget exists in the editor shell · adding one is a horizontal feature). **OUT col-level** via STRUCTURED_FIELD_SHAPES col exclusion (not added to cols list for either shape) + dedicated contract test `test_a16_salute_raw_svg_fields_excluded` blindates exclusion at 2 layers (`get_field_spec` returns None · `validate_key_path` raises `InvalidEditableField`). Raw SVG joins the 4 prior OUT categories: flat-list-of-str · nested-list-of-str · form structures · structural identifiers/commerce-state-like booleans. (iii) **Bool flag handling: OUT col-level (no horizontal field type added)**. The editor schema currently supports text/textarea/richtext/image/url/select but not bool. Adding bool support would be a horizontal feature affecting every archetype with bool fields (Bottega/Luxe `available` already precedent). For Salute, `prevenzione.packages[].is_popular` (editorial highlight badge flag) stays OUT col-level while `popular_label` ('Più richiesto' badge text) stays editable IN — customer can change the badge label but not toggle which package is 'most popular'. Pragmatic limitation that preserves no-horizontal-feature discipline. Same reasoning applies to Benessere `home.calendar[].has_slots`/`.soldout` (commerce-state-like scheduler flags · pending A.16b OUT). (iv) **Stringent IN col-level audit extended to 5 archetypes**. Precedent chain: Sapore forno.pizza_signatures.n (A.14) → Brace ordina routes n (A.14b) → Bottega shop.products n/edition/tag (A.15) → Luxe collezione.products drop/n/tag (A.15b) → **Salute home.journey_steps/prevenzione.how_steps/prenota.help_steps `num` + prevenzione.packages.popular_label (A.16)**. Audit-driven · non-inertial: fields that look technical but are customer-facing editorial content must stay IN. Salute IN: `num` (visible step numbering '01'/'02'/'03'/'04' across journey + how + help · pre-existing precedent) · `popular_label` (customer-facing 'Più richiesto' badge text). Salute OUT (truly structural/technical): `is_popular` (bool · no type support) · `icon_svg` (raw SVG · safety + UX). Dedicated contract tests `test_a16_salute_visible_catalog_fields_kept_in` + `test_a16_salute_complex_shapes_excluded_from_perimeter` + `test_a16_salute_raw_svg_fields_excluded` blindate both calls. (v) **Boundary editor-vs-clinic-admin verified runtime**. Step-0 audit confirmed `apps/catalog/views.py::LiveTemplateView` has ZERO imports from scheduler/booking/patient model namespaces — the editor surface (template_content registry · presentational demo content) and clinic-admin state (no real `apps/clinic_admin/` exists yet, but the boundary contract is established for future Phase 4+ work) stay ORTHOGONAL by design. Light guardrail test `test_a16_salute_clinic_admin_boundary` smoke-checks that the editor schema doesn't accidentally reference future clinic-admin namespaces (`appointment.` · `booking.` · `patient.` · `scheduler.` · `calendar_slot.` · `medical_record.` · `prescription.`) — narrow sanity check, not an architectural test. This boundary is the reason A.16 stays a pure 3-file enrollment surface (schema + `_base.html` + tests) with zero touches to any admin/data-like layer. Same shape as A.15/A.15b commerce-admin boundary — second category-class boundary established (clinic-admin · ecommerce-admin). (vi) **Outside-gate fixture rotation to `wellness`/`benessere-centro-olistico`** — pre-planted signal for A.16b. 6 fixture rotated as standard closure sub-recipe. 13 contract tests + 1 lifecycle test. Schema LOC delta +512 — **sotto** the 600-LOC soft guardrail (leaner than initial planning estimate 500-600 · 16 entries without deep-path = mid-complexity between Bottega 14 and Brace 30). Zero service-layer / rendering / editor-widget / `apps/commerce/` changes required — pure 3-file enrollment surface. Salute brings the catalog to 15 archetype slugs enrolled · 15 multi-locale enrolled · 16 templates editable end-to-end · 6 families closed + medical-other half-open. Baseline tip `e25b622`. No new binding introduced.

- **2026-04-19 · Session 74 · A.16b** — Sixteenth enrollment: `wellness` (Benessere · medical-other family · second template · MIDDLE phase of 3-phase staged progression) with `test_a16b_benessere_full_multilocale_lifecycle_end_to_end`. Removes wellness-out guard half of the DUAL-OUT GUARD planted in A.16 Salute · **family-out guard PRESERVED** for A.16c closer. **Operational clarification added to D-098 (no new D-number)**: (i) **Guard removal sub-recipe extended from 1-removal (2-phase) to 2-removal (3-phase) phase**. Previous 4 guard removal precedents (Villa-out A.12b · Pixel-out A.13b · Brace-out A.14b · Luxe-out A.15b) all applied to 2-phase staged progressions where 1 opener planted 1 guard and 1 closer removed it. A.16 Salute planted 2 guards (wellness-out + family-out) in a 3-phase progression · A.16b removes wellness-out symmetrically via `test_a16b_benessere_out_guard_was_removed_from_salute_tests` (5th precedent) while family-out guard is PRESERVED unchanged at all 3 layers (registration-time + lifecycle start + lifecycle end) · A.16c will remove family-out via 6th precedent. **First time 1 opener plants 2 guards and each closure phase removes 1 of them** — sub-recipe generalizes from 'one-opener-one-closer' (A.12→A.12b topology) to 'one-opener-two-closers' (A.16→A.16b→A.16c topology). D-098 invariant preserved throughout: one lifecycle test per enrolled template regardless of phase count. (ii) **Scheduler-state bool flags + nested list-of-str uniformly OUT across 2 calendar lists**. Benessere ships `home.calendar` + `prenota.calendar` (both 7 rows × 6 cols) as identical shape · both calendars share `day`/`num`/`month` IN (editorial visible labels) and `has_slots`/`soldout`/`slots` OUT (scheduler-state-like). 4 bool flag cols (2 lists × 2 cols) OUT col-level per Luxe `available` + Salute `is_popular` precedent re-application. 2 nested list-of-str cols (`slots`: concrete time-slots like `['10:00','14:30']`) OUT col-level per Juris precedent. Uniform enforcement across both calendars confirms **scheduler-state is a per-col-type policy category, not a per-archetype special case** — same OUT decision applies regardless of which calendar list the flag/slot appears on, as long as the col-type matches. This is mechanical reuse of 3 prior precedents (Luxe `available` + Salute `is_popular` + Juris nested-list-of-str) re-applied to the same content-tree shape twice. (iii) **First-ever NOVEL SHAPE DEFERRAL outcome formalized**: `home.ambients` tuple-with-image (4 × `(image_url, title, sub)` · positional tuple with image as first element) discovered during Step 0 audit · **ZERO precedent in any existing archetype** (all prior tuple shapes across 15 enrolled archetypes carry text-only cols). Adding image-typed cell inside a tuple col would be a first-ever infrastructure exercise requiring runtime verification of the 3 editor layers' tuple-cell-with-image-type handling (`services._resolve_path` for save · `schema._iter_indexed_groups` for sidebar rendering · `rendering._apply_indexed` for public preview merge). Decision: whole list OUT first-wave via schema omission · mechanical-reuse principle preserved · no horizontal-feature introduction · 4-tile image coverage loss accepted for middle-phase enrollment · explicit future expansion candidate after dedicated infra verification (could become mini-phase after A.16c closure or deferred indefinitely). **Establishes a 4th deferral category** (novel-shape-deferred) alongside mutable repeater · image per-locale · detail-page editing. Contract test `test_a16b_benessere_complex_shapes_excluded_from_perimeter` blindates `home.ambients` + `.0` + `.0.0` as rejected via `validate_key_path` — schema omission is enforced, not just a decision on paper. (iv) **Stringent IN col-level audit extended to 6 archetypes**. Precedent chain: Sapore (A.14) → Brace (A.14b) → Bottega (A.15) → Luxe (A.15b) → Salute (A.16) → **Benessere (A.16b · calendar day/num/month + journey.num + pillars.init + packages.tag)**. Audit-driven · non-inertial: fields that look structural but are customer-facing editorial content must stay IN. Benessere IN: `home.calendar[].day`/`num`/`month` + `prenota.calendar[].day`/`num`/`month` (visible calendar labels · customer edits 'Lun'/'14'/'Apr' editorial text · not scheduler state) · `home.journey[].num` ('01'/'02'/'03'/'04' visible step numbering) · `filosofia.pillars[].init` ('A'/'B'/'C' editorial typographic device) · `rituali.packages[].tag` ('Giornata singola' · 'Tre giorni' editorial category badges). Benessere OUT (truly scheduler/structural): 4× bool flags (`has_slots`/`soldout`) + 2× nested str-list (`slots`) + includes/tags (nested str-list · Juris) + interest_options (nested str-list) + form structures + pages/posts. Dedicated contract tests `test_a16b_benessere_visible_catalog_fields_kept_in` + `test_a16b_benessere_calendar_bool_flags_excluded` + `test_a16b_benessere_nested_str_lists_excluded` + `test_a16b_benessere_complex_shapes_excluded_from_perimeter` blindate all 4 decision categories. (v) **Boundary editor-vs-clinic-admin re-verified on second medical-other archetype**. Benessere preserves the same architectural boundary validated Step-0 in A.16 Salute: `apps/catalog/views.py::LiveTemplateView` still does NOT import from `scheduler.`/`booking.`/`patient.`/`scheduler.`/`calendar_slot.`/`medical_record.`/`prescription.`/`appointment.` namespaces. Light guardrail test `test_a16b_benessere_clinic_admin_boundary` mirrors A.16 Salute contract (same 7-prefix smoke check). Second medical-other archetype confirms boundary holds uniformly across the family, not a Salute-special case. Same pattern as A.15/A.15b ecommerce commerce-admin boundary (re-verified across 2 archetypes). (vi) **Outside-gate fixture rotation to `family`/`famiglia-pediatria`** — pre-planted signal for A.16c closer. 6 fixture rotated as standard closure sub-recipe. 15 contract tests + 1 lifecycle test. Schema LOC delta +559 — **sotto** the 600-LOC soft guardrail (consistent with Bottega 585 · Luxe 598 · Salute 512 · mid-complexity range · zero deep-path · 17 shape entries). Zero service-layer / rendering / editor-widget / `apps/commerce/` changes required — pure 3-file enrollment surface confirmed as sub-recipe standard after **6 consecutive staged enrollments**. Benessere brings the catalog to 16 archetype slugs enrolled · 16 multi-locale enrolled · 17 templates editable end-to-end · 6 families closed + medical-other half-open with 1 template pending (Famiglia A.16c closer). Baseline tip `e9cc419`. No new binding introduced.

- **2026-04-19 · Session 75 · A.16c** — Seventeenth enrollment: `family` (Famiglia · medical-other family · third template · CLOSER phase of 3-phase staged progression) with `test_a16c_family_full_multilocale_lifecycle_end_to_end`. **CLOSES the medical-other family** · removes family-out guard residuo from A.16 Salute tests (6th precedent of guard removal pattern · completes the 2-removal-phase sub-recipe variant established in A.16b). **5th staged dedicated-schema family closure** (real-estate + portfolio + restaurant-continuation + ecommerce + medical-other). **Operational clarification added to D-098 (no new D-number)**: (i) **2-removal-phase sub-recipe variant COMPLETED**. First 3-template staged progression completes: A.16 Salute planted DUAL-OUT GUARD for wellness + family · A.16b removed wellness-out (5th precedent) · A.16c removes family-out (6th precedent) via `test_a16c_family_out_guard_was_removed_from_salute_tests`. First "one-opener-two-closers" topology fully closed (vs prior "one-opener-one-closer" 2-phase topologies · Villa/Pixel/Brace/Luxe). Sub-recipe now supports both variants · D-098 invariant preserved throughout: one lifecycle test per enrolled template regardless of phase count. Same-phase ratio: 6 removals across 17 enrolled archetypes (no removals for shared-schema or sequential dedicated-schema topologies · only staged). (ii) **Deep-path Sapore precedent cross-pattern confirmed on 3rd archetype**. `crescita.topics[].items` is tuple-in-dict-list parent (4 sections × 4 tuples × 2 cols = 32 cells) · structurally identical to Sapore `menu.sections.{i}.dishes` (5 sections × 7-6-5 tuples × 3 cols). Registered as 4 separate STRUCTURED_FIELD_SHAPES entries mirroring Sapore's 5-entry pattern. **f66ac24 render-side contract-alignment fix (A.14 Step 2) already covers the path walk** · zero new infrastructure required. Cross-pattern precedent chain now: Sapore (A.14 · tuple-in-dict-list · first · f66ac24 fix committed) → Brace (A.14b · dict-in-dict-list parent via list-index · cross-pattern validation) → **Famiglia (A.16c · tuple-in-dict-list mirror of Sapore · 3rd precedent · f66ac24 mechanical reuse)**. Browser walk verified end-to-end: deep-path override `crescita.topics.0.items.0.q` edits only the target cell · sibling `.a` + sibling `.items.1.q` + sibling next topic `.topics.1.items.0.q` + sibling farthest `.topics.3.items.3.q` + parent scalars `.topics.0.meta`/`.title` all UNCHANGED in both draft (editor iframe) and published (public preview) states. (iii) **Deep-path cells are plain-key globals per D-098 structured-list policy · uniform across 17 enrolled archetypes**. Discovery during lifecycle test writing: deep-path text cells like `crescita.topics.0.items.0.q` classify as structured-list cells under `_is_under_structured_list()` · therefore non-translatable · stored as plain-key globals (mechanical reuse of Sapore precedent verified via browser walk rendering universally on all 5 locales). This confirms that EVERY indexed-list cell (image or text · parent or deep-path) is global across all 17 enrolled archetypes · D-098 binding consistency. (iv) **Stringent IN col-level audit extended to 7 archetypes**. Precedent chain: Sapore (A.14) → Brace (A.14b) → Bottega (A.15) → Luxe (A.15b) → Salute (A.16) → Benessere (A.16b) → **Famiglia (A.16c · meta editorial area labels + icon Bootstrap text-refs + tag + exp_label/exp_value + wa_label + age range labels)**. Famiglia IN: `crescita.topics[].meta` ('Area 01') · `crescita.topics[].icon` ('apple'/'moon'/'vaccine'/'comp' Bootstrap text refs · NOT raw SVG unlike Salute) · `pediatre.doctors[].tag` ('Fondatrice') · `pediatre.doctors[].exp_label`/`exp_value` · `home.doctors[].wa_label` · `home.age_groups[].range` + `home.journey_steps[].age` editorial range labels. Famiglia OUT (Juris precedent): `home.age_groups[].items` + `pediatre.doctors[].specs` nested list-of-str · `crescita.topics[].items` parent col (IN via 4 sub-path entries). Zero raw SVG (unlike Salute · 0 icon_svg fields) · zero bool flags (unlike Benessere · no scheduler-state). (v) **Novel col name `src` on home.gallery image-in-dict-row · mechanical reuse via col registration**. First precedent of col name `src` (vs prior `image`/`portrait`/`avatar` across 16 enrolled archetypes) · zero infra impact · schema just registers col with `type: "image"`. Extends image col naming flexibility · no code path depends on a specific col name. (vi) **6 RTL rules sufficient for `.fm-*` skin base layout**. Famiglia has the lowest RTL rule count of all 17 enrolled archetypes (vs Salute 18 / Benessere 13 / most others 18-31). Browser walk FOCUS A verified visual coherence: body/nav/hero/topics section all computed `direction: rtl` · text-align: `start` flips correctly via html[dir="rtl"] propagation · 16 `<details>` elements on crescita page render correctly in AR · 5420/4537 Arabic chars in home/crescita public preview. Rule count is a minimum bar · the skin's base CSS inherits RTL cleanly from `html[dir="rtl"]` · lightweight targeted overrides handle layout flips without a large ruleset. Boundary editor-vs-clinic-admin re-verified on 3rd medical-other archetype (light smoke test `test_a16c_family_clinic_admin_boundary` mirrors A.16/A.16b 7-prefix check). 15 contract tests + 1 lifecycle test. Schema LOC delta **+600** — right AT the 600-LOC soft guardrail (vs Sapore +663 precedent for similar deep-path shape · Brace +826 · leaner than both). Zero service-layer / rendering / editor-widget / `apps/commerce/` changes required — pure 3-file enrollment surface confirmed as sub-recipe standard after **7 consecutive staged enrollments**. Famiglia brings the catalog to 17 archetype slugs enrolled · 17 multi-locale enrolled · 18 templates editable end-to-end · **7 families closed** + zero half-open. Baseline tip `761044c`. No new binding introduced.
