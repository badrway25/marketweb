...
Last updated: 2026-04-20 — after **Session 76 A.17 Aura (agency-digital-studio · agency-secondary family · SINGLE-TEMPLATE CLOSER) Editor + Multi-locale Enrollment merge · CLOSES AGENCY-SECONDARY FAMILY** (baseline tip `2b0d11e`, pushed to origin)

## Current state — read this before opening any new workstream (2026-04-20)

**Eighteen archetype slugs enrolled** — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) + **editorial-designer-grid** (Chiara) + **cinematic-photographer** (Pixel) + **trattoria-warm** (Sapore) + **street-modern** (Brace) + **artisan-workshop** (Bottega) + **fashion-editorial** (Luxe) + **clinic** (Salute) + **wellness** (Benessere) + **family** (Famiglia) + **agency-digital-studio** (Aura) — all multi-locale enrolled. **19 templates editable end-to-end.** A.17 Aura enters as SINGLE-TEMPLATE CLOSER of the agency-secondary family (one phase · first single-template family closure precedent · Vertex already on agency-creative-studio primary). **EIGHT families editor-complete**: law (A.10+A.11 sequential dedicated-schema) · medical-specialist (A.9 shared-schema) · real-estate (A.12+A.12b staged dedicated-schema) · portfolio (A.13+A.13b staged dedicated-schema) · restaurant-continuation (A.14+A.14b staged dedicated-schema) · ecommerce (A.15+A.15b staged dedicated-schema) · medical-other (A.16+A.16b+A.16c staged dedicated-schema · first 3-phase variant) · **agency-secondary (A.17 single-template dedicated-schema · first single-template variant)**. **Zero half-open families remain**.

A.17 validates four critical framings:

1. **Single-template dedicated-schema variant formalized as first-ever topology**. D-098 topology distribution now: 1 shared-schema (A.9 specialist) · 1 sequential dedicated-schema (A.10+A.11 law) · 5 staged dedicated-schema (real-estate · portfolio · restaurant-continuation · ecommerce · medical-other) · **1 single-template dedicated-schema (A.17 agency-secondary)** · Total 8 closed families. Aura's family has 1 template so shared-schema is irrelevant and staged progression inapplicable · single-phase closure is the minimum unit of a family closure · zero guard-planting / guard-removal sub-recipe required. Brings total closed families to 8.

2. **brief.slots OUT entire explicit · 5th form-structure precedent**. Aura's `brief` page is the 5th archetype to carry pseudo-admin form scaffolding after Gusto/Juris/Casa/Villa. brief.slots is tuple `(id, label)` where `id="mon10"`/`"async"` is a form-option-value (structural form-schema requiring calendar integration · not a customer copy edit) and label is editorially visible. **Decision: OUT entire list despite editorial label visibility** — list identity (which days/hours offered) is a form-schema concern. Dedicated contract test `test_a17_aura_brief_slots_kept_out_explicit` blindates brief.slots + 5 sub-paths + sibling step1/2/3/labels/placeholders/scope_options all rejected by validate_key_path.

3. **7th uniform enforcement of per-item content policy · posts registry-only**. Aura `posts[]` ships 6 project_detail records (served via `/lavori/<slug>/` kind-conversion in LiveTemplateView) including 6 cover_image surfaces. Detail-page editing stays OUT of scope consistent with 6 prior enforcements (Chiara A.13 · Pixel A.13b · Sapore A.14 · Brace A.14b · Bottega A.15 · Luxe A.15b). Dedicated contract test `test_a17_aura_posts_stay_registry_only` blindates 12 posts paths rejected by validate_key_path + autosave POST on 5 posts paths rejects 400. Playwright verified posts absent from sidebar (0 inputs + 0 labels + Cmd+K search `posts` → 0 risultati).

4. **Outside-gate fixture rotation completes a 7-precedent chain · last non-enrolled archetype**. 7th precedent of guard-removal / fixture rotation: Villa-out A.12b → Pixel-out A.13b → Brace-out A.14b → Luxe-out A.15b → Wellness-out A.16b → Family-out A.16c → **Aura-out A.17**. Rotation target: `startup-saas-landing`/`elevate-startup-landing` — the LAST non-enrolled archetype. Post-A.17 outside-gate pattern faces inflection: A.17b will close catalog to 20/20 editable leaving no non-enrolled archetype as fixture. A.17b Step 0 will address gate retirement / null-slug sentinel / synthetic never-enrolled reference. Playwright validated UI gate: `/projects/start/?template=elevate-startup-landing` redirects 302 to catalog detail page (NOT editor).

Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **18 archetype slugs enrolled · 18 multi-locale enrolled · 19 templates editable end-to-end**. Note: "18 enrolled" does NOT mean "every catalog archetype covered" — the DNA registry carries 1 residual slug (`startup-saas-landing` · Elevate) that remains editor-unsupported (1 single-template family still open · zero half-open · A.17b closer). All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → **357/357 PASS**
- `python smoke_full.py` → 834/834 routes HTTP 200

Baseline `phase-integration-baseline-v15` tip: **`2b0d11e`** (A.17 merge), pushed to `origin/phase-integration-baseline-v15`.
