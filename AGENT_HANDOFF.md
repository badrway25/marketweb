...
Last updated: 2026-04-19 — after **Session 75 A.16c Famiglia (family · medical-other family · third template · CLOSER PHASE) Editor + Multi-locale Enrollment merge · CLOSES MEDICAL-OTHER FAMILY** (baseline tip `761044c`, pushed to origin)

## Current state — read this before opening any new workstream (2026-04-19)

**Seventeen archetype slugs enrolled** — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) + **editorial-designer-grid** (Chiara) + **cinematic-photographer** (Pixel) + **trattoria-warm** (Sapore) + **street-modern** (Brace) + **artisan-workshop** (Bottega) + **fashion-editorial** (Luxe) + **clinic** (Salute) + **wellness** (Benessere) + **family** (Famiglia) — all multi-locale enrolled. **18 templates editable end-to-end.** A.16c Famiglia enters as CLOSER phase of the medical-other 3-phase staged dedicated-schema progression (A.16 Salute opener · A.16b Benessere middle · A.16c Famiglia closer · **medical-other family CLOSED**). Family-out guard half of the DUAL-OUT GUARD planted in A.16 Salute removed symmetrically (6th precedent of guard removal pattern · completes the 2-removal-phase sub-recipe variant established in A.16b). **SEVEN families editor-complete**: law (A.10+A.11 sequential dedicated-schema) · medical-specialist (A.9 shared-schema) · real-estate (A.12+A.12b staged dedicated-schema) · portfolio (A.13+A.13b staged dedicated-schema) · restaurant-continuation (A.14+A.14b staged dedicated-schema) · ecommerce (A.15+A.15b staged dedicated-schema) · **medical-other (A.16+A.16b+A.16c staged dedicated-schema · first 3-phase variant)**. **Zero half-open families remain**.

A.16c validates three critical framings:

1. **2-removal-phase sub-recipe variant COMPLETED · first 3-template staged progression closed**. A.16 Salute planted DUAL-OUT GUARD for both `wellness` + `family` · A.16b removed wellness-out (5th precedent) · A.16c removes family-out (6th precedent) via `test_a16c_family_out_guard_was_removed_from_salute_tests`. First time a 3-phase staged progression completes: "one-opener-two-closers" topology closure complete (vs prior "one-opener-one-closer" 2-phase topologies · Villa/Pixel/Brace/Luxe). D-098 invariant preserved throughout: one lifecycle test per enrolled template regardless of phase count. **5th staged dedicated-schema family closure** brings total closed families to 7.

2. **Deep-path Sapore precedent re-used mechanically · zero infra needed (3rd archetype confirmation)**. `crescita.topics[].items` is tuple-in-dict-list parent (4 sections × 4 tuples × 2 cols = 32 cells) · identical shape to Sapore's `menu.sections.{i}.dishes`. Registered as 4 separate STRUCTURED_FIELD_SHAPES entries mirroring Sapore's 5-entry pattern. **f66ac24 render-side contract-alignment fix already covers the path walk** · zero new infrastructure required. Cross-pattern precedent chain: Sapore A.14 (tuple-in-dict-list · first) → Brace A.14b (dict-in-dict-list via list-index) → **Famiglia A.16c (tuple-in-dict-list mirror of Sapore · 3rd precedent · mechanical reuse)**. Browser walk verified end-to-end: deep-path override `crescita.topics.0.items.0.q` edits only the target cell · sibling `.a` + sibling `.items.1.q` + sibling next topic + sibling farthest cell + parent scalars ALL UNCHANGED in both draft (editor iframe) and published (public preview) states · surgical precision across 32 deep-path cells.

3. **Deep-path cells are plain-key globals per D-098 structured-list policy · uniform across 17 enrolled archetypes**. Discovery during lifecycle test writing: deep-path text cells classify as structured-list cells under `_is_under_structured_list()` · therefore non-translatable · stored as plain-key globals. This confirms that EVERY indexed-list cell (image or text · parent or deep-path) is global across all 17 enrolled archetypes. Verified via browser walk: override renders universally on all 5 locales on crescita FAQ page.

Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **17 archetype slugs enrolled · 17 multi-locale enrolled · 18 templates editable end-to-end**. Note: "17 enrolled" does NOT mean "every catalog archetype covered" — the DNA registry carries 2 residual slugs (`agency-digital-studio`, `startup-saas-landing`) that remain editor-unsupported (2 single-template families · zero half-open). All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → **341/341 PASS**
- `python smoke_full.py` → 834/834 routes HTTP 200

Baseline `phase-integration-baseline-v15` tip: **`761044c`** (A.16c merge), pushed to `origin/phase-integration-baseline-v15`.
