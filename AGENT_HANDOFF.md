...
Last updated: 2026-04-18 — after **Session 66 A.12b Villa (ultra-luxury-cinematic · real-estate family) Editor + Multi-locale Enrollment merge** (baseline tip `258fa56`, pushed to origin)

## Current state — read this before opening any new workstream (2026-04-18)

Eight archetype slugs enrolled — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) — all multi-locale enrolled. **9 templates editable end-to-end.** The real-estate family (Casa + Villa) closed in A.12b, becoming the third family-closure after law (A.10+A.11 sequential dedicated-schema) and medical-specialist (A.9 shared-schema). All three D-098 family-progression topologies now have ≥1 real precedent: shared-schema single-phase · sequential dedicated-schema · staged dedicated-schema. A.12b also confirmed **image-inside-list-of-dict-row is a reusable enrollment pattern** (not a horizontal feature) — Vertex has exposed it since A.3a/A.4; Villa became the second archetype to use it, with zero service-layer / rendering / widget changes required. Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **8 archetype slugs enrolled · 8 multi-locale enrolled · 9 templates editable end-to-end**. Note: "8 enrolled" does NOT mean "every catalog archetype covered" — the DNA registry carries additional slugs (editorial-designer-grid, cinematic-photographer, trattoria-warm, street-modern, medical-clinic, wellness-serene, family-warm, agency-digital-studio, startup-saas-landing, artisan-workshop, fashion-editorial) that remain editor-unsupported. All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → 216/216 PASS
- `python smoke_full.py` → 834/834 routes HTTP 200

Baseline `phase-integration-baseline-v15` tip: **`258fa56`** (A.12b merge), pushed to `origin/phase-integration-baseline-v15`.
...
### Phase A.13+ — candidates (no commitment yet)

Pick one when the next workstream opens. Recommended framing: **A.13 portfolio family (Chiara + Pixel) Step-0 planning session** — closes another of the 8 MVP categories. Chiara ships `editorial-designer-grid` with novel `project_detail` / `series_detail` page kinds (Session 34 rollout); Pixel ships `cinematic-photographer` with imagery-heavy single-template shape. Step-0 audit decides (a) shared-schema vs staged dedicated-schema (likely the latter), and (b) whether `project_detail` / `series_detail` page-scoped fields enter the perimeter (probably out — detail editing stays registry-only consistent with every prior family closure's policy on Lex notabili / Juris insights / Casa posts / Villa posts). If the project_detail analysis reveals complexity that scope-creeps the phase, pivot to ecommerce (Bottega + Luxe) or restaurant-continuation (Sapore + Brace) as fallback families. Alternatives: medical-other (Salute + Benessere + Famiglia, three separate phases), Aura individual enrollment, Elevate individual enrollment, editor polish / operator tools / remote storage (defer unless customer signal). MEMORY.md maintenance mini-phase is also queued as a separate housekeeping task (not bundled with an enrollment).
