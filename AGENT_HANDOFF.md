...
Last updated: 2026-04-19 — after **Session 72 A.15b Luxe (fashion-editorial · ecommerce family · second template) Editor + Multi-locale Enrollment merge · CLOSES ECOMMERCE FAMILY** (baseline tip `bd9ffcb`, pushed to origin)

## Current state — read this before opening any new workstream (2026-04-19)

**Fourteen archetype slugs enrolled** — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) + **editorial-designer-grid** (Chiara) + **cinematic-photographer** (Pixel) + **trattoria-warm** (Sapore) + **street-modern** (Brace) + **artisan-workshop** (Bottega) + **fashion-editorial** (Luxe) — all multi-locale enrolled. **15 templates editable end-to-end.** A.15b closes the ecommerce family via staged dedicated-schema progression — **fourth consecutive staged closure** after real-estate (A.12+A.12b) · portfolio (A.13+A.13b) · restaurant-continuation (A.14+A.14b). **Six families editor-complete**: law (A.10+A.11 sequential dedicated-schema) · medical-specialist (A.9 shared-schema) · real-estate (A.12+A.12b staged dedicated-schema) · portfolio (A.13+A.13b staged dedicated-schema) · restaurant-continuation (A.14+A.14b staged dedicated-schema) · **ecommerce (A.15+A.15b staged dedicated-schema)**. No half-open families remain.

A.15b validates three critical framings:

1. **Editor edits SHOWCASE / template-content, NOT commerce-admin state · re-verified on second ecommerce archetype**. Boundary runtime-preserved: `apps/catalog/views.py::LiveTemplateView` still does NOT import from `apps.commerce` · Luxe's template_content (`collezione` catalog listings · single product demo · lookbook editorial · maison about) is registry-driven · real catalog state stays in `apps/commerce/` via seller dashboard Phase 3a/3b. Light guardrail test `test_a15b_luxe_commerce_admin_boundary` mirrors A.15 Bottega's contract. Second ecommerce archetype confirms boundary holds uniformly, not a Bottega-special case.

2. **Rendered vs storage-only image widget distinction is PER-SKIN, not per-schema**. Bottega ships 22 image widgets = 16 rendered + 6 storage-only (typographic-first DNA · crest-mark glyphs replace portraits). Luxe ships **31 image widgets ALL RENDERED** — photographically editorial campaign-driven DNA paints every surface (1 scalar top + 2 nested-dict scalar + 28 image-in-dict-row cells). The schema exposes image fields identically in both cases (same `type: "image"` · same STRUCTURED_FIELD_SHAPES col · same plain-key per-locale storage). Only skin rendering differs. This confirms storage-only vs rendered is a **skin-level concern** (whether HTML paints the field), NOT a schema-level concern (whether the field is editable).

3. **Stringent IN col-level audit extended to drop/n/tag · 4th archetype**. Sapore forno.pizza_signatures.n (A.14) → Brace ordina routes n (A.14b) → Bottega shop.products n/edition/tag (A.15) → **Luxe collezione.products drop/n/tag (A.15b)**. Audit-driven, non-inertial: fields that LOOK structural but are customer-facing editorial content (drop badges · Look numbering · availability tags like 'Lista d'attesa' · 'Capsula' · 'Archivio') stay IN; id/available stay OUT. Dedicated contract tests blindate both IN and OUT calls per archetype.

Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **14 archetype slugs enrolled · 14 multi-locale enrolled · 15 templates editable end-to-end**. Note: "14 enrolled" does NOT mean "every catalog archetype covered" — the DNA registry carries 5 residual slugs (`clinic`, `wellness`, `family`, `agency-digital-studio`, `startup-saas-landing`) that remain editor-unsupported (5 templates across 3 open families). All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → **295/295 PASS**
- `python smoke_full.py` → 834/834 routes HTTP 200

Baseline `phase-integration-baseline-v15` tip: **`bd9ffcb`** (A.15b merge), pushed to `origin/phase-integration-baseline-v15`.
...
### Phase A.16+ — candidates (ecommerce CLOSED · three single-template families residue)

Pick one when the next workstream opens. A.15b closes the ecommerce family. No half-open families remain. Residue is 5 editor-only-pending templates across 3 families:
- **medical-other** (3 templates · `clinic` Salute + `wellness` Benessere + `family` Famiglia · 5-locale parity PERFECT since Sessions 51/52)
- **agency-secondary** (1 template · `agency-digital-studio` Aura)
- **startup-saas** (1 template · `startup-saas-landing` Elevate)

Recommended framing: **A.16 Salute (`clinic`) Step-0 planning session** — first of the medical-other family · opens a 3-template staged progression (A.16 Salute + A.16b Benessere + A.16c Famiglia) · outside-gate fixture already rotated to `clinic`/`salute-studio-medico` in A.15b as next candidate signal. Precedent match: A.10 Lex (single-template inside a multi-template category) — Benessere + Famiglia stay OUT via Salute-out dual guard planted Step 1 · removed in A.16b/A.16c. Shared-schema unlikely across all 3 medical-other (divergent DNA: booking-widget vs calendar-spot vs phone-and-chat) — Step-0 runtime audit decides.

Alternatives: Aura individual enrollment (closes agency-secondary · single template · low novelty) · Elevate individual enrollment (closes startup-saas · single template · low novelty) · editor polish / operator tools / remote storage (defer unless customer signal). MEMORY.md maintenance mini-phase remains queued as a separate housekeeping task (not bundled with an enrollment · auto-memory index ~32KB sopra warning-soglia 24.4KB · grows ~1KB per enrollment phase · 15 phases A.6→A.15b accumulated).
