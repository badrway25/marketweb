...
Last updated: 2026-04-18 — after **Session 67 A.13 Chiara (editorial-designer-grid · portfolio family) Editor + Multi-locale Enrollment merge** (baseline tip `e55d55b`, pushed to origin)

## Current state — read this before opening any new workstream (2026-04-18)

Nine archetype slugs enrolled — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) + **editorial-designer-grid** (Chiara) — all multi-locale enrolled. **10 templates editable end-to-end.** A.13 opens the portfolio family with Chiara, leaving Pixel (`cinematic-photographer`) explicitly held out until A.13b — same staged dedicated-schema progression pattern as real-estate (A.12+A.12b). Three families editor-complete: law (A.10+A.11 sequential dedicated-schema) · medical-specialist (A.9 shared-schema) · real-estate (A.12+A.12b staged dedicated-schema). A.13 also confirmed **deep-path image-in-dict-row works at arbitrary depth** (Chiara `home.featured_works.items[].image` at path 2 levels deep through parent dict · `_resolve_path` walks any depth · zero infrastructure work) and **the detail-page registry-only policy is a coherent perimeter decision**, not an accidental limitation — 5 uniform enforcements now across Lex/Juris/Casa/Villa/Chiara per-item content. Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **9 archetype slugs enrolled · 9 multi-locale enrolled · 10 templates editable end-to-end**. Note: "9 enrolled" does NOT mean "every catalog archetype covered" — the DNA registry carries additional slugs (cinematic-photographer, trattoria-warm, street-modern, medical-clinic, wellness-serene, family-warm, agency-digital-studio, startup-saas-landing, artisan-workshop, fashion-editorial) that remain editor-unsupported. All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → 227/227 PASS
- `python smoke_full.py` → 834/834 routes HTTP 200

Baseline `phase-integration-baseline-v15` tip: **`e55d55b`** (A.13 merge), pushed to `origin/phase-integration-baseline-v15`.
...
### Phase A.13b+ — candidates (no commitment yet)

Pick one when the next workstream opens. Recommended framing: **A.13b Pixel (cinematic-photographer) enrollment — close the portfolio family** — natural continuation from A.13, mirrors A.12→A.12b real-estate closure. Pixel has 2 novel page kinds (`series_list` + `publications` vs Chiara's 1 `process`) but simpler shape overall: mostly list-of-tuple, 1 scalar `home.hero_image`, no image-in-dict-row in first wave (posts stays registry-only per consistent policy including `posts[].cover_image`). Lighter than Chiara · recipe mechanical reuse. Expected schema LOC comfortably under 500. Alternatives: restaurant-continuation (Sapore + Brace), ecommerce (Bottega + Luxe · with Step-0 check on commerce-foundation overlap), medical-other (Salute/Benessere/Famiglia · three separate phases), Aura individual enrollment, Elevate individual enrollment, editor polish / operator tools / remote storage (defer unless customer signal). MEMORY.md maintenance mini-phase remains queued as a separate housekeeping task (not bundled with an enrollment).
