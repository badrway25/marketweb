...
- `/templates/business/fiscus-commercialista/` — detail page with cluster "Servizi finanziari" + style "Dashboard light" + tier "Standard" + 3 feature chips + 2 use-case chips + Personalizza CTA + related block (14 items).
- `/templates/business/fiscus-commercialista/preview/` — IT live preview with voice anchor hero "L'adempimento corretto, non la trovata." + Primo appuntamento CTA + fiscal content throughout. No Pragma leak. No horizontal overflow.
- `/templates/business/fiscus-commercialista/preview/?lang=<en|fr|es|ar>` — 4 locale previews. AR with `<html dir="rtl" lang="ar">` and Latin digits + Latin proper names + Italian normative refs preserved with parenthetical Arabic clarification on first mention.
- `/templates/?cluster=financial-services` → Fiscus card listed.
- `/templates/search/typeahead/?q=commercialista` → Fiscus in templates pool · financial-services + notary-commercialista in clusters pool.
- Homepage `/` — Fiscus correctly NOT shown in featured pool (featured=False respected).
- smoke_full 892/892 HTTP 200 across catalog + marketplace + preview + post detail routes.
...
**Wave 2 Pilot #2 · `solaria-coaching`** per D-102 cadence. Open branch `phase-x4-wave2-solaria-coaching-v1` from the merged X.4 tip (AFTER Pilot #1 merges). Consume `cluster_blueprints/coaching.md` + `imagery/packs/coaching.md` (both drafted in X.3). Commit 1 infrastructure + IT tree at draft · Commit 2 parallel sub-agent locales + flip + cascade absorption.

### Follow-up work that is NOT on the table

- No new archetypes (D-099 binding).
- No `apps/editor` / `apps/projects` / `apps/commerce` / `models.py` / `migrations` / `selectors.py` / `views.py` / `urls.py` / `docs/content-factory` / `live_templates` structural edits during Wave 2 pilots (D-102 scope binding).
- No Wave 2 template merged without: 5 locales · Playwright MCP 8-point walk green · test cascade absorbed · smoke coverage extended · `sync_template_tiers` applied.
...
**Read this first before opening any new workstream.** The editor enrollment program A.6 → A.17b is **complete**. The baseline is **full**. No further archetype enrollment is on the roadmap.

- **19 archetype slugs enrolled · 19 multi-locale enrolled · 20 templates editable end-to-end · 20/20 catalog editable · 9/9 families closed · zero half-open · zero non-enrolled real archetypes.**
- **Outside-gate pattern RETIRED via synthetic sentinel (D-099)** · the old "fixture = real non-enrolled archetype" pattern is no longer applicable because the catalog is 20/20. The sentinel `__unsupported_archetype_sentinel__` replaces it for helper-level negative assertions only. No fake WebTemplate was introduced. No helper signature refactor. See D-099 in `DECISIONS.md`.
- **D-098 topology distribution FINAL** · 1 shared-schema (A.9 specialist) + 1 sequential dedicated-schema (A.10+A.11 law) + 5 staged dedicated-schema (real-estate · portfolio · restaurant-continuation · ecommerce · medical-other · including first 3-phase variant in medical-other) + **2 single-template dedicated-schema (A.17 agency-secondary + A.17b startup-saas)** = 9 closed families.
- **Any future workstream sits in maintenance / polish / signal-gated widening**. The primary candidate is the MEMORY.md maintenance mini-phase (auto-memory index ~35-36KB · can be shortened). See TODO_NEXT.md for the full list.

### Follow-up work the handoff recipient can safely open
- MEMORY.md maintenance mini-phase (housekeeping · separate from any enrollment)
- `home.ambients` tuple-with-image widening (A.16b deferral · customer-signal-gated)
- Operator / remote-storage / polish — only if a real signal surfaces

### Follow-up work that is NOT on the table
- No `A.18 template enrollment` · the program is closed
- No re-introduction of a real non-enrolled archetype as test fixture · D-099 binding
- No unilateral lift of detail-page editing · horizontal feature · needs its own phase
- No opening of product-admin / billing / Stripe-customer-editing surfaces · D-099/Elevate boundary

## Current state — full enrolled baseline (2026-04-20)

**Nineteen archetype slugs enrolled** — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) + **editorial-designer-grid** (Chiara) + **cinematic-photographer** (Pixel) + **trattoria-warm** (Sapore) + **street-modern** (Brace) + **artisan-workshop** (Bottega) + **fashion-editorial** (Luxe) + **clinic** (Salute) + **wellness** (Benessere) + **family** (Famiglia) + **agency-digital-studio** (Aura) + **startup-saas-landing** (Elevate) — all multi-locale enrolled. **20 templates editable end-to-end.** **NINE families editor-complete**: law (A.10+A.11 sequential dedicated-schema) · medical-specialist (A.9 shared-schema) · real-estate (A.12+A.12b staged dedicated-schema) · portfolio (A.13+A.13b staged dedicated-schema) · restaurant-continuation (A.14+A.14b staged dedicated-schema) · ecommerce (A.15+A.15b staged dedicated-schema) · medical-other (A.16+A.16b+A.16c staged dedicated-schema · first 3-phase variant) · agency-secondary (A.17 single-template dedicated-schema) · **startup-saas (A.17b single-template dedicated-schema · 2nd precedent · CLOSES the program)**.

A.17b validates four critical framings:

1. **Editor enrollment program CLOSED · D-098 topology distribution FINAL.** 1 shared-schema + 1 sequential dedicated-schema + 5 staged dedicated-schema + 2 single-template dedicated-schema = 9 closed families. Catalog 20/20 editable. No further topology variant is possible on the current catalog.

2. **Outside-gate retirement via synthetic sentinel · D-099 new binding.** Old pattern retired. Sentinel `__unsupported_archetype_sentinel__` used ONLY for helper-level negative assertions in tests. 8 prior call-sites rotated from `"startup-saas-landing"` → sentinel. Test renamed/converted/deleted: `test_a17_aura_out_guard_was_removed_from_prior_tests` → `test_a17b_elevate_enrollment_and_outside_gate_retirement` · `test_unsupported_archetype_raises` → `test_every_enrolled_archetype_creates_project` · `test_customize_start_unsupported_archetype_redirects_to_detail` DELETED as post-20/20 redundant (unknown-template bounce still covered by `test_customize_start_unknown_template_redirects_to_catalog`). New `test_a17b_all_archetypes_enrolled_catalog_complete` as FINAL-PROGRAM-CLOSURE guardrail. UI-side verified: `/projects/start/?template=elevate-startup-landing` → editor · `/projects/start/?template=does-not-exist` → /templates/ bounce · no real slug is treated as "unsupported".

3. **Editor-vs-product-admin boundary · 3rd category-class precedent.** After editor-vs-commerce-admin (Bottega/Luxe A.15/A.15b) + editor-vs-clinic-admin (Salute/Benessere/Famiglia A.16/A.16b/A.16c) · Elevate adds editor-vs-product-admin. Marketing-editorial copy (prices, tier names, module numbers, shiplog version, integrations, FAQ, blurbs) stays IN. Billing / subscription / auth / usage telemetry / feature flags / admin / dashboard / session namespaces stay OUT · 9-prefix isolation test `test_a17b_elevate_editor_vs_product_admin_boundary`. `prezzi.billing_toggle_options` OUT = 6th form-structure precedent (billing-state form-option-value tuple `(id, label)`). `demo.form_fields` + `form_sections` + `upload_field` OUT = 7th form-structure precedent (lead-form scaffolding).

4. **Simplest image profile of any enrolled archetype · 1 single image surface.** `home.product_demo_card.poster` nested-dict scalar · rendered · plain-key global. Simpler than Aura 12 · Famiglia 16 · Luxe 31 · Chiara 5 · Pragma 1 scalar top-level. Playwright walk verified 1 widget in sidebar DOM (`data-ed-image-key="home.product_demo_card.poster"`) · zero image-in-dict-row · zero deep-path. Posts list ABSENT by design (first enrollment where posts[] is truly absent from registry · not registry-only · Elevate is a landing-page template without project_detail records).

Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **19 archetype slugs enrolled · 19 multi-locale enrolled · 20 templates editable end-to-end · 20/20 catalog editable**. "19 enrolled" NOW means "every DNA archetype covered" — zero residual slugs remain outside the editor gate. All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → **375/375 PASS**
- `python smoke_full.py` → 834/834 routes HTTP 200

Baseline `phase-integration-baseline-v15` tip: **`3074b00`** (A.17b merge), pushed to `origin/phase-integration-baseline-v15`.
...
### Phase A.17+ — candidates (ZERO half-open families · Aura + Elevate single-template residues)

Pick one when the next workstream opens. A.16c closes the medical-other family · **all 3 staged family progressions have closed** · zero half-open families remain. Residue is 2 editor-only-pending templates across 2 single-template families:
- **agency-secondary** (1 template · `agency-digital-studio` Aura)
- **startup-saas** (1 template · `startup-saas-landing` Elevate)

Recommended framing: **A.17 Aura (`agency-digital-studio`) Step-0 planning session** — closes agency-secondary · brings catalog to 18 archetype slugs / 19 templates editable / 8 families closed · leaves only Elevate for A.17b. Single-template family · low novelty · mechanical reuse of established single-template recipe (precedent chain: Pragma A.6 · Lex A.10 · Juris A.11 · Casa A.12 · Villa A.12b · Chiara A.13 · Pixel A.13b · Sapore A.14 · Brace A.14b · Bottega A.15 · Luxe A.15b · Salute A.16 · Benessere A.16b · Famiglia A.16c). Outside-gate fixture already rotated to `aura-digital-studio` in A.16c as next candidate signal.

Alternatives: Elevate individual enrollment first (same structural profile · single-template · closes startup-saas category) · MEMORY.md maintenance mini-phase (housekeeping · separate · auto-memory index ~35-36KB sopra warning-soglia 24.4KB · could queue after A.17 + A.17b close everything as clean consolidation point) · `home.ambients` tuple-with-image widening mini-phase (deferred novel shape from A.16b · low value · customer-signal-gated).
