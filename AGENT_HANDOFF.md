...
Last updated: 2026-04-18 — after **Session 69 A.14 Sapore (trattoria-warm · restaurant-continuation family · first template) Editor + Multi-locale Enrollment merge · OPENS RESTAURANT-CONTINUATION FAMILY** (baseline tip `8fae2df`, pushed to origin)

## Current state — read this before opening any new workstream (2026-04-18)

**Eleven archetype slugs enrolled** — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) + **editorial-designer-grid** (Chiara) + **cinematic-photographer** (Pixel) + **trattoria-warm** (Sapore) — all multi-locale enrolled. **12 templates editable end-to-end.** A.14 opens the restaurant-continuation family via the same staged dedicated-schema progression pattern already validated twice (real-estate A.12→A.12b, portfolio A.13→A.13b): family opens with one dedicated archetype, closes in a second dedicated phase with the sibling archetype (Brace `street-modern` pending A.14b). **Four families editor-complete**: law (A.10+A.11 sequential dedicated-schema) · medical-specialist (A.9 shared-schema) · real-estate (A.12+A.12b staged dedicated-schema) · portfolio (A.13+A.13b staged dedicated-schema). A.14 introduces the **deep-path tuple-in-dict-list parent shape precedent** (Sapore `menu.sections.{i}.dishes`) via an **in-band contract-alignment bugfix** on the render-side splicer — the 3 editor layers that walk dotted paths (`services._resolve_path`, `schema._resolve_path`, `rendering._apply_indexed`) now speak the same language on list numeric indexing, closing a historical runtime incoherence that only surfaced with Sapore's novel shape. Menu rows stay **inside the perimeter** (tested end-to-end in 5 public-preview locales), keeping the enrollment credible. Detail-page registry-only uniform enforcement stays at **6 archetypes** (Lex/Juris/Casa/Villa/Chiara/Pixel) — Sapore's `posts` list is empty in the registry (structural absence, not a perimeter decision). Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **11 archetype slugs enrolled · 11 multi-locale enrolled · 12 templates editable end-to-end**. Note: "11 enrolled" does NOT mean "every catalog archetype covered" — the DNA registry carries additional slugs (`street-modern`, `medical-clinic`, `wellness-serene`, `family-warm`, `agency-digital-studio`, `startup-saas-landing`, `artisan-workshop`, `fashion-editorial`) that remain editor-unsupported (8 templates across 5 open families, one of which is half-open). All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → 252/252 PASS
...
- **Brace (`street-modern`) explicitly NOT enrolled** — dual guard inside `test_a14_sapore_archetype_registered` asserts `street-modern NOT in _ARCHETYPE_SCHEMAS NOR in _MULTILOCALE_ENABLED_ARCHETYPES` + all 10 pre-A.14 archetypes still enrolled. The lifecycle test additionally runs a runtime `assertNotIn("street-modern", ...)` at the end-of-test. Both guards will be removed in A.14b together with Brace's own registrations — contract-tested via `test_a14b_brace_out_guard_was_removed_from_sapore_tests` mirror (same pattern as A.12b Villa-out-removal and A.13b Pixel-out-removal).
...
### Phase A.14b+ — candidates (restaurant-continuation family half-open · Brace is the top candidate)

Pick one when the next workstream opens. A.14 opens a family; it does NOT close one. Recommended framing: **A.14b Brace (street-modern) enrollment — close the restaurant-continuation family** — natural continuation from A.14, mirrors A.12→A.12b and A.13→A.13b closures. Brace has heavier shape than Sapore (18 lists-of-dict vs 12 · 5 image-in-dict-row lists vs 2 · ~45 image surfaces vs 13 · food photos per dish in `menu.sections[].items[].image`) **but no new infrastructure needed** because A.14 Step 2 already aligned the 3 editor layers on list numeric indexing, and Chiara/Villa already precedentated dict-in-dict-list image cols. Recipe: mechanical-reuse of A.14 Sapore pattern. Expected schema LOC potentially above 600 (non-blocking exception acceptable). Alternatives: ecommerce (Bottega + Luxe · with Step-0 check on commerce-foundation overlap), medical-other (Salute/Benessere/Famiglia · three separate phases), Aura individual enrollment (closes agency-secondary), Elevate individual enrollment (closes startup-saas), editor polish / operator tools / remote storage (defer unless customer signal). MEMORY.md maintenance mini-phase remains queued as a separate housekeeping task (not bundled with an enrollment).
