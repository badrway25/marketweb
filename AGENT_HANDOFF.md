...
Last updated: 2026-04-19 — after **Session 71 A.15 Bottega (artisan-workshop · ecommerce family · first template) Editor + Multi-locale Enrollment merge · OPENS ECOMMERCE FAMILY** (baseline tip `fab3ebc`, pushed to origin)

## Current state — read this before opening any new workstream (2026-04-19)

**Thirteen archetype slugs enrolled** — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) + **editorial-designer-grid** (Chiara) + **cinematic-photographer** (Pixel) + **trattoria-warm** (Sapore) + **street-modern** (Brace) + **artisan-workshop** (Bottega) — all multi-locale enrolled. **14 templates editable end-to-end.** A.15 opens the ecommerce family via staged dedicated-schema progression — fourth consecutive staged opening after real-estate (A.12) · portfolio (A.13) · restaurant-continuation (A.14). Luxe (`fashion-editorial`) stays OUT until A.15b. **Five families editor-complete**: law (A.10+A.11 sequential dedicated-schema) · medical-specialist (A.9 shared-schema) · real-estate (A.12+A.12b staged dedicated-schema) · portfolio (A.13+A.13b staged dedicated-schema) · restaurant-continuation (A.14+A.14b staged dedicated-schema). **Ecommerce family HALF-OPEN** (Bottega IN · Luxe pending A.15b).

A.15 validates two critical framings:

1. **Editor edits SHOWCASE / template-content, NOT commerce-admin state**. Boundary runtime-verified: `apps/catalog/views.py::LiveTemplateView` does NOT import from `apps.commerce` · the editor surface (`/templates/ecommerce/<slug>/preview/...` · registry-driven demo content) and the commerce-admin surface (`/shop/...` · Storefront/Product/Variant/Cart/Order/PaymentIntent managed via seller dashboard Phase 3a/3b) are ORTHOGONAL. Zero cross-contamination by design. A light guardrail test (`test_a15_bottega_commerce_admin_boundary`) smoke-checks that the editor schema doesn't accidentally reference `apps.commerce` model namespaces. Any customer using the editor on Bottega is customizing the TEMPLATE SHOWCASE for a client pitch / portfolio demo. The real catalog lives elsewhere.

2. **Rendered vs storage-only image widget distinction is INTENTIONAL**. Bottega sidebar has 22 image widgets = 16 rendered + 6 storage-only. The 6 storage-only fields (`product.artisan_portrait` · `atelier.founder_portrait` · `home.makers.{0..3}.portrait`) are exposed in the editor because the registry data is preserved (Session 42 D-073 artisan-workshop typographic-first conversion: crest-mark glyphs + monogram stamps replace portrait photos · data kept for future skin variants). The 16 rendered fields (`home.latest_items.{0..3}.image` · `shop.products.{0..8}.image` · `product.related_items.{0..2}.image`) are product-thumbnail dict-row cells the skin actually paints. Editor storage contract (plain-key per-locale) is blindato identical for both categories; only VISIBILITY differs per skin rendering. This is the explicit handling of a pre-existing DNA-honest skin choice, not a bug.

Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **13 archetype slugs enrolled · 13 multi-locale enrolled · 14 templates editable end-to-end**. Note: "13 enrolled" does NOT mean "every catalog archetype covered" — the DNA registry carries additional slugs (`fashion-editorial`, `medical-clinic`, `wellness-serene`, `family-warm`, `agency-digital-studio`, `startup-saas-landing`) that remain editor-unsupported (6 templates across 4 open families, one of which is half-open). All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → 280/280 PASS
...
### Phase A.15b+ — candidates (ecommerce family half-open · Luxe chiude)

Pick one when the next workstream opens. A.15 opens a family; it does NOT close one. Recommended framing: **A.15b Luxe (`fashion-editorial`) Step-0 planning session** — closes the ecommerce family opened in A.15 with Bottega. Fourth consecutive staged closure after real-estate (A.12+A.12b) · portfolio (A.13+A.13b) · restaurant-continuation (A.14+A.14b). Luxe has heavier shape than Bottega (12 lists-of-dict vs 10 · 6 image-in-dict-row lists vs 4 · 31 image surfaces vs 22 · lookbook band adds image-heavy editorial · `.fe-*` skin with 21 RTL rules) but **no new infrastructure needed** · full mechanical reuse of A.15 Bottega recipe (nested-dict scalar + shallow image-in-dict-row · both precedented · boundary editor-vs-commerce-admin already runtime-verified). Alternatives: medical-other (Salute/Benessere/Famiglia · three separate phases preferred over bundling · A.6→A.15 discipline 14 phases strong) · Aura individual enrollment (closes agency-secondary) · Elevate individual enrollment (closes startup-saas) · editor polish / operator tools / remote storage (defer unless customer signal). MEMORY.md maintenance mini-phase remains queued as a separate housekeeping task (not bundled with an enrollment · auto-memory index ~32KB sopra warning-soglia 24.4KB).
