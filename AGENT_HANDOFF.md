...
Last updated: 2026-04-18 — after **Session 68 A.13b Pixel (cinematic-photographer · portfolio family) Editor + Multi-locale Enrollment merge · CLOSES PORTFOLIO FAMILY** (baseline tip `1c6b561`, pushed to origin)

## Current state — read this before opening any new workstream (2026-04-18)

**Ten archetype slugs enrolled** — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) + **editorial-designer-grid** (Chiara) + **cinematic-photographer** (Pixel) — all multi-locale enrolled. **11 templates editable end-to-end.** A.13b closes the portfolio family via the same staged dedicated-schema progression pattern validated by real-estate (A.12 Casa + A.12b Villa): family opens with one dedicated archetype, closes in a second dedicated phase with the sibling archetype. **Four families editor-complete**: law (A.10+A.11 sequential dedicated-schema) · medical-specialist (A.9 shared-schema) · real-estate (A.12+A.12b staged dedicated-schema) · **portfolio (A.13+A.13b staged dedicated-schema)**. A.13b consolidates the **single-scalar-image archetype enrollment pattern** (Pixel exposes only `home.hero_image` · no image-in-dict-row · no nested-dict image · no mutable repeaters) and brings the **detail-page registry-only uniform enforcement to 6 archetypes** (Lex/Juris/Casa/Villa/Chiara/Pixel · posts.* and series_detail stay registry-only). Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **10 archetype slugs enrolled · 10 multi-locale enrolled · 11 templates editable end-to-end**. Note: "10 enrolled" does NOT mean "every catalog archetype covered" — the DNA registry carries additional slugs (`trattoria-warm`, `street-modern`, `medical-clinic`, `wellness-serene`, `family-warm`, `agency-digital-studio`, `startup-saas-landing`, `artisan-workshop`, `fashion-editorial`) that remain editor-unsupported (9 templates across 5 open families). All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → 239/239 PASS
...
### Phase A.14+ — candidates (no commitment yet · portfolio family closed)

Pick one when the next workstream opens. A.13b closed a family; it does NOT prescribe the next workstream. Recommended framing: **A.14 restaurant-continuation (Sapore + Brace beyond Gusto)** — closes the restaurant category (currently 1/3 enrolled via Gusto). Two templates, likely distinct archetypes (`trattoria-warm` + `street-modern` per DNA registry). Step-0 audit decides topology; most probable outcome is sequential or staged dedicated-schema like real-estate / portfolio. Highest leverage residual family. Alternatives: ecommerce (Bottega + Luxe · with Step-0 check on commerce-foundation overlap), medical-other (Salute/Benessere/Famiglia · three separate phases), Aura individual enrollment (closes agency-secondary), Elevate individual enrollment (closes startup-saas), editor polish / operator tools / remote storage (defer unless customer signal). MEMORY.md maintenance mini-phase remains queued as a separate housekeeping task (not bundled with an enrollment).
