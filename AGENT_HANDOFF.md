...
Last updated: 2026-04-19 — after **Session 73 A.16 Salute (clinic · medical-other family · first template) Editor + Multi-locale Enrollment merge · OPENS MEDICAL-OTHER FAMILY** (baseline tip `e25b622`, pushed to origin)

## Current state — read this before opening any new workstream (2026-04-19)

**Fifteen archetype slugs enrolled** — Vertex (`agency-creative-studio`) + Pragma (`corporate-suite`) + Gusto (`fine-dining`) + **specialist** (Cardio + Derm shared) + **classic-gold** (Lex) + **modern-transparent** (Juris) + **mass-market** (Casa) + **ultra-luxury-cinematic** (Villa) + **editorial-designer-grid** (Chiara) + **cinematic-photographer** (Pixel) + **trattoria-warm** (Sapore) + **street-modern** (Brace) + **artisan-workshop** (Bottega) + **fashion-editorial** (Luxe) + **clinic** (Salute) — all multi-locale enrolled. **16 templates editable end-to-end.** A.16 opens the medical-other family via staged dedicated-schema progression EXTENDED TO **3-PHASE VARIANT** (first 3-template family to enroll · A.16 Salute opener · A.16b Benessere middle · A.16c Famiglia closer). **Six families editor-complete**: law (A.10+A.11 sequential dedicated-schema) · medical-specialist (A.9 shared-schema) · real-estate (A.12+A.12b staged dedicated-schema) · portfolio (A.13+A.13b staged dedicated-schema) · restaurant-continuation (A.14+A.14b staged dedicated-schema) · ecommerce (A.15+A.15b staged dedicated-schema). **Medical-other family HALF-OPEN** (Salute IN · Benessere/Famiglia pending A.16b/A.16c).

A.16 validates three critical framings:

1. **First 3-template staged progression — guard-removal sub-recipe extends to 2 removal phases**. Previous staged closures were 2-template families (real-estate · portfolio · restaurant-continuation · ecommerce). Medical-other has 3 templates with 3 distinct archetypes (clinic · wellness · family) and ~0% content-tree overlap (verified Step 0 audit) · shared-schema impossible. **DUAL-OUT GUARD** planted: Salute test_a16_salute_archetype_registered + lifecycle test asserts BOTH `wellness` + `family` OUT at 3 layers (registration-time + runtime start + runtime end). A.16b Benessere will remove `wellness`-out guard via `test_a16b_benessere_out_guard_was_removed_from_salute_tests` (5th precedent). A.16c Famiglia will remove `family`-out guard via `test_a16c_family_out_guard_was_removed_from_salute_tests` (6th precedent). Sub-recipe pattern extends from 1-removal to 2-removal phase, preserving D-098 invariant regardless of phase count.

2. **NOVEL 5th OUT category precedent: raw `icon_svg` fields**. Salute ships 18 raw SVG XML markup fields (`home.specialties[].icon_svg` × 8 · `servizi.services[].icon_svg` × 10) used for inline icon rendering on specialty/service cards. Editing raw SVG XML as plain text is unsafe (XSS-like content via paste · Pillow.verify only covers image binaries) and customer-unfriendly UX (no SVG-editor widget · adding one is a horizontal feature). **OUT col-level** via STRUCTURED_FIELD_SHAPES col exclusion + dedicated contract test `test_a16_salute_raw_svg_fields_excluded` blindates exclusion at 2 layers (`get_field_spec` returns None · `validate_key_path` raises). Raw SVG joins the 4 prior OUT categories: flat-list-of-str · nested-list-of-str · form structures · structural identifiers/commerce-state-like booleans.

3. **`is_popular` bool flag handled as audit-driven OUT** (preserves `popular_label` text editability). The editor schema currently supports text/textarea/richtext/image/url/select but not bool. Adding bool support would be a horizontal feature affecting every archetype with bool fields (Bottega/Luxe `available` already precedent). For Salute, `prevenzione.packages[].is_popular` (editorial highlight badge flag) stays OUT col-level while `popular_label` ('Più richiesto' badge text) stays editable IN — customer can change the badge label but not toggle which package is "most popular". Pragmatic limitation that preserves the no-horizontal-feature discipline. Same reasoning will apply to Benessere `home.calendar[].has_slots`/`.soldout` (commerce-state-like scheduler flags · pending A.16b OUT).

Catalog 20/20 `published_live` unchanged since D-082 / Session 53. Editor footprint: **15 archetype slugs enrolled · 15 multi-locale enrolled · 16 templates editable end-to-end**. Note: "15 enrolled" does NOT mean "every catalog archetype covered" — the DNA registry carries 4 residual slugs (`wellness`, `family`, `agency-digital-studio`, `startup-saas-landing`) that remain editor-unsupported (4 templates across 3 open families · 1 half-open). All acceptance gates are green:

- `python manage.py check` → 0 issues
- `python manage.py test apps` → **309/309 PASS**
- `python smoke_full.py` → 834/834 routes HTTP 200

Baseline `phase-integration-baseline-v15` tip: **`e25b622`** (A.16 merge), pushed to `origin/phase-integration-baseline-v15`.
...
### Phase A.16b+ — candidates (medical-other HALF-OPEN · Benessere chiude prossima · Famiglia chiude family)

Pick one when the next workstream opens. A.16 opens the medical-other family · half-open · 2 siblings pending. Residue is 4 editor-only-pending templates across 3 families (1 family half-open):
- **medical-other half-open** (2 templates · `wellness` Benessere + `family` Famiglia · pending A.16b + A.16c)
- **agency-secondary** (1 template · `agency-digital-studio` Aura)
- **startup-saas** (1 template · `startup-saas-landing` Elevate)

Recommended framing: **A.16b Benessere (`wellness`) Step-0 planning session** — middle phase of medical-other 3-phase progression · removes wellness-out guard half of the dual-out planted in A.16 (family-out stays active for A.16c). Half-open family is stronger ROI than opening a new front. Step-0 audit needed for: `home.calendar`/`prenota.calendar` `has_slots`/`soldout` bool flags (commerce-state-like · same OUT decision as Luxe `available` + Salute `is_popular`) · 1 novel page kind `gallery` · 19 image surfaces (3 scalar + 16 image-in-dict-row across 3 lists) · `contatti.form_fields.interest_options` nested str-list OUT. Outside-gate fixture already rotated to `wellness`/`benessere-centro-olistico` in A.16 as next candidate signal.

Alternatives: Famiglia individual enrollment first (deep-path tuple-in-dict-list · `crescita.topics[].items` Sapore-precedent shape · novel `faq` page kind · NO form_fields · could be deferred to A.16c if Step-0 audit identifies preference for tackling deep-path complexity later) · Aura individual enrollment (closes agency-secondary · pulls focus away from medical-other half-open · only justifiable if customer signal surfaces) · Elevate individual enrollment (same trade-off as Aura). MEMORY.md maintenance mini-phase remains queued as a separate housekeeping task (auto-memory index ~33-34KB sopra warning-soglia 24.4KB · grows ~1KB per enrollment phase · 16 phases A.6→A.16 accumulated).
